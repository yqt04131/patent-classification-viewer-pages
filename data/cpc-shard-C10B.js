window.CPC_SHARD_C10B = {
  "generatedAt": "2026-03-20T02:17:57.393Z",
  "total": 185,
  "entries": {
    "C10B": {
      "code": "C10B",
      "level": 5,
      "parent": "C10",
      "ja": "ガス，コークス，タールまたはこれらの類似物を製造するための炭素質物の分解乾留（鉱油の分解Ｃ１０Ｇ；鉱物の地下ガス化Ｅ２１Ｂ４３／２９５）",
      "en": "DESTRUCTIVE DISTILLATION OF CARBONACEOUS MATERIALS FOR PRODUCTION OF GAS, COKE, TAR, OR SIMILAR MATERIALS (cracking oils C10G; underground gasification of minerals E21B43/295)<br><br><u>WARNING</u><br><br>In this subclass non-limiting references (in the sense of paragraph 39 of the Guide to the IPC) may still be displayed in the scheme.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C10": {
      "code": "C10",
      "level": 4,
      "parent": "C",
      "ja": "石油，ガスまたはコークス工業；一酸化炭素を含有する工業ガス；燃料；潤滑剤；でい炭",
      "en": "PETROLEUM, GAS OR COKE INDUSTRIES; TECHNICAL GASES CONTAINING CARBON MONOXIDE; FUELS; LUBRICANTS; PEAT",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C": {
      "code": "C",
      "level": 2,
      "parent": null,
      "ja": "化学；冶金<br>（１）セクションＣにおいて，化学元素の種類に関する定義は次の通りである:<br>アルカリ金属:Ｌｉ，Ｎａ，Ｋ，Ｒｂ，Ｃｓ，Ｆｒ<br>アルカリ土類金属:Ｃａ，Ｓｒ，Ｂａ，Ｒａ<br>ランタニド:原子番号５７から７１までの元素<br>希土類:Ｓｃ，Ｙ，ランタニド<br>アクチニド:原子番号８９から１０３までの元素<br>耐火金属:Ｔｉ，Ｖ，Ｃｒ，Ｚｒ，Ｎｂ，Ｍｏ，Ｈｆ，Ｔａ，Ｗ<br>ハロゲン:Ｆ，Ｃｌ，Ｂｒ，Ｉ，Ａｔ<br>希ガス:Ｈｅ，Ｎｅ，Ａｒ，Ｋｒ，Ｘｅ，Ｒｎ<br>白金族:Ｏｓ，Ｉｒ，Ｐｔ，Ｒｕ，Ｒｈ，Ｐｄ<br>貴金属:Ａｇ，Ａｕ，白金族<br>軽金属:アルカリ金属，アルカリ土類金属，Ｂｅ，Ａｌ，Ｍｇ<br>重金属:軽金属以外の金属<br>鉄族:Ｆｅ，Ｃｏ，Ｎｉ<br>非金属:Ｈ，Ｂ，Ｃ，Ｓｉ，Ｎ，Ｐ，Ｏ，Ｓ，Ｓｅ，Ｔｅ，希ガス，ハロゲン<br>金属:非金属以外の元素<br>遷移元素:原子番号２１から３０まで，３９から４８まで，５７から８０まで，８９以上の元素<br>（２）Ｃセクションは以下のものを包含する:<br>（ａ）無機化合物，有機化合物，高分子化合物およびそれらの調製法を包含する純粋化学；<br>（ｂ）たとえばガラス，セラミック，肥料，プラスチック組成物，ペイント，石油工業の製品のような上記化合物を含有する組成物を包含する応用化学。それはまた，爆発物，染料，接着剤，潤滑剤および洗剤のように，ある目的に適した特別の性質をもつための組成物をも包含する；<br>（ｃ）コークスの製造，固体または気体燃料の製造，油脂およびろうの製造および精製，発酵工業（例．ビールおよびぶどう酒の製造），製糖工業のようなある境界工業；<br>（ｄ）ある操作または処理であって，純粋に機械的な処理，例．レザーおよび皮の機械的処理，または部分的機械的処理，例．水処理，または腐食防止一般であるもの；<br>（ｅ）冶金，鉄または非鉄合金<br>（３）ＩＰＣの全てのセクションにおいて，相反する指示がない限り、元素記号周期律表への言及は、以下に示す８つのグループを表す。例えば、グループＣ０７Ｆ３／００「周期律表の第２族の元素を含有する化合物」は、ＩＩＡ及びＩＩＢ族の元素への言及とする。<br>「図」<br>（４）（ａ）化学的部分または面および非化学的部分または面の両方を有する操作，処理，製品または物品の場合に，一般規則として，化学的部分または面はＣセクションに包含される。<br>（ｂ）これらのうちのある場合には，化学的部分または面には非化学的部分または面を伴い，純粋に機械的部分または面を伴うことさえある，というのは非化学的面は操作または処理にとって本質的であるかまたはそれらの重要な要素を構成するからである；実際，全体のさまざまな部分または面を分割しないのがより論理的と思える。これは応用化学や上記注（１）（ｃ），（ｄ）および（ｅ）の工業，操作および処理の場合に適用される。例えば，ガラス製造用特殊炉はＣ０３に包含されＦ２７には包含されない。<br>（ｃ）しかしながら，機械的面（または非化学的面）に非化学的な面が伴うという例外もある，例えば:<br>―サブクラスＡ６１Ｋにおけるある抽出プロセス；<br>―サブクラスＡ６１Ｌにおける空気の化学的精製；<br>―サブクラスＡ６２Ｄにおける化学的消火方法；<br>―クラスＢ０１における化学的プロセスおよび装置；<br>―サブクラスＢ２７Ｋにおける木材の含浸；<br>―サブクラスＧ０１Ｎにおける化学的分析または試験法；<br>―クラスＧ０３における写真材料またはプロセス，そして一般に，Ｄセクションにおける繊維の化学的処理および繊維素，紙等の製造<br>（ｄ）さらに他の場合，純粋化学的面はＣセクションに包含され，応用化学的面はＡ，Ｂ，Ｆセクションのような他のセクションに包含される，例．以下の物質または組成物の使用:<br>―サブクラスＡ０１Ｎに包含される植物または動物の処理；<br>―クラスＡ２３に包含される食物；<br>―クラスＦ４２に包含される弾薬または爆薬<br>（ｅ）化学的面および機械的面が非常に交錯して，簡単明確な分割ができない時，または化学的処理が自然なまたは論理的な延長として機械的プロセスを要する時は，化学的面に加えて，Ｃセクションは機械的面の一部のみ，例．クラスＣ０４に包含される人造石の後処理，を包含する。この後者の場合に，たとえその区別がむしろ任意であってもその位置を明確にするよう注または参照を通常設けている。",
      "en": "CHEMISTRY; METALLURGY<br><br><u>NOTE</u><br><br>In section C, the definitions of groups of chemical elements are as follows: <br>Alkali metals: Li, Na, K, Rb, Cs, Fr <br>Alkaline earth metals: Ca, Sr, Ba, Ra<br>Lanthanides: elements with atomic numbers 57 to 71 inclusive <br>Rare earths: Sc, Y, Lanthanides<br>Actinides: elements with atomic numbers 89 to 103 inclusive<br>Refractory metals: Ti, V, Cr, Zr, Nb, Mo, Hf, Ta, W <br>Halogens: F, Cl, Br, I, At <br>Noble gases: He, Ne, Ar, Kr, Xe, Rn <br>Platinum group: Os, Ir, Pt, Ru, Rh, Pd <br>Noble metals: Ag, Au, Platinum group <br>Light metals: alkali metals, alkaline earth metals, Be, Al, Mg<br>Heavy metals: metals other than light metals<br>Iron group: Fe, Co, Ni<br>Non-metals: H, B, C, Si, N, P, O, S, Se, Te, noble gases, halogens<br>Metals: elements other than non-metals <br>Transition elements: elements with atomic numbers 21 to 30 inclusive, 39 to 48 inclusive, 57 to 80 inclusive, 89 upwards<br>Section Ccovers:<br>pure chemistry, which covers inorganic compounds, organic compounds, macromolecular compounds, and their methods of preparation;<br>applied chemistry, which covers compositions containing the above compounds, such as: glass, ceramics, fertilisers, plastics compositions, paints, products of the petroleum industry. It also covers certain compositions on account of their having particular properties rendering them suitable for certain purposes, as in the case of explosives, dyestuffs, adhesives, lubricants and detergents;<br>certain marginal industries, such as the manufacture of coke and of solid or gaseous fuels, the production and refining of oils, fats and waxes, the fermentation industry (e.g., brewing and wine-making), the sugar industry;<br>certain operations or treatments, which are either purely mechanical, e.g., the mechanical treatment of leather and skins, or partly mechanical, e.g., the treatment of water or the prevention of corrosion in general;<br>metallurgy, ferrous or non-ferrous alloys.<br>In all sections of the CPC, in the absence of an indication to the contrary, the Periodic Table of chemical elements referred to is the one with 18 groups as represented in the table below.<br><br><br>In the case of operations, treatments, products or articles having both a chemical and a non-chemical part or aspect, the general rule is that the chemical part or aspect is covered by section C.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Notes (2) c), d) and e). For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example:<br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g. the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Note (2) bullets 2, 3, 4 and 5. For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example: <br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g., the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "C10B1/00": {
      "code": "C10B1/00",
      "level": 7,
      "parent": null,
      "ja": "レトルト",
      "en": "Retorts",
      "count": 173,
      "ipcCode": "C10B1/00",
      "coreCode": "C10B1/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B1/02": {
      "code": "C10B1/02",
      "level": 8,
      "parent": "C10B1/00",
      "ja": "固定式レトルト",
      "en": "Stationary retorts",
      "count": 121,
      "ipcCode": "C10B1/02",
      "coreCode": "C10B1/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B1/04": {
      "code": "C10B1/04",
      "level": 9,
      "parent": "C10B1/02",
      "ja": "垂直式レトルト",
      "en": "Vertical retorts",
      "count": 1775,
      "ipcCode": "C10B1/04",
      "coreCode": "C10B1/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B1/06": {
      "code": "C10B1/06",
      "level": 9,
      "parent": "C10B1/02",
      "ja": "水平式レトルト",
      "en": "Horizontal retorts",
      "count": 289,
      "ipcCode": "C10B1/06",
      "coreCode": "C10B1/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B1/08": {
      "code": "C10B1/08",
      "level": 9,
      "parent": "C10B1/02",
      "ja": "傾斜式レトルト",
      "en": "Inclined retorts",
      "count": 60,
      "ipcCode": "C10B1/08",
      "coreCode": "C10B1/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B1/10": {
      "code": "C10B1/10",
      "level": 8,
      "parent": "C10B1/00",
      "ja": "回転式レトルト",
      "en": "Rotary retorts",
      "count": 837,
      "ipcCode": "C10B1/10",
      "coreCode": "C10B1/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B3/00": {
      "code": "C10B3/00",
      "level": 7,
      "parent": null,
      "ja": "垂直室をもつコークス炉",
      "en": "Coke ovens with vertical chambers",
      "count": 286,
      "ipcCode": "C10B3/00",
      "coreCode": "C10B3/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B3/02": {
      "code": "C10B3/02",
      "level": 8,
      "parent": "C10B3/00",
      "ja": "熱交換装置をもつもの",
      "en": "with heat-exchange devices",
      "count": 236,
      "ipcCode": "C10B3/02",
      "coreCode": "C10B3/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/00": {
      "code": "C10B5/00",
      "level": 7,
      "parent": null,
      "ja": "水平室をもつコークス炉",
      "en": "Coke ovens with horizontal chambers",
      "count": 103,
      "ipcCode": "C10B5/00",
      "coreCode": "C10B5/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/02": {
      "code": "C10B5/02",
      "level": 8,
      "parent": "C10B5/00",
      "ja": "垂直な加熱用煙道をもつもの",
      "en": "with vertical heating flues",
      "count": 580,
      "ipcCode": "C10B5/02",
      "coreCode": "C10B5/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/04": {
      "code": "C10B5/04",
      "level": 9,
      "parent": "C10B5/02",
      "ja": "横断煙道をもつもの",
      "en": "with cross-over inter-connections",
      "count": 17,
      "ipcCode": "C10B5/04",
      "coreCode": "C10B5/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/06": {
      "code": "C10B5/06",
      "level": 8,
      "parent": "C10B5/00",
      "ja": "水平な加熱用煙道をもつもの",
      "en": "with horizontal heating flues",
      "count": 83,
      "ipcCode": "C10B5/06",
      "coreCode": "C10B5/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/08": {
      "code": "C10B5/08",
      "level": 8,
      "parent": "C10B5/00",
      "ja": "水平なおよび垂直な加熱用煙道をもつもの",
      "en": "with horizontal and vertical heating flues",
      "count": 9,
      "ipcCode": "C10B5/08",
      "coreCode": "C10B5/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/10": {
      "code": "C10B5/10",
      "level": 8,
      "parent": "C10B5/00",
      "ja": "熱交換装置をもつもの",
      "en": "with heat-exchange devices",
      "count": 278,
      "ipcCode": "C10B5/10",
      "coreCode": "C10B5/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/12": {
      "code": "C10B5/12",
      "level": 9,
      "parent": "C10B5/10",
      "ja": "蓄熱室をもつもの",
      "en": "with regenerators",
      "count": 136,
      "ipcCode": "C10B5/12",
      "coreCode": "C10B5/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/14": {
      "code": "C10B5/14",
      "level": 10,
      "parent": "C10B5/12",
      "ja": "炭化室の長さ方向に配置されているもの",
      "en": "situated in the longitudinal direction of the chambers",
      "count": 15,
      "ipcCode": "C10B5/14",
      "coreCode": "C10B5/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/16": {
      "code": "C10B5/16",
      "level": 11,
      "parent": "C10B5/14",
      "ja": "炉団別々の煙道をもつもの",
      "en": "with separated flues",
      "count": 18,
      "ipcCode": "C10B5/16",
      "coreCode": "C10B5/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/18": {
      "code": "C10B5/18",
      "level": 10,
      "parent": "C10B5/12",
      "ja": "炉団方向に配置されているもの",
      "en": "situated in the longitudinal direction of the oven battery",
      "count": 6,
      "ipcCode": "C10B5/18",
      "coreCode": "C10B5/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B5/20": {
      "code": "C10B5/20",
      "level": 9,
      "parent": "C10B5/10",
      "ja": "換熱器をもつもの",
      "en": "with recuperators",
      "count": 66,
      "ipcCode": "C10B5/20",
      "coreCode": "C10B5/20",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/00": {
      "code": "C10B7/00",
      "level": 7,
      "parent": null,
      "ja": "炉内に原料を送り込む機械的手段をもつコークス炉",
      "en": "Coke ovens with mechanical conveying means for the raw material inside the oven",
      "count": 263,
      "ipcCode": "C10B7/00",
      "coreCode": "C10B7/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/02": {
      "code": "C10B7/02",
      "level": 8,
      "parent": "C10B7/00",
      "ja": "回転式かき取り装置をもつもの",
      "en": "with rotary scraping devices",
      "count": 357,
      "ipcCode": "C10B7/02",
      "coreCode": "C10B7/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/04": {
      "code": "C10B7/04",
      "level": 8,
      "parent": "C10B7/00",
      "ja": "動揺または震動装置をもつもの",
      "en": "with shaking or vibrating devices",
      "count": 58,
      "ipcCode": "C10B7/04",
      "coreCode": "C10B7/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/06": {
      "code": "C10B7/06",
      "level": 8,
      "parent": "C10B7/00",
      "ja": "エンドレス・コンベヤをもつもの",
      "en": "with endless conveying devices",
      "count": 302,
      "ipcCode": "C10B7/06",
      "coreCode": "C10B7/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/08": {
      "code": "C10B7/08",
      "level": 9,
      "parent": "C10B7/06",
      "ja": "垂直方向式のもの",
      "en": "in vertical direction",
      "count": 16,
      "ipcCode": "C10B7/08",
      "coreCode": "C10B7/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/10": {
      "code": "C10B7/10",
      "level": 8,
      "parent": "C10B7/00",
      "ja": "コンベヤスクリュウをもつもの",
      "en": "with conveyor-screws",
      "count": 462,
      "ipcCode": "C10B7/10",
      "coreCode": "C10B7/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/12": {
      "code": "C10B7/12",
      "level": 8,
      "parent": "C10B7/00",
      "ja": "傾注または揺動突き上げ手段を有するもの",
      "en": "with tilting or rocking means",
      "count": 5,
      "ipcCode": "C10B7/12",
      "coreCode": "C10B7/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B7/14": {
      "code": "C10B7/14",
      "level": 8,
      "parent": "C10B7/00",
      "ja": "トロッコ，コンテナまたは台車をもつもの",
      "en": "with trucks, containers, or trays",
      "count": 310,
      "ipcCode": "C10B7/14",
      "coreCode": "C10B7/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B9/00": {
      "code": "C10B9/00",
      "level": 7,
      "parent": null,
      "ja": "ビーハイブ炉",
      "en": "Beehive ovens",
      "count": 64,
      "ipcCode": "C10B9/00",
      "coreCode": "C10B9/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B11/00": {
      "code": "C10B11/00",
      "level": 7,
      "parent": null,
      "ja": "傾斜室をもつコークス炉",
      "en": "Coke ovens with inclined chambers",
      "count": 79,
      "ipcCode": "C10B11/00",
      "coreCode": "C10B11/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B13/00": {
      "code": "C10B13/00",
      "level": 7,
      "parent": null,
      "ja": "機械的な圧力下に原料を運搬および保持する手段をもつコークス炉",
      "en": "Coke ovens with means for bringing and keeping the charge under mechanical pressure",
      "count": 64,
      "ipcCode": "C10B13/00",
      "coreCode": "C10B13/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B15/00": {
      "code": "C10B15/00",
      "level": 7,
      "parent": null,
      "ja": "その他のコークス炉",
      "en": "Other coke ovens",
      "count": 113,
      "ipcCode": "C10B15/00",
      "coreCode": "C10B15/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B15/02": {
      "code": "C10B15/02",
      "level": 8,
      "parent": "C10B15/00",
      "ja": "炉底加熱のもの",
      "en": "with floor heating",
      "count": 209,
      "ipcCode": "C10B15/02",
      "coreCode": "C10B15/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B17/00": {
      "code": "C10B17/00",
      "level": 7,
      "parent": null,
      "ja": "コークス炉の予熱",
      "en": "Preheating of coke ovens",
      "count": 56,
      "ipcCode": "C10B17/00",
      "coreCode": "C10B17/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B19/00": {
      "code": "C10B19/00",
      "level": 7,
      "parent": null,
      "ja": "電気的手段によるコークス炉の加熱",
      "en": "Heating of coke ovens by electrical means",
      "count": 544,
      "ipcCode": "C10B19/00",
      "coreCode": "C10B19/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/00": {
      "code": "C10B21/00",
      "level": 7,
      "parent": null,
      "ja": "可燃性ガスによるコークス炉の加熱",
      "en": "Heating of coke ovens with combustible gases",
      "count": 153,
      "ipcCode": "C10B21/00",
      "coreCode": "C10B21/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/02": {
      "code": "C10B21/02",
      "level": 8,
      "parent": "C10B21/00",
      "ja": "貧ガスによるもの",
      "en": "with lean gas",
      "count": 20,
      "ipcCode": "C10B21/02",
      "coreCode": "C10B21/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/04": {
      "code": "C10B21/04",
      "level": 8,
      "parent": "C10B21/00",
      "ja": "富ガスによるもの",
      "en": "with rich gas",
      "count": 25,
      "ipcCode": "C10B21/04",
      "coreCode": "C10B21/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/06": {
      "code": "C10B21/06",
      "level": 8,
      "parent": "C10B21/00",
      "ja": "貧ガスまたは富ガスの使用に適したコークス炉の加熱",
      "en": "in coke ovens suitable for the use of lean gas or rich gas",
      "count": 14,
      "ipcCode": "C10B21/06",
      "coreCode": "C10B21/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/08": {
      "code": "C10B21/08",
      "level": 8,
      "parent": "C10B21/00",
      "ja": "特殊な加熱用ガスを使用するもの",
      "en": "by applying special heating gases",
      "count": 60,
      "ipcCode": "C10B21/08",
      "coreCode": "C10B21/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/10": {
      "code": "C10B21/10",
      "level": 8,
      "parent": "C10B21/00",
      "ja": "燃焼を調節および制御するもの",
      "en": "Regulating and controlling the combustion",
      "count": 683,
      "ipcCode": "C10B21/10",
      "coreCode": "C10B21/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/12": {
      "code": "C10B21/12",
      "level": 9,
      "parent": "C10B21/10",
      "ja": "バーナ",
      "en": "Burners",
      "count": 111,
      "ipcCode": "C10B21/12",
      "coreCode": "C10B21/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/14": {
      "code": "C10B21/14",
      "level": 9,
      "parent": "C10B21/10",
      "ja": "ドラフトを逆転させる装置",
      "en": "Devices for reversing the draught",
      "count": 157,
      "ipcCode": "C10B21/14",
      "coreCode": "C10B21/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/16": {
      "code": "C10B21/16",
      "level": 9,
      "parent": "C10B21/10",
      "ja": "加熱用煙道と蓄熱室との間の開度を制御または変化させるもの",
      "en": "by controlling or varying the openings between the heating flues and the regenerator flues",
      "count": 114,
      "ipcCode": "C10B21/16",
      "coreCode": "C10B21/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/18": {
      "code": "C10B21/18",
      "level": 9,
      "parent": "C10B21/10",
      "ja": "煙道ガスを再循環させるもの",
      "en": "Recirculating the flue gases",
      "count": 162,
      "ipcCode": "C10B21/18",
      "coreCode": "C10B21/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/20": {
      "code": "C10B21/20",
      "level": 8,
      "parent": "C10B21/00",
      "ja": "室式炉の加熱方法",
      "en": "Methods of heating ovens of the chamber oven type",
      "count": 358,
      "ipcCode": "C10B21/20",
      "coreCode": "C10B21/20",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/22": {
      "code": "C10B21/22",
      "level": 9,
      "parent": "C10B21/20",
      "ja": "加熱用ガスおよび空気を種々の高さで導入する方法",
      "en": "by introducing the heating gas and air at various levels",
      "count": 195,
      "ipcCode": "C10B21/22",
      "coreCode": "C10B21/22",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/24": {
      "code": "C10B21/24",
      "level": 10,
      "parent": "C10B21/22",
      "ja": "垂直の加熱用煙突の頂部および底部に導入するもの",
      "en": "at the top and the bottom of the vertical heating flues",
      "count": 10,
      "ipcCode": "C10B21/24",
      "coreCode": "C10B21/24",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B21/26": {
      "code": "C10B21/26",
      "level": 9,
      "parent": "C10B21/20",
      "ja": "垂直の加熱煙道の頂部にのみ加熱用ガスおよび空気を導入する方法",
      "en": "by introducing the heating gas and air at the top of the vertical flues only",
      "count": 5,
      "ipcCode": "C10B21/26",
      "coreCode": "C10B21/26",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B23/00": {
      "code": "C10B23/00",
      "level": 7,
      "parent": null,
      "ja": "コークス炉のその他の加熱方法",
      "en": "Other methods of heating coke ovens",
      "count": 94,
      "ipcCode": "C10B23/00",
      "coreCode": "C10B23/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/00": {
      "code": "C10B25/00",
      "level": 7,
      "parent": null,
      "ja": "コークス炉のドアまたは密閉装置",
      "en": "Doors or closures for coke ovens",
      "count": 49,
      "ipcCode": "C10B25/00",
      "coreCode": "C10B25/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/02": {
      "code": "C10B25/02",
      "level": 8,
      "parent": "C10B25/00",
      "ja": "ドア；ドアのフレーム",
      "en": "Doors; Door frames",
      "count": 157,
      "ipcCode": "C10B25/02",
      "coreCode": "C10B25/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/04": {
      "code": "C10B25/04",
      "level": 9,
      "parent": "C10B25/02",
      "ja": "垂直室をもつ炉に使用するもの",
      "en": "for ovens with vertical chambers",
      "count": 109,
      "ipcCode": "C10B25/04",
      "coreCode": "C10B25/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/06": {
      "code": "C10B25/06",
      "level": 9,
      "parent": "C10B25/02",
      "ja": "水平室をもつ炉に使用するもの",
      "en": "for ovens with horizontal chambers",
      "count": 490,
      "ipcCode": "C10B25/06",
      "coreCode": "C10B25/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/08": {
      "code": "C10B25/08",
      "level": 9,
      "parent": "C10B25/02",
      "ja": "ドアの開閉",
      "en": "Closing and opening the doors",
      "count": 118,
      "ipcCode": "C10B25/08",
      "coreCode": "C10B25/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/10": {
      "code": "C10B25/10",
      "level": 10,
      "parent": "C10B25/08",
      "ja": "垂直室をもつ炉のためのもの",
      "en": "for ovens with vertical chambers",
      "count": 167,
      "ipcCode": "C10B25/10",
      "coreCode": "C10B25/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/12": {
      "code": "C10B25/12",
      "level": 10,
      "parent": "C10B25/08",
      "ja": "水平室をもつ炉のためのもの",
      "en": "for ovens with horizontal chambers",
      "count": 308,
      "ipcCode": "C10B25/12",
      "coreCode": "C10B25/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/14": {
      "code": "C10B25/14",
      "level": 10,
      "parent": "C10B25/08",
      "ja": "蓋取り装置",
      "en": "Devices for lifting doors",
      "count": 336,
      "ipcCode": "C10B25/14",
      "coreCode": "C10B25/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/16": {
      "code": "C10B25/16",
      "level": 9,
      "parent": "C10B25/02",
      "ja": "密閉；密閉の方法",
      "en": "Sealing; Means for sealing",
      "count": 591,
      "ipcCode": "C10B25/16",
      "coreCode": "C10B25/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/18": {
      "code": "C10B25/18",
      "level": 9,
      "parent": "C10B25/02",
      "ja": "冷却",
      "en": "Cooling",
      "count": 8,
      "ipcCode": "C10B25/18",
      "coreCode": "C10B25/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/20": {
      "code": "C10B25/20",
      "level": 8,
      "parent": "C10B25/00",
      "ja": "装入口の蓋または密閉装置",
      "en": "Lids or closures for charging holes",
      "count": 176,
      "ipcCode": "C10B25/20",
      "coreCode": "C10B25/20",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/22": {
      "code": "C10B25/22",
      "level": 9,
      "parent": "C10B25/20",
      "ja": "垂直室をもつ炉のためのもの",
      "en": "for ovens with vertical chambers",
      "count": 36,
      "ipcCode": "C10B25/22",
      "coreCode": "C10B25/22",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B25/24": {
      "code": "C10B25/24",
      "level": 9,
      "parent": "C10B25/20",
      "ja": "水平室をもつ炉のためのもの",
      "en": "for ovens with horizontal chambers",
      "count": 154,
      "ipcCode": "C10B25/24",
      "coreCode": "C10B25/24",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B27/00": {
      "code": "C10B27/00",
      "level": 7,
      "parent": null,
      "ja": "乾留ガスの回収装置",
      "en": "Arrangements for withdrawal of the distillation gases",
      "count": 911,
      "ipcCode": "C10B27/00",
      "coreCode": "C10B27/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B27/02": {
      "code": "C10B27/02",
      "level": 8,
      "parent": "C10B27/00",
      "ja": "炭化室の種々の高さに配置した回収口をもつもの",
      "en": "with outlets arranged at different levels in the chamber",
      "count": 158,
      "ipcCode": "C10B27/02",
      "coreCode": "C10B27/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B27/04": {
      "code": "C10B27/04",
      "level": 8,
      "parent": "C10B27/00",
      "ja": "炉の装入作業中のもの",
      "en": "during the charging operation of the oven",
      "count": 279,
      "ipcCode": "C10B27/04",
      "coreCode": "C10B27/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B27/06": {
      "code": "C10B27/06",
      "level": 8,
      "parent": "C10B27/00",
      "ja": "導管の細部，例．弁",
      "en": "Conduit details, e.g. valves",
      "count": 826,
      "ipcCode": "C10B27/06",
      "coreCode": "C10B27/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B29/00": {
      "code": "C10B29/00",
      "level": 7,
      "parent": null,
      "ja": "コークス炉のその他の細部",
      "en": "Other details of coke ovens",
      "count": 493,
      "ipcCode": "C10B29/00",
      "coreCode": "C10B29/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B29/02": {
      "code": "C10B29/02",
      "level": 8,
      "parent": "C10B29/00",
      "ja": "れんが積み，例．ケーシング，ライニング，壁",
      "en": "Brickwork, e.g. casings, linings, walls",
      "count": 696,
      "ipcCode": "C10B29/02",
      "coreCode": "C10B29/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B29/04": {
      "code": "C10B29/04",
      "level": 8,
      "parent": "C10B29/00",
      "ja": "膨張または収縮を制御あるいは防止するもの",
      "en": "Controlling or preventing expansion or contraction",
      "count": 90,
      "ipcCode": "C10B29/04",
      "coreCode": "C10B29/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B29/06": {
      "code": "C10B29/06",
      "level": 8,
      "parent": "C10B29/00",
      "ja": "れんが積みのガス漏えいを防止または修理するもの",
      "en": "Preventing or repairing leakages of the brickwork",
      "count": 340,
      "ipcCode": "C10B29/06",
      "coreCode": "C10B29/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B29/08": {
      "code": "C10B29/08",
      "level": 8,
      "parent": "C10B29/00",
      "ja": "炉の締付けまたは基礎",
      "en": "Bracing or foundation of the ovens",
      "count": 143,
      "ipcCode": "C10B29/08",
      "coreCode": "C10B29/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/00": {
      "code": "C10B31/00",
      "level": 7,
      "parent": null,
      "ja": "装入装置",
      "en": "Charging devices",
      "count": 591,
      "ipcCode": "C10B31/00",
      "coreCode": "C10B31/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/02": {
      "code": "C10B31/02",
      "level": 8,
      "parent": "C10B31/00",
      "ja": "垂直に装入するためのもの",
      "en": "for charging vertically",
      "count": 359,
      "ipcCode": "C10B31/02",
      "coreCode": "C10B31/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/04": {
      "code": "C10B31/04",
      "level": 9,
      "parent": "C10B31/02",
      "ja": "水平室をもつコークス炉",
      "en": "coke ovens with horizontal chambers",
      "count": 531,
      "ipcCode": "C10B31/04",
      "coreCode": "C10B31/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/06": {
      "code": "C10B31/06",
      "level": 8,
      "parent": "C10B31/00",
      "ja": "水平に供給するためのもの",
      "en": "for charging horizontally",
      "count": 163,
      "ipcCode": "C10B31/06",
      "coreCode": "C10B31/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/08": {
      "code": "C10B31/08",
      "level": 9,
      "parent": "C10B31/06",
      "ja": "水平室を有するコークス炉",
      "en": "coke ovens with horizontal chambers",
      "count": 56,
      "ipcCode": "C10B31/08",
      "coreCode": "C10B31/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/10": {
      "code": "C10B31/10",
      "level": 10,
      "parent": "C10B31/08",
      "ja": "１度のコンパクトな装入を行なうもの",
      "en": "with one compact charge",
      "count": 129,
      "ipcCode": "C10B31/10",
      "coreCode": "C10B31/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B31/12": {
      "code": "C10B31/12",
      "level": 8,
      "parent": "C10B31/00",
      "ja": "液体物質のためのもの",
      "en": "for liquid materials",
      "count": 48,
      "ipcCode": "C10B31/12",
      "coreCode": "C10B31/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/00": {
      "code": "C10B33/00",
      "level": 7,
      "parent": null,
      "ja": "排出装置；コークス案内装置",
      "en": "Discharging devices; Coke guides",
      "count": 267,
      "ipcCode": "C10B33/00",
      "coreCode": "C10B33/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/003": {
      "code": "C10B33/003",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "｛無公害排出のための配置｝",
      "en": "{Arrangements for pollution-free discharge}",
      "count": 352,
      "ipcCode": "C10B33/00",
      "coreCode": "C10B33/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/006": {
      "code": "C10B33/006",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "",
      "en": "{Decoking tools, e.g. hydraulic coke removing tools with boring or cutting nozzles}",
      "count": 170,
      "ipcCode": "C10B33/00",
      "coreCode": "C10B33/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/02": {
      "code": "C10B33/02",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "固定装置によるコークスの引出し，例．ギヤ，スクリュー",
      "en": "Extracting coke with built-in devices, e.g. gears, screws",
      "count": 337,
      "ipcCode": "C10B33/02",
      "coreCode": "C10B33/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/04": {
      "code": "C10B33/04",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "引出し装置",
      "en": "Pulling-out devices",
      "count": 65,
      "ipcCode": "C10B33/04",
      "coreCode": "C10B33/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/06": {
      "code": "C10B33/06",
      "level": 9,
      "parent": "C10B33/04",
      "ja": "水平室のためのもの",
      "en": "for horizontal chambers",
      "count": 21,
      "ipcCode": "C10B33/06",
      "coreCode": "C10B33/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/08": {
      "code": "C10B33/08",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "押出し装置，例．ラム",
      "en": "Pushers, e.g. rams",
      "count": 190,
      "ipcCode": "C10B33/08",
      "coreCode": "C10B33/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/10": {
      "code": "C10B33/10",
      "level": 9,
      "parent": "C10B33/08",
      "ja": "水平室のためのもの",
      "en": "for horizontal chambers",
      "count": 235,
      "ipcCode": "C10B33/10",
      "coreCode": "C10B33/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/12": {
      "code": "C10B33/12",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "排出弁",
      "en": "Discharge valves",
      "count": 70,
      "ipcCode": "C10B33/12",
      "coreCode": "C10B33/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B33/14": {
      "code": "C10B33/14",
      "level": 8,
      "parent": "C10B33/00",
      "ja": "コークス案内装置",
      "en": "Coke guides",
      "count": 187,
      "ipcCode": "C10B33/14",
      "coreCode": "C10B33/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B35/00": {
      "code": "C10B35/00",
      "level": 7,
      "parent": null,
      "ja": "装入および排出の組み合わされた装置",
      "en": "Combined charging and discharging devices",
      "count": 113,
      "ipcCode": "C10B35/00",
      "coreCode": "C10B35/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B37/00": {
      "code": "C10B37/00",
      "level": 7,
      "parent": null,
      "ja": "炉内への石炭を装入する機械的処理",
      "en": "Mechanical treatments of coal charges in the oven",
      "count": 68,
      "ipcCode": "C10B37/00",
      "coreCode": "C10B37/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B37/02": {
      "code": "C10B37/02",
      "level": 8,
      "parent": "C10B37/00",
      "ja": "ならし装入，例．棒によるもの",
      "en": "Levelling charges, e.g. with bars",
      "count": 264,
      "ipcCode": "C10B37/02",
      "coreCode": "C10B37/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B37/04": {
      "code": "C10B37/04",
      "level": 8,
      "parent": "C10B37/00",
      "ja": "加圧装入（コークス化中のものはＣ１０Ｂ４７／１２）",
      "en": "Compressing charges (during coking C10B47/12)",
      "count": 139,
      "ipcCode": "C10B37/04",
      "coreCode": "C10B37/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B37/06": {
      "code": "C10B37/06",
      "level": 8,
      "parent": "C10B37/00",
      "ja": "装入物中に孔穴部の形成",
      "en": "Forming holes in charges",
      "count": 29,
      "ipcCode": "C10B37/06",
      "coreCode": "C10B37/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/00": {
      "code": "C10B39/00",
      "level": 7,
      "parent": null,
      "ja": "コークスの冷却または消火",
      "en": "Cooling or quenching coke",
      "count": 207,
      "ipcCode": "C10B39/00",
      "coreCode": "C10B39/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/02": {
      "code": "C10B39/02",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "炉外乾式冷却",
      "en": "Dry cooling outside the oven",
      "count": 1564,
      "ipcCode": "C10B39/02",
      "coreCode": "C10B39/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/04": {
      "code": "C10B39/04",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "湿式消火",
      "en": "Wet quenching",
      "count": 605,
      "ipcCode": "C10B39/04",
      "coreCode": "C10B39/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/06": {
      "code": "C10B39/06",
      "level": 9,
      "parent": "C10B39/04",
      "ja": "炉内におけるもの",
      "en": "in the oven",
      "count": 62,
      "ipcCode": "C10B39/06",
      "coreCode": "C10B39/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/08": {
      "code": "C10B39/08",
      "level": 9,
      "parent": "C10B39/04",
      "ja": "コークス消火塔",
      "en": "Coke-quenching towers",
      "count": 167,
      "ipcCode": "C10B39/08",
      "coreCode": "C10B39/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/10": {
      "code": "C10B39/10",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "かくはん手段との組み合わせ，例．回転テーブルまたはドラム",
      "en": "combined with agitating means, e.g. rotating tables or drums",
      "count": 71,
      "ipcCode": "C10B39/10",
      "coreCode": "C10B39/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/12": {
      "code": "C10B39/12",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "運搬手段との組み合わせ",
      "en": "combined with conveying means",
      "count": 133,
      "ipcCode": "C10B39/12",
      "coreCode": "C10B39/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/14": {
      "code": "C10B39/14",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "車",
      "en": "Cars",
      "count": 449,
      "ipcCode": "C10B39/14",
      "coreCode": "C10B39/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/16": {
      "code": "C10B39/16",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "選別との組み合わせ",
      "en": "combined with sorting",
      "count": 27,
      "ipcCode": "C10B39/16",
      "coreCode": "C10B39/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B39/18": {
      "code": "C10B39/18",
      "level": 8,
      "parent": "C10B39/00",
      "ja": "コークスの傾斜（コークワーク）",
      "en": "Coke ramps",
      "count": 73,
      "ipcCode": "C10B39/18",
      "coreCode": "C10B39/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B41/00": {
      "code": "C10B41/00",
      "level": 7,
      "parent": null,
      "ja": "安全装置，例．コークスの排出に使用する信号装置または制御装置",
      "en": "Safety devices, e.g. signalling or controlling devices for use in the discharge of coke",
      "count": 985,
      "ipcCode": "C10B41/00",
      "coreCode": "C10B41/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B41/005": {
      "code": "C10B41/005",
      "level": 8,
      "parent": "C10B41/00",
      "ja": "",
      "en": "{for charging coal}",
      "count": 180,
      "ipcCode": "C10B41/00",
      "coreCode": "C10B41/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B41/02": {
      "code": "C10B41/02",
      "level": 8,
      "parent": "C10B41/00",
      "ja": "コークス排出のためのもの",
      "en": "for discharging coke",
      "count": 161,
      "ipcCode": "C10B41/02",
      "coreCode": "C10B41/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B41/04": {
      "code": "C10B41/04",
      "level": 9,
      "parent": "C10B41/02",
      "ja": "電気的手段によるもの",
      "en": "by electrical means",
      "count": 148,
      "ipcCode": "C10B41/04",
      "coreCode": "C10B41/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B41/06": {
      "code": "C10B41/06",
      "level": 9,
      "parent": "C10B41/02",
      "ja": "空気圧式または水圧式手段によるもの",
      "en": "by pneumatic or hydraulic means",
      "count": 15,
      "ipcCode": "C10B41/06",
      "coreCode": "C10B41/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B41/08": {
      "code": "C10B41/08",
      "level": 8,
      "parent": "C10B41/00",
      "ja": "乾留ガスの回収のためのもの",
      "en": "for the withdrawal of the distillation gases",
      "count": 295,
      "ipcCode": "C10B41/08",
      "coreCode": "C10B41/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/00": {
      "code": "C10B43/00",
      "level": 7,
      "parent": null,
      "ja": "固着物の防止または除去",
      "en": "Preventing or removing incrustations",
      "count": 320,
      "ipcCode": "C10B43/00",
      "coreCode": "C10B43/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/02": {
      "code": "C10B43/02",
      "level": 8,
      "parent": "C10B43/00",
      "ja": "固着物の除去",
      "en": "Removing incrustations",
      "count": 207,
      "ipcCode": "C10B43/02",
      "coreCode": "C10B43/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/04": {
      "code": "C10B43/04",
      "level": 9,
      "parent": "C10B43/02",
      "ja": "機械的手段によるもの",
      "en": "by mechanical means",
      "count": 544,
      "ipcCode": "C10B43/04",
      "coreCode": "C10B43/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/06": {
      "code": "C10B43/06",
      "level": 10,
      "parent": "C10B43/04",
      "ja": "導管，弁および類似物から",
      "en": "from conduits, valves or the like",
      "count": 161,
      "ipcCode": "C10B43/06",
      "coreCode": "C10B43/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/08": {
      "code": "C10B43/08",
      "level": 9,
      "parent": "C10B43/02",
      "ja": "液体によるもの",
      "en": "with liquids",
      "count": 156,
      "ipcCode": "C10B43/08",
      "coreCode": "C10B43/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/10": {
      "code": "C10B43/10",
      "level": 9,
      "parent": "C10B43/02",
      "ja": "焼却によるもの",
      "en": "by burning out",
      "count": 72,
      "ipcCode": "C10B43/10",
      "coreCode": "C10B43/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/12": {
      "code": "C10B43/12",
      "level": 10,
      "parent": "C10B43/10",
      "ja": "バーナ",
      "en": "Burners",
      "count": 5,
      "ipcCode": "C10B43/12",
      "coreCode": "C10B43/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B43/14": {
      "code": "C10B43/14",
      "level": 8,
      "parent": "C10B43/00",
      "ja": "固着物の防止",
      "en": "Preventing incrustations",
      "count": 237,
      "ipcCode": "C10B43/14",
      "coreCode": "C10B43/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B45/00": {
      "code": "C10B45/00",
      "level": 7,
      "parent": null,
      "ja": "その他の細部",
      "en": "Other details",
      "count": 1055,
      "ipcCode": "C10B45/00",
      "coreCode": "C10B45/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B45/005": {
      "code": "C10B45/005",
      "level": 8,
      "parent": "C10B45/00",
      "ja": "",
      "en": "{Devices for recovering spilled coke, e.g. recovering the coke falling out the oven when opening doors or withdrawing the leveler bar}",
      "count": 145,
      "ipcCode": "C10B45/00",
      "coreCode": "C10B45/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B45/02": {
      "code": "C10B45/02",
      "level": 8,
      "parent": "C10B45/00",
      "ja": "炉外で圧縮一様化した装入石炭を製造するための装置（ブリケットプレスＢ３０Ｂ）",
      "en": "Devices for producing compact unified coal charges outside the oven (briquetting presses B30B)",
      "count": 298,
      "ipcCode": "C10B45/02",
      "coreCode": "C10B45/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/00": {
      "code": "C10B47/00",
      "level": 7,
      "parent": null,
      "ja": "間接加熱による固体炭素質物の乾留，例．外部燃焼によるもの",
      "en": "Destructive distillation of solid carbonaceous materials with indirect heating, e.g. by external combustion",
      "count": 294,
      "ipcCode": "C10B47/00",
      "coreCode": "C10B47/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/02": {
      "code": "C10B47/02",
      "level": 8,
      "parent": "C10B47/00",
      "ja": "固定式装入によるもの",
      "en": "with stationary charge",
      "count": 178,
      "ipcCode": "C10B47/02",
      "coreCode": "C10B47/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/04": {
      "code": "C10B47/04",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "シャフトにおけるもの",
      "en": "in shaft furnaces",
      "count": 68,
      "ipcCode": "C10B47/04",
      "coreCode": "C10B47/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/06": {
      "code": "C10B47/06",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "レトルトにおけるもの",
      "en": "in retorts",
      "count": 178,
      "ipcCode": "C10B47/06",
      "coreCode": "C10B47/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/08": {
      "code": "C10B47/08",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "ビーハイブ炉におけるもの",
      "en": "in beehive ovens",
      "count": 2,
      "ipcCode": "C10B47/08",
      "coreCode": "C10B47/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/10": {
      "code": "C10B47/10",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "室式コークス炉におけるもの",
      "en": "in coke ovens of the chamber type",
      "count": 37,
      "ipcCode": "C10B47/10",
      "coreCode": "C10B47/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/12": {
      "code": "C10B47/12",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "その場合コークス化中装入物が機械的圧力を受けるもの",
      "en": "in which the charge is subjected to mechanical pressures during coking",
      "count": 40,
      "ipcCode": "C10B47/12",
      "coreCode": "C10B47/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/14": {
      "code": "C10B47/14",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "高温液体の助けによるもの，例．溶融塩",
      "en": "with the aid of hot liquids, e.g. molten salts",
      "count": 34,
      "ipcCode": "C10B47/14",
      "coreCode": "C10B47/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/16": {
      "code": "C10B47/16",
      "level": 9,
      "parent": "C10B47/02",
      "ja": "レトルトの内側および外側からの間接加熱手段をもつもの",
      "en": "with indirect heating means both inside and outside the retorts",
      "count": 31,
      "ipcCode": "C10B47/16",
      "coreCode": "C10B47/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/18": {
      "code": "C10B47/18",
      "level": 8,
      "parent": "C10B47/00",
      "ja": "移動装入によるもの",
      "en": "with moving charge",
      "count": 411,
      "ipcCode": "C10B47/18",
      "coreCode": "C10B47/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/20": {
      "code": "C10B47/20",
      "level": 9,
      "parent": "C10B47/18",
      "ja": "移動床形式にしたがうもの（Ｃ１０Ｂ４７／２６が優先）",
      "en": "according to the moving bed type (C10B47/26 takes precedence)",
      "count": 121,
      "ipcCode": "C10B47/20",
      "coreCode": "C10B47/20",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/22": {
      "code": "C10B47/22",
      "level": 9,
      "parent": "C10B47/18",
      "ja": "分散した形におけるもの（Ｃ１０Ｂ４７／２６が優先）",
      "en": "in dispersed form (C10B47/26 takes precedence)",
      "count": 111,
      "ipcCode": "C10B47/22",
      "coreCode": "C10B47/22",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/24": {
      "code": "C10B47/24",
      "level": 10,
      "parent": "C10B47/22",
      "ja": "流動床方式によるもの",
      "en": "according to the \"fluidised bed\" technique",
      "count": 116,
      "ipcCode": "C10B47/24",
      "coreCode": "C10B47/24",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/26": {
      "code": "C10B47/26",
      "level": 9,
      "parent": "C10B47/18",
      "ja": "高温の液体の助けによるもの，例．溶融塩",
      "en": "with the aid of hot liquids, e.g. molten salts",
      "count": 48,
      "ipcCode": "C10B47/26",
      "coreCode": "C10B47/26",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/28": {
      "code": "C10B47/28",
      "level": 8,
      "parent": "C10B47/00",
      "ja": "その他の方法",
      "en": "Other processes",
      "count": 50,
      "ipcCode": "C10B47/28",
      "coreCode": "C10B47/28",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/30": {
      "code": "C10B47/30",
      "level": 9,
      "parent": "C10B47/28",
      "ja": "回転炉またはレトルトにおけるもの",
      "en": "in rotary ovens or retorts",
      "count": 616,
      "ipcCode": "C10B47/30",
      "coreCode": "C10B47/30",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/32": {
      "code": "C10B47/32",
      "level": 9,
      "parent": "C10B47/28",
      "ja": "機械的運搬手段をもつ炉におけるもの",
      "en": "in ovens with mechanical conveying means",
      "count": 111,
      "ipcCode": "C10B47/32",
      "coreCode": "C10B47/32",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/34": {
      "code": "C10B47/34",
      "level": 10,
      "parent": "C10B47/32",
      "ja": "回転式かき取り装置をもつもの",
      "en": "with rotary scraping devices",
      "count": 78,
      "ipcCode": "C10B47/34",
      "coreCode": "C10B47/34",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/36": {
      "code": "C10B47/36",
      "level": 11,
      "parent": "C10B47/34",
      "ja": "多段式炉におけるもの",
      "en": "in multi-stage ovens",
      "count": 21,
      "ipcCode": "C10B47/36",
      "coreCode": "C10B47/36",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/38": {
      "code": "C10B47/38",
      "level": 10,
      "parent": "C10B47/32",
      "ja": "動揺または震動装置をもつもの",
      "en": "with shaking or vibrating devices",
      "count": 16,
      "ipcCode": "C10B47/38",
      "coreCode": "C10B47/38",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/40": {
      "code": "C10B47/40",
      "level": 10,
      "parent": "C10B47/32",
      "ja": "エンドレスコンベヤをもつもの",
      "en": "with endless conveying devices",
      "count": 76,
      "ipcCode": "C10B47/40",
      "coreCode": "C10B47/40",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/42": {
      "code": "C10B47/42",
      "level": 11,
      "parent": "C10B47/40",
      "ja": "垂直方向におけるもの",
      "en": "in vertical direction",
      "count": 9,
      "ipcCode": "C10B47/42",
      "coreCode": "C10B47/42",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/44": {
      "code": "C10B47/44",
      "level": 10,
      "parent": "C10B47/32",
      "ja": "コンベヤスクリューをもつもの",
      "en": "with conveyor-screws",
      "count": 826,
      "ipcCode": "C10B47/44",
      "coreCode": "C10B47/44",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/46": {
      "code": "C10B47/46",
      "level": 10,
      "parent": "C10B47/32",
      "ja": "トロッコ，コンテナ，台車をもつもの",
      "en": "with trucks, containers, or trays",
      "count": 99,
      "ipcCode": "C10B47/46",
      "coreCode": "C10B47/46",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B47/48": {
      "code": "C10B47/48",
      "level": 10,
      "parent": "C10B47/32",
      "ja": "傾注または揺動手段をもつもの",
      "en": "with tilting or rocking means",
      "count": 17,
      "ipcCode": "C10B47/48",
      "coreCode": "C10B47/48",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/00": {
      "code": "C10B49/00",
      "level": 7,
      "parent": null,
      "ja": "処理される固体物質の部分燃焼を含む熱運搬体で直接加熱する固体炭素質物の乾留",
      "en": "Destructive distillation of solid carbonaceous materials by direct heating with heat-carrying agents including the partial combustion of the solid material to be treated",
      "count": 481,
      "ipcCode": "C10B49/00",
      "coreCode": "C10B49/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/02": {
      "code": "C10B49/02",
      "level": 8,
      "parent": "C10B49/00",
      "ja": "熱ガスまたは蒸気によるもの，例．装入物の部分燃焼により得られる熱ガス",
      "en": "with hot gases or vapours, e.g. hot gases obtained by partial combustion of the charge",
      "count": 816,
      "ipcCode": "C10B49/02",
      "coreCode": "C10B49/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/04": {
      "code": "C10B49/04",
      "level": 9,
      "parent": "C10B49/02",
      "ja": "処理される固体物質の移動中におけるもの",
      "en": "while moving the solid material to be treated",
      "count": 483,
      "ipcCode": "C10B49/04",
      "coreCode": "C10B49/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/06": {
      "code": "C10B49/06",
      "level": 10,
      "parent": "C10B49/04",
      "ja": "移動床形式によるもの",
      "en": "according to the moving bed type",
      "count": 227,
      "ipcCode": "C10B49/06",
      "coreCode": "C10B49/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/08": {
      "code": "C10B49/08",
      "level": 10,
      "parent": "C10B49/04",
      "ja": "分散した形におけるもの",
      "en": "in dispersed form",
      "count": 283,
      "ipcCode": "C10B49/08",
      "coreCode": "C10B49/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/10": {
      "code": "C10B49/10",
      "level": 11,
      "parent": "C10B49/08",
      "ja": "流動床方式によるもの",
      "en": "according to the \"fluidised bed\" technique",
      "count": 360,
      "ipcCode": "C10B49/10",
      "coreCode": "C10B49/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/12": {
      "code": "C10B49/12",
      "level": 11,
      "parent": "C10B49/08",
      "ja": "接線方向の混合によるもの，例．渦巻室",
      "en": "by mixing tangentially, e.g. in vortex chambers",
      "count": 41,
      "ipcCode": "C10B49/12",
      "coreCode": "C10B49/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/14": {
      "code": "C10B49/14",
      "level": 8,
      "parent": "C10B49/00",
      "ja": "高温液体によるもの，例．溶融塩",
      "en": "with hot liquids, e.g. molten metals",
      "count": 226,
      "ipcCode": "C10B49/14",
      "coreCode": "C10B49/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/16": {
      "code": "C10B49/16",
      "level": 8,
      "parent": "C10B49/00",
      "ja": "分割した形における固体熱運搬体の移動によるもの",
      "en": "with moving solid heat-carriers in divided form",
      "count": 335,
      "ipcCode": "C10B49/16",
      "coreCode": "C10B49/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/18": {
      "code": "C10B49/18",
      "level": 9,
      "parent": "C10B49/16",
      "ja": "移動床形式によるもの",
      "en": "according to the \"moving bed\" type",
      "count": 72,
      "ipcCode": "C10B49/18",
      "coreCode": "C10B49/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/20": {
      "code": "C10B49/20",
      "level": 9,
      "parent": "C10B49/16",
      "ja": "分散した形におけるもの",
      "en": "in dispersed form",
      "count": 141,
      "ipcCode": "C10B49/20",
      "coreCode": "C10B49/20",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B49/22": {
      "code": "C10B49/22",
      "level": 10,
      "parent": "C10B49/20",
      "ja": "流動床方式によるもの",
      "en": "according to the \"fluidised bed\" technique",
      "count": 398,
      "ipcCode": "C10B49/22",
      "coreCode": "C10B49/22",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B51/00": {
      "code": "C10B51/00",
      "level": 7,
      "parent": null,
      "ja": "直接加熱と間接加熱の組み合わせによる固体炭素質物の乾留",
      "en": "Destructive distillation of solid carbonaceous materials by combined direct and indirect heating",
      "count": 291,
      "ipcCode": "C10B51/00",
      "coreCode": "C10B51/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B53/00": {
      "code": "C10B53/00",
      "level": 7,
      "parent": null,
      "ja": "特殊な固体原料または特殊な形態の固体原料に特に適用される乾留（でい炭の湿式炭化Ｃ１０Ｆ）",
      "en": "Destructive distillation, specially adapted for particular solid raw materials or solid raw materials in special form (wet carbonising of peat C10F)",
      "count": 4263,
      "ipcCode": "C10B53/00",
      "coreCode": "C10B53/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B53/02": {
      "code": "C10B53/02",
      "level": 8,
      "parent": "C10B53/00",
      "ja": "繊維素含有物質（木酢液質酸の製造Ｃ１０Ｃ５／００）",
      "en": "of cellulose-containing material (production of pyroligneous acid C10C5/00)",
      "count": 5993,
      "ipcCode": "C10B53/02",
      "coreCode": "C10B53/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B53/04": {
      "code": "C10B53/04",
      "level": 8,
      "parent": "C10B53/00",
      "ja": "微粉炭",
      "en": "of powdered coal",
      "count": 1128,
      "ipcCode": "C10B53/04",
      "coreCode": "C10B53/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B53/06": {
      "code": "C10B53/06",
      "level": 8,
      "parent": "C10B53/00",
      "ja": "油頁岩および／または瀝青岩",
      "en": "of oil shale and/or or bituminous rocks",
      "count": 447,
      "ipcCode": "C10B53/06",
      "coreCode": "C10B53/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B53/07": {
      "code": "C10B53/07",
      "level": 8,
      "parent": "C10B53/00",
      "ja": "｛ＩＰＣ８｝｛以下の固体原料｝合成高分子材料，例．タイヤ[Ｎ:（廃物一般，例．家庭廃棄物Ｃ１０Ｂ５３／００）]（一部解重合された材料を得るための乾熱処理による有機高分子化合物またはそれを基にした組成物の廃物材料の回収または処理Ｃ０８Ｊ１１／１０；ゴムまたはゴム廃物からの液体炭化水素混合物の製造Ｃ１０Ｇ１／１０）．[Ｎ:特記．グループＣ１０Ｂ５３／０７は未完成である。Ｃ１０Ｂ５３／００の他のサブグループに現在分類されている文献はＣ１０Ｂ５３／０７に再分類中である。]",
      "en": "{of solid raw materials consisting} of synthetic polymeric materials, e.g. tyres ({waste in general, e.g. household waste C10B53/00;} recovery or working-up of waste materials of organic macromolecular compounds or compositions based thereon by dry-heat treatment for obtaining partially depolymerised materials C08J11/10; production of liquid hydrocarbon mixtures from rubber or rubber waste C10G1/10)",
      "count": 2901,
      "ipcCode": "C10B53/07",
      "coreCode": "C10B53/07",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B53/08": {
      "code": "C10B53/08",
      "level": 8,
      "parent": "C10B53/00",
      "ja": "ブリケット，塊および類似物におけるもの",
      "en": "in the form of briquettes, lumps and the like",
      "count": 953,
      "ipcCode": "C10B53/08",
      "coreCode": "C10B53/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B55/00": {
      "code": "C10B55/00",
      "level": 7,
      "parent": null,
      "ja": "鉱油，ビチューメン，タールおよび類似物またはこれらの混合物と固体炭素質物を用いるコークス化（油の分解Ｃ１０Ｇ）",
      "en": "Coking mineral oils, bitumen, tar, and the like or mixtures thereof with solid carbonaceous material (cracking oils C10G)",
      "count": 1357,
      "ipcCode": "C10B55/00",
      "coreCode": "C10B55/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B55/02": {
      "code": "C10B55/02",
      "level": 8,
      "parent": "C10B55/00",
      "ja": "固体物質を用いるもの",
      "en": "with solid materials",
      "count": 262,
      "ipcCode": "C10B55/02",
      "coreCode": "C10B55/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B55/04": {
      "code": "C10B55/04",
      "level": 9,
      "parent": "C10B55/02",
      "ja": "移動する固体物質を用いるもの",
      "en": "with moving solid materials",
      "count": 59,
      "ipcCode": "C10B55/04",
      "coreCode": "C10B55/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B55/06": {
      "code": "C10B55/06",
      "level": 10,
      "parent": "C10B55/04",
      "ja": "移動床形式によるもの",
      "en": "according to the \"moving bed\" type",
      "count": 44,
      "ipcCode": "C10B55/06",
      "coreCode": "C10B55/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B55/08": {
      "code": "C10B55/08",
      "level": 10,
      "parent": "C10B55/04",
      "ja": "分散した形におけるもの",
      "en": "in dispersed form",
      "count": 54,
      "ipcCode": "C10B55/08",
      "coreCode": "C10B55/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B55/10": {
      "code": "C10B55/10",
      "level": 11,
      "parent": "C10B55/08",
      "ja": "流動床方法によるもの",
      "en": "according to the \"fluidised bed\" technique",
      "count": 392,
      "ipcCode": "C10B55/10",
      "coreCode": "C10B55/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/00": {
      "code": "C10B57/00",
      "level": 7,
      "parent": null,
      "ja": "上記に含まれないその他の方法；一般的な乾留方法の特徴",
      "en": "Other carbonising or coking processes; Features of destructive distillation processes in general",
      "count": 1656,
      "ipcCode": "C10B57/00",
      "coreCode": "C10B57/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/005": {
      "code": "C10B57/005",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "｛コークスの後処理，例．か焼，脱硫｝",
      "en": "{After-treatment of coke, e.g. calcination desulfurization}",
      "count": 462,
      "ipcCode": "C10B57/00",
      "coreCode": "C10B57/00",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/02": {
      "code": "C10B57/02",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "多段式炭化またはコークス化方法",
      "en": "Multi-step carbonising or coking processes",
      "count": 708,
      "ipcCode": "C10B57/02",
      "coreCode": "C10B57/02",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/04": {
      "code": "C10B57/04",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "特別な組成をもつ装入物の使用",
      "en": "using charges of special composition",
      "count": 1225,
      "ipcCode": "C10B57/04",
      "coreCode": "C10B57/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/045": {
      "code": "C10B57/045",
      "level": 9,
      "parent": "C10B57/04",
      "ja": "",
      "en": "{containing mineral oils, bitumen, tar or the like or mixtures thereof}",
      "count": 383,
      "ipcCode": "C10B57/04",
      "coreCode": "C10B57/04",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/06": {
      "code": "C10B57/06",
      "level": 9,
      "parent": "C10B57/04",
      "ja": "添加剤を含有するもの",
      "en": "containing additives",
      "count": 1214,
      "ipcCode": "C10B57/06",
      "coreCode": "C10B57/06",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/08": {
      "code": "C10B57/08",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "装入物の非機械的な前処理（Ｃ１０Ｌ９／００が優先）　｛例．脱硫｝",
      "en": "Non-mechanical pretreatment of the charge (C10L9/00 takes precedence), {e.g. desulfurization}",
      "count": 648,
      "ipcCode": "C10B57/08",
      "coreCode": "C10B57/08",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/10": {
      "code": "C10B57/10",
      "level": 9,
      "parent": "C10B57/08",
      "ja": "乾燥",
      "en": "Drying",
      "count": 1546,
      "ipcCode": "C10B57/10",
      "coreCode": "C10B57/10",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/12": {
      "code": "C10B57/12",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "コークス化中の添加剤の使用",
      "en": "Applying additives during coking",
      "count": 216,
      "ipcCode": "C10B57/12",
      "coreCode": "C10B57/12",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/14": {
      "code": "C10B57/14",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "低温炭化方法の特徴",
      "en": "Features of low-temperature carbonising processes",
      "count": 221,
      "ipcCode": "C10B57/14",
      "coreCode": "C10B57/14",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/16": {
      "code": "C10B57/16",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "高温炭化方法の特徴",
      "en": "Features of high-temperature carbonising processes",
      "count": 186,
      "ipcCode": "C10B57/16",
      "coreCode": "C10B57/16",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    },
    "C10B57/18": {
      "code": "C10B57/18",
      "level": 8,
      "parent": "C10B57/00",
      "ja": "コークス炉内での乾留ガスの性質の改質（コークス炉外側はＣ１０Ｋ）",
      "en": "Modifying the properties of the distillation gases in the oven (outside the oven C10K)",
      "count": 525,
      "ipcCode": "C10B57/18",
      "coreCode": "C10B57/18",
      "note": "",
      "sourceFile": "cpc_C10B.txt",
      "kind": "cpc"
    }
  }
};

window.CPC_SHARD_B25H = {
  "generatedAt": "2026-03-20T02:17:56.503Z",
  "total": 46,
  "entries": {
    "B25H": {
      "code": "B25H",
      "level": 5,
      "parent": "B25",
      "ja": "　作業場設備，例．工作物の印付け用；作業場用の貯蔵手段（貯蔵または包装Ｂ６５）｛特記１．このサブクラスは旧分類表（ＩｄＴ）８７Ａおよび４２Ｂの有効期限の満了に伴い１９７５年１月１日に導入された。２．特許文献は，有効期限が満了したＩｄＴ分類表からＩＰＣ分類表へ現在も継続的に再分類されている。３．ＩＰＣからＩｄＴグループへの一般的なコンコーダンスは以下のとおりである:Ｂ２５Ｈ１／００　－　Ｂ２５Ｈ１／２０　:　８７Ａ２２Ｇ，８７Ａ２２Ｇ２Ｂ｝",
      "en": "WORKSHOP EQUIPMENT, e.g. FOR MARKING-OUT WORK; STORAGE MEANS FOR WORKSHOPS (storing or packaging B65)<br><br><u>WARNING</u><br><br>In this subclass non-limiting references (in the sense of paragraph 39 of the Guide to the IPC) may still be displayed in the scheme.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B25": {
      "code": "B25",
      "level": 4,
      "parent": "B",
      "ja": "手工具；可搬型動力工具；手工具用の柄；作業場設備；マニプレータ<br>このクラスにおいては，下記の用語は以下に示す意味で用いる:<br>―可搬型は人手で取扱い易いようにするための，例．組立てラインにそって使用されるばねで吊り下げられた可搬型の装置に関連した，吊り下げ具を含む。",
      "en": "HAND TOOLS; PORTABLE POWER-DRIVEN TOOLS; MANIPULATORS<br><br><u>NOTE</u><br><br>The work \"portable\" is to be understood as including suspension for easy manual handling, e.g. in connection with springsuspended portable apparatus for use along assembly lines",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B": {
      "code": "B",
      "level": 2,
      "parent": null,
      "ja": "処理操作；運輸",
      "en": "PERFORMING OPERATIONS; TRANSPORTING<br><br><u>NOTE</u><br><br>The following notes are meant to assist in the use of classes B01&#160;-&#160;B09; they must not be read as modifying in any way the elaborations.<br>In this sub-section, the separation of different materials, e.g. of different matter, size, or state, is predominantly found in the following subclasses: <br>B01D<br>B03B, B03C, B03D<br>B04B, B04C<br>B07B, B07C<br>The classifying characteristics of these subclasses are: <br>the physical state of the matter to be separated <br>the principle of the process used <br>particular kinds of apparatus The first of these characteristics involves six different aspects, assembled in three groups.<br>liquid/liquid or liquid/gas and gas/gas <br>solid/liquid or solid/gas <br>solid/solid<br>Theses subclasses are to be used according to the following general rules: <br>B01D is the most general class as far as separation other  than solids from solids is concerned. <br>Apparatus for separating solids from solids are covered by B03B when the process concerned is regarded as the equivalent of \"washing\" in the sense of the mining art, even if such apparatus is a pneumatic one, especially pneumatic tables or jigs. Screens PER SE are not covered by this subclass but are classified in B07B, even if they are being used in a wet process. All other apparatus for the separation of solids from solids according to dry methods are classified in B07B. <br>If the separation takes place as a result of the detection or measurement of some feature of the material or articles to be sorted it is classified in B07C. <br>It should also be noted that the separation of isotopes of the same chemical element is covered by B01D59/00, whatever process or apparatus is employed.<br>The following scheme illustrates the classification according to these rules. <br>LIQUID/LIQUID <br>      LIQUID/GAS <br>      GAS/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force, using centrifuges <br>    or free-vortex apparatus                   B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br>SOLID/LIQUID <br>      SOLID/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force                       B01DB01D<br>       using centrifuges or free-vortex <br>       apparatus                               B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br><br>SOLID/SOLID <br>  _____________________________________________________________________ <br>  Dry Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General for material in bulk                 B07BB07B<br>  Individual sorting                           B07CB07C<br>     Screening, sifting, pneumatic sorting     B07BB07B<br>       using pneumatic tables or jigs          B03BB03B<br>     by magnetic or electrostatic effect       B03CB03C<br>     by centrifugal force                      B07BB07B<br>       using centrifuges or free-vortex <br>       apparatus                               B07BB04B, B04C<br>  _____________________________________________________________________ <br>  Wet Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B03BB03B<br>     flotation, differential sedimentation     B03DB03D<br>     screening                                 B07BB07B<br>  _____________________________________________________________________ <br>  Combinations = dry methods - wet methods: B03B<br>  _____________________________________________________________________<br>",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "B25H1/00": {
      "code": "B25H1/00",
      "level": 7,
      "parent": null,
      "ja": "仕事台；可搬式工具またはそれによって加工する工作物を置く為の可搬式スタンドまたは支持台｛注．このグループにおいては，＋ＩＤＴという表現は以下の内容を意味する。　－　ここで分類される文献は作業台，木挽台，仕事台に関するＩＤＴグループに分類されていた。　－　これらの文献は該ＩＤＴグル－プからこのグループへ管理的に再分類された｝",
      "en": "Work benches; Portable stands or supports for positioning portable tools or work to be operated on thereby",
      "count": 2542,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0007": {
      "code": "B25H1/0007",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "｛機関，自動車または自転車用のもの｝",
      "en": "{for engines, motor-vehicles or bicycles}",
      "count": 1172,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0014": {
      "code": "B25H1/0014",
      "level": 9,
      "parent": "B25H1/0007",
      "ja": "｛自動二輪車または自転車用のもの｝",
      "en": "{for motorcycles or bicycles}",
      "count": 397,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0021": {
      "code": "B25H1/0021",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "｛可搬式工具の位置決めのため，または工具を工作物に固定するためのスタンド，支持台または案内装置（Ｂ２３Ｂ４１／０８(ＥＣＬＡ)が優先）｝",
      "en": "{Stands, supports or guiding devices for positioning portable tools or for securing them to the work (B23B41/00 takes precedence)}",
      "count": 981,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0028": {
      "code": "B25H1/0028",
      "level": 9,
      "parent": "B25H1/0021",
      "ja": "｛工具平衡器｝",
      "en": "{Tool balancers}",
      "count": 326,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0035": {
      "code": "B25H1/0035",
      "level": 9,
      "parent": "B25H1/0021",
      "ja": "｛伸縮式支持台，例．望遠鏡型のもの｝",
      "en": "{Extensible supports, e.g. telescopic}",
      "count": 442,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0042": {
      "code": "B25H1/0042",
      "level": 9,
      "parent": "B25H1/0021",
      "ja": "｛スタンド（Ｂ２５Ｈ１／００７１が優先）｝",
      "en": "{Stands (B25H1/0071 takes precedence)}",
      "count": 432,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/005": {
      "code": "B25H1/005",
      "level": 10,
      "parent": "B25H1/0042",
      "ja": "｛仕事台に取り付けるもの｝",
      "en": "{attached to a workbench}",
      "count": 273,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0057": {
      "code": "B25H1/0057",
      "level": 9,
      "parent": "B25H1/0021",
      "ja": "｛手工具を工作物に固定するための装置｝",
      "en": "{Devices for securing hand tools to the work}",
      "count": 333,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0064": {
      "code": "B25H1/0064",
      "level": 10,
      "parent": "B25H1/0057",
      "ja": "｛加工物に取り付けられるスタンド｝",
      "en": "{Stands attached to the workpiece}",
      "count": 465,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0071": {
      "code": "B25H1/0071",
      "level": 11,
      "parent": "B25H1/0064",
      "ja": "｛磁気手段によるもの｝",
      "en": "{by magnetic means}",
      "count": 202,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0078": {
      "code": "B25H1/0078",
      "level": 9,
      "parent": "B25H1/0021",
      "ja": "｛手工具用案内装置｝",
      "en": "{Guiding devices for hand tools}",
      "count": 622,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0085": {
      "code": "B25H1/0085",
      "level": 10,
      "parent": "B25H1/0078",
      "ja": "｛水平器を用いることによるもの｝",
      "en": "{by means of levels}",
      "count": 162,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/0092": {
      "code": "B25H1/0092",
      "level": 10,
      "parent": "B25H1/0078",
      "ja": "｛光学手段によるもの｝",
      "en": "{by optical means}",
      "count": 185,
      "ipcCode": "B25H1/00",
      "coreCode": "B25H1/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/02": {
      "code": "B25H1/02",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "テーブル型のもの",
      "en": "of table type",
      "count": 2393,
      "ipcCode": "B25H1/02",
      "coreCode": "B25H1/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/04": {
      "code": "B25H1/04",
      "level": 9,
      "parent": "B25H1/02",
      "ja": "可搬式のもの",
      "en": "portable",
      "count": 1615,
      "ipcCode": "B25H1/04",
      "coreCode": "B25H1/04",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/06": {
      "code": "B25H1/06",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "きゃたつ型のもの",
      "en": "of trestle type",
      "count": 2181,
      "ipcCode": "B25H1/06",
      "coreCode": "B25H1/06",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/08": {
      "code": "B25H1/08",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "工作物保持具の取付のための設備をもつもの",
      "en": "with provision for attachment of work holders",
      "count": 2823,
      "ipcCode": "B25H1/08",
      "coreCode": "B25H1/08",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/10": {
      "code": "B25H1/10",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "工具または工作物用の保持具の調整のための設備をもつもの",
      "en": "with provision for adjusting holders for tool or work",
      "count": 4351,
      "ipcCode": "B25H1/10",
      "coreCode": "B25H1/10",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/12": {
      "code": "B25H1/12",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "貯蔵区分をもつもの",
      "en": "with storage compartments",
      "count": 1683,
      "ipcCode": "B25H1/12",
      "coreCode": "B25H1/12",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/14": {
      "code": "B25H1/14",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "台の上面を調整する設備をもつもの",
      "en": "with provision for adjusting the bench top",
      "count": 1943,
      "ipcCode": "B25H1/14",
      "coreCode": "B25H1/14",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/16": {
      "code": "B25H1/16",
      "level": 9,
      "parent": "B25H1/14",
      "ja": "高さ",
      "en": "in height",
      "count": 2866,
      "ipcCode": "B25H1/16",
      "coreCode": "B25H1/16",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/18": {
      "code": "B25H1/18",
      "level": 9,
      "parent": "B25H1/14",
      "ja": "傾斜",
      "en": "in inclination",
      "count": 912,
      "ipcCode": "B25H1/18",
      "coreCode": "B25H1/18",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H1/20": {
      "code": "B25H1/20",
      "level": 8,
      "parent": "B25H1/00",
      "ja": "作業域をシールドする設備をもつもの",
      "en": "with provision for shielding the work area",
      "count": 471,
      "ipcCode": "B25H1/20",
      "coreCode": "B25H1/20",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/00": {
      "code": "B25H3/00",
      "level": 7,
      "parent": null,
      "ja": "工作物，工具または器具に近づき易くするかまたはそれらの取扱いを容易にするための作業用の貯蔵手段または装置（内容物を分配するための特別の手段をもつ容器または包装Ｂ６５Ｄ８３／００；　｛釘分配具　Ｂ２５Ｃ３／００｝）",
      "en": "Storage means or arrangements for workshops facilitating access to, or handling of, work tools or instruments (containers or packages with special means for dispensing contents B65D83/00; {nail dispensers B25C3/00})",
      "count": 3114,
      "ipcCode": "B25H3/00",
      "coreCode": "B25H3/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/003": {
      "code": "B25H3/003",
      "level": 8,
      "parent": "B25H3/00",
      "ja": "｛ドリルビット用ホルダまたは類似のもの｝",
      "en": "{Holders for drill bits or the like}",
      "count": 1645,
      "ipcCode": "B25H3/00",
      "coreCode": "B25H3/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/006": {
      "code": "B25H3/006",
      "level": 8,
      "parent": "B25H3/00",
      "ja": "｛特殊な一つの手動装置，例．電気ドリル，に特に適した格納手段｝",
      "en": "{Storage means specially adapted for one specific hand apparatus, e.g. an electric drill}",
      "count": 980,
      "ipcCode": "B25H3/00",
      "coreCode": "B25H3/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/02": {
      "code": "B25H3/02",
      "level": 8,
      "parent": "B25H3/00",
      "ja": "箱",
      "en": "Boxes",
      "count": 3839,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/021": {
      "code": "B25H3/021",
      "level": 9,
      "parent": "B25H3/02",
      "ja": "｛多数の連結した格納要素からなるもの｝",
      "en": "{comprising a number of connected storage elements}",
      "count": 538,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/022": {
      "code": "B25H3/022",
      "level": 10,
      "parent": "B25H3/021",
      "ja": "｛つながりが固定しているもの｝",
      "en": "{in fixed relationship}",
      "count": 657,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/023": {
      "code": "B25H3/023",
      "level": 10,
      "parent": "B25H3/021",
      "ja": "｛格納要素内部にアクセスするため，互いの位置を移動できるもの｝",
      "en": "{movable relative to one another for access to their interiors}",
      "count": 677,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/025": {
      "code": "B25H3/025",
      "level": 11,
      "parent": "B25H3/023",
      "ja": "｛共通の軸の周りを回転することによるもの｝",
      "en": "{by rotation about a common axis}",
      "count": 260,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/026": {
      "code": "B25H3/026",
      "level": 11,
      "parent": "B25H3/023",
      "ja": "｛底部に対して実質上垂直な方向に変動することによるもの｝",
      "en": "{by displacement in a direction substantially perpendicated to th base}",
      "count": 156,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/027": {
      "code": "B25H3/027",
      "level": 11,
      "parent": "B25H3/023",
      "ja": "｛互いに沿ってスライドすることによるもの｝",
      "en": "{by sliding along one another}",
      "count": 93,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/028": {
      "code": "B25H3/028",
      "level": 11,
      "parent": "B25H3/023",
      "ja": "｛スライドしながら共通フレームの中から抜き出すことによるもの｝",
      "en": "{by sliding extraction from within a common frame}",
      "count": 772,
      "ipcCode": "B25H3/02",
      "coreCode": "B25H3/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/04": {
      "code": "B25H3/04",
      "level": 8,
      "parent": "B25H3/00",
      "ja": "たな",
      "en": "Racks",
      "count": 4338,
      "ipcCode": "B25H3/04",
      "coreCode": "B25H3/04",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H3/06": {
      "code": "B25H3/06",
      "level": 8,
      "parent": "B25H3/00",
      "ja": "皿",
      "en": "Trays",
      "count": 1040,
      "ipcCode": "B25H3/06",
      "coreCode": "B25H3/06",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H5/00": {
      "code": "B25H5/00",
      "level": 7,
      "parent": null,
      "ja": "車両と組み合わされて使用される工具，器具，工作物保持手段または貯蔵手段（車輪またはその部品の保持手段Ｂ６０Ｂ３０／００）；作業者のささえ，例．職人のすべり止め　｛（作業場内の排気ガス収集または除去Ｂ０８Ｂ１５／００２；　車両昇降機，　例．ジャッキ，プラットフォームまたはランプＢ６６Ｆ）｝　",
      "en": "Tool, instrument or work supports or storage means used in association with vehicles; Workers&apos; supports, e.g. mechanics&apos; creepers {(collecting or removing exhaust gases in workshops B08B15/002; vehicle lifting devices, e.g. jacks, platforms or ramps B66F)}",
      "count": 3121,
      "ipcCode": "B25H5/00",
      "coreCode": "B25H5/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H7/00": {
      "code": "B25H7/00",
      "level": 7,
      "parent": null,
      "ja": "工作物の印付けまたは芯出し（測定，ゲージングＧ０１；光学器械Ｇ０２Ｂ；写真手段によるものＧ０３Ｃ）",
      "en": "Marking-out or setting-out work (measuring, gauging G01; optical apparatus G02B; by photographic means G03C)",
      "count": 1028,
      "ipcCode": "B25H7/00",
      "coreCode": "B25H7/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H7/005": {
      "code": "B25H7/005",
      "level": 8,
      "parent": "B25H7/00",
      "ja": "｛パイプ用｝",
      "en": "{for pipes}",
      "count": 328,
      "ipcCode": "B25H7/00",
      "coreCode": "B25H7/00",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H7/02": {
      "code": "B25H7/02",
      "level": 8,
      "parent": "B25H7/00",
      "ja": "平らな表面をもつ板",
      "en": "Plates having a flat surface",
      "count": 613,
      "ipcCode": "B25H7/02",
      "coreCode": "B25H7/02",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H7/04": {
      "code": "B25H7/04",
      "level": 8,
      "parent": "B25H7/00",
      "ja": "印付け用装置，例．けがき針（センターポンチＢ２５Ｄ５／００；　｛紐線引きＢ４４Ｄ３／３８｝）",
      "en": "Devices, e.g. scribers, for marking (centre punches B25D5/00{; cord line chalkers B44D3/38})",
      "count": 3451,
      "ipcCode": "B25H7/04",
      "coreCode": "B25H7/04",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    },
    "B25H7/045": {
      "code": "B25H7/045",
      "level": 9,
      "parent": "B25H7/04",
      "ja": "｛印付け要素の細部構造に特徴があるもの｝",
      "en": "{characterised by constructional details of the marking elements}",
      "count": 1117,
      "ipcCode": "B25H7/04",
      "coreCode": "B25H7/04",
      "note": "",
      "sourceFile": "cpc_B25H.txt",
      "kind": "cpc"
    }
  }
};

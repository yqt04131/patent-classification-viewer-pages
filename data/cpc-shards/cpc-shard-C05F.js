window.CPC_SHARD_C05F = {
  "generatedAt": "2026-03-20T02:17:57.237Z",
  "total": 58,
  "entries": {
    "C05F": {
      "code": "C05F",
      "level": 5,
      "parent": "C05",
      "ja": "　サブクラスＣ０５Ｂ，Ｃ０５Ｃに包含されない有機質肥料，例．廃棄物またはじんかいからの肥料｛（ミミズの繁殖Ａ０１Ｋ６７／０３３２）｝｛特記．　ＩＰＣグループＣ０５Ｆ１５／００(ＥＣＬＡ)は，内部ＥＣＬＡ分類表には使用されない。このグループに包含される主題事項は，ＥＣＬＡグループＣ０５Ｆ１／００からＣ０５Ｆ１１／００のひとつに分類される。（クラスＣ０５のタイトルの後の内部注．を参照）｝注．　たい肥化の方法に特徴がある工程またはそのための装置は，グループＣ０５Ｆ１７／００に分類される。",
      "en": "ORGANIC FERTILISERS NOT COVERED BY SUBCLASSES C05B, C05C, e.g. FERTILISERS FROM WASTE OR REFUSE<br><br><u>NOTE</u><br><br>Processes where the composting step is the characterising feature, or apparatus therefor, are classified in group C05F17/00.<br><br><u>WARNING</u><br>The following IPC groups are not in the CPC scheme. The subject matter for these IPC groups is classified in the following CPC groups: C05F15/00 covered by C05F1/00-C05F11/00(see also internal note after the title of class C05)",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C05": {
      "code": "C05",
      "level": 4,
      "parent": "C",
      "ja": "肥料；肥料の製造<br>（１）サブクラスへの細分化の基礎となる化学元素を二種以上含有する肥料混合物中の成分または単一肥料は該当サブクラスの最初にのみ分類される。このようにしてニトロフォスフェートまたはアンモニア化過りん酸石灰はＣ０５Ｂに分類されるが，Ｃ０５Ｃには分類されず，リン酸マグネシウムはＣ０５Ｂに分類されるが，Ｃ０５Ｄには分類されず，また石灰窒素はＣ０５Ｃには分類されるがＣ０５Ｄには分類されない。<br>（２）検索に有用な情報を表していると思われる混合物の成分は，注（１）に従って付加的に分類してもよい。例えばこれは，分類記号の組合せを用いた混合物の検索が可能となることが重要であると考えられる場合にあり得る。こうした非義務的分類は，「付加情報」として付与される。",
      "en": "FERTILISERS; MANUFACTURE THEREOF<br><br><u>NOTE</u><br><br>An ingredient in a mixture of fertilisers, or a single fertiliser which contains more than one of the chemical elements on which the subdivision into subclasses is based, is classified only in the first of the appropriate subclasses. Thus, a nitrophosphate or an ammoniated superphosphate is classified in C05B but not in C05C, magnesium phosphate is classified in C05B but not in C05D, and calcium cyanamide in C05C but not in C05D.<br><br>In this class, mixtures of fertilizers are classified in the first appropriate place. After the notation of the appropriate classification symbol and separated therefrom by a + sign, notations concerning the ingredients of the mixture, not covered by the chosen classification symbol, may be added. These notations are selected from class C05 and are presented in the following way, e.g. C05B1/02 + C05D1/02 + C05D9/02",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C": {
      "code": "C",
      "level": 2,
      "parent": null,
      "ja": "化学；冶金<br>（１）セクションＣにおいて，化学元素の種類に関する定義は次の通りである:<br>アルカリ金属:Ｌｉ，Ｎａ，Ｋ，Ｒｂ，Ｃｓ，Ｆｒ<br>アルカリ土類金属:Ｃａ，Ｓｒ，Ｂａ，Ｒａ<br>ランタニド:原子番号５７から７１までの元素<br>希土類:Ｓｃ，Ｙ，ランタニド<br>アクチニド:原子番号８９から１０３までの元素<br>耐火金属:Ｔｉ，Ｖ，Ｃｒ，Ｚｒ，Ｎｂ，Ｍｏ，Ｈｆ，Ｔａ，Ｗ<br>ハロゲン:Ｆ，Ｃｌ，Ｂｒ，Ｉ，Ａｔ<br>希ガス:Ｈｅ，Ｎｅ，Ａｒ，Ｋｒ，Ｘｅ，Ｒｎ<br>白金族:Ｏｓ，Ｉｒ，Ｐｔ，Ｒｕ，Ｒｈ，Ｐｄ<br>貴金属:Ａｇ，Ａｕ，白金族<br>軽金属:アルカリ金属，アルカリ土類金属，Ｂｅ，Ａｌ，Ｍｇ<br>重金属:軽金属以外の金属<br>鉄族:Ｆｅ，Ｃｏ，Ｎｉ<br>非金属:Ｈ，Ｂ，Ｃ，Ｓｉ，Ｎ，Ｐ，Ｏ，Ｓ，Ｓｅ，Ｔｅ，希ガス，ハロゲン<br>金属:非金属以外の元素<br>遷移元素:原子番号２１から３０まで，３９から４８まで，５７から８０まで，８９以上の元素<br>（２）Ｃセクションは以下のものを包含する:<br>（ａ）無機化合物，有機化合物，高分子化合物およびそれらの調製法を包含する純粋化学；<br>（ｂ）たとえばガラス，セラミック，肥料，プラスチック組成物，ペイント，石油工業の製品のような上記化合物を含有する組成物を包含する応用化学。それはまた，爆発物，染料，接着剤，潤滑剤および洗剤のように，ある目的に適した特別の性質をもつための組成物をも包含する；<br>（ｃ）コークスの製造，固体または気体燃料の製造，油脂およびろうの製造および精製，発酵工業（例．ビールおよびぶどう酒の製造），製糖工業のようなある境界工業；<br>（ｄ）ある操作または処理であって，純粋に機械的な処理，例．レザーおよび皮の機械的処理，または部分的機械的処理，例．水処理，または腐食防止一般であるもの；<br>（ｅ）冶金，鉄または非鉄合金<br>（３）ＩＰＣの全てのセクションにおいて，相反する指示がない限り、元素記号周期律表への言及は、以下に示す８つのグループを表す。例えば、グループＣ０７Ｆ３／００「周期律表の第２族の元素を含有する化合物」は、ＩＩＡ及びＩＩＢ族の元素への言及とする。<br>「図」<br>（４）（ａ）化学的部分または面および非化学的部分または面の両方を有する操作，処理，製品または物品の場合に，一般規則として，化学的部分または面はＣセクションに包含される。<br>（ｂ）これらのうちのある場合には，化学的部分または面には非化学的部分または面を伴い，純粋に機械的部分または面を伴うことさえある，というのは非化学的面は操作または処理にとって本質的であるかまたはそれらの重要な要素を構成するからである；実際，全体のさまざまな部分または面を分割しないのがより論理的と思える。これは応用化学や上記注（１）（ｃ），（ｄ）および（ｅ）の工業，操作および処理の場合に適用される。例えば，ガラス製造用特殊炉はＣ０３に包含されＦ２７には包含されない。<br>（ｃ）しかしながら，機械的面（または非化学的面）に非化学的な面が伴うという例外もある，例えば:<br>―サブクラスＡ６１Ｋにおけるある抽出プロセス；<br>―サブクラスＡ６１Ｌにおける空気の化学的精製；<br>―サブクラスＡ６２Ｄにおける化学的消火方法；<br>―クラスＢ０１における化学的プロセスおよび装置；<br>―サブクラスＢ２７Ｋにおける木材の含浸；<br>―サブクラスＧ０１Ｎにおける化学的分析または試験法；<br>―クラスＧ０３における写真材料またはプロセス，そして一般に，Ｄセクションにおける繊維の化学的処理および繊維素，紙等の製造<br>（ｄ）さらに他の場合，純粋化学的面はＣセクションに包含され，応用化学的面はＡ，Ｂ，Ｆセクションのような他のセクションに包含される，例．以下の物質または組成物の使用:<br>―サブクラスＡ０１Ｎに包含される植物または動物の処理；<br>―クラスＡ２３に包含される食物；<br>―クラスＦ４２に包含される弾薬または爆薬<br>（ｅ）化学的面および機械的面が非常に交錯して，簡単明確な分割ができない時，または化学的処理が自然なまたは論理的な延長として機械的プロセスを要する時は，化学的面に加えて，Ｃセクションは機械的面の一部のみ，例．クラスＣ０４に包含される人造石の後処理，を包含する。この後者の場合に，たとえその区別がむしろ任意であってもその位置を明確にするよう注または参照を通常設けている。",
      "en": "CHEMISTRY; METALLURGY<br><br><u>NOTE</u><br><br>In section C, the definitions of groups of chemical elements are as follows: <br>Alkali metals: Li, Na, K, Rb, Cs, Fr <br>Alkaline earth metals: Ca, Sr, Ba, Ra<br>Lanthanides: elements with atomic numbers 57 to 71 inclusive <br>Rare earths: Sc, Y, Lanthanides<br>Actinides: elements with atomic numbers 89 to 103 inclusive<br>Refractory metals: Ti, V, Cr, Zr, Nb, Mo, Hf, Ta, W <br>Halogens: F, Cl, Br, I, At <br>Noble gases: He, Ne, Ar, Kr, Xe, Rn <br>Platinum group: Os, Ir, Pt, Ru, Rh, Pd <br>Noble metals: Ag, Au, Platinum group <br>Light metals: alkali metals, alkaline earth metals, Be, Al, Mg<br>Heavy metals: metals other than light metals<br>Iron group: Fe, Co, Ni<br>Non-metals: H, B, C, Si, N, P, O, S, Se, Te, noble gases, halogens<br>Metals: elements other than non-metals <br>Transition elements: elements with atomic numbers 21 to 30 inclusive, 39 to 48 inclusive, 57 to 80 inclusive, 89 upwards<br>Section Ccovers:<br>pure chemistry, which covers inorganic compounds, organic compounds, macromolecular compounds, and their methods of preparation;<br>applied chemistry, which covers compositions containing the above compounds, such as: glass, ceramics, fertilisers, plastics compositions, paints, products of the petroleum industry. It also covers certain compositions on account of their having particular properties rendering them suitable for certain purposes, as in the case of explosives, dyestuffs, adhesives, lubricants and detergents;<br>certain marginal industries, such as the manufacture of coke and of solid or gaseous fuels, the production and refining of oils, fats and waxes, the fermentation industry (e.g., brewing and wine-making), the sugar industry;<br>certain operations or treatments, which are either purely mechanical, e.g., the mechanical treatment of leather and skins, or partly mechanical, e.g., the treatment of water or the prevention of corrosion in general;<br>metallurgy, ferrous or non-ferrous alloys.<br>In all sections of the CPC, in the absence of an indication to the contrary, the Periodic Table of chemical elements referred to is the one with 18 groups as represented in the table below.<br><br><br>In the case of operations, treatments, products or articles having both a chemical and a non-chemical part or aspect, the general rule is that the chemical part or aspect is covered by section C.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Notes (2) c), d) and e). For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example:<br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g. the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Note (2) bullets 2, 3, 4 and 5. For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example: <br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g., the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "C05F1/00": {
      "code": "C05F1/00",
      "level": 7,
      "parent": null,
      "ja": "動物の死体またはその一部から造られた肥料",
      "en": "Fertilisers made from animal corpses, or parts thereof",
      "count": 1606,
      "ipcCode": "C05F1/00",
      "coreCode": "C05F1/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F1/002": {
      "code": "C05F1/002",
      "level": 8,
      "parent": "C05F1/00",
      "ja": "｛魚または魚の廃棄物からのもの｝",
      "en": "{from fish or from fish-wastes}",
      "count": 1877,
      "ipcCode": "C05F1/00",
      "coreCode": "C05F1/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F1/005": {
      "code": "C05F1/005",
      "level": 8,
      "parent": "C05F1/00",
      "ja": "｛肉の廃棄物または動物由来の他の廃棄物，例．皮，毛髪，ひづめ，羽毛，血，からのもの｝",
      "en": "{from meat-wastes or from other wastes of animal origin, e.g. skins, hair, hoofs, feathers, blood}",
      "count": 4617,
      "ipcCode": "C05F1/00",
      "coreCode": "C05F1/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F1/007": {
      "code": "C05F1/007",
      "level": 8,
      "parent": "C05F1/00",
      "ja": "｛動物由来またはその廃棄物の誘導物質，例．皮革，乳製品，からのもの｝",
      "en": "{from derived products of animal origin or their wastes, e.g. leather, dairy products}",
      "count": 1400,
      "ipcCode": "C05F1/00",
      "coreCode": "C05F1/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F1/02": {
      "code": "C05F1/02",
      "level": 8,
      "parent": "C05F1/00",
      "ja": "製造装置",
      "en": "Apparatus for the manufacture",
      "count": 366,
      "ipcCode": "C05F1/02",
      "coreCode": "C05F1/02",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F3/00": {
      "code": "C05F3/00",
      "level": 7,
      "parent": null,
      "ja": "人間または動物の排せつ物，例．厩肥，からの肥料",
      "en": "Fertilisers from human or animal excrements, e.g. manure",
      "count": 24362,
      "ipcCode": "C05F3/00",
      "coreCode": "C05F3/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F3/02": {
      "code": "C05F3/02",
      "level": 8,
      "parent": "C05F3/00",
      "ja": "グアノ",
      "en": "Guano",
      "count": 359,
      "ipcCode": "C05F3/02",
      "coreCode": "C05F3/02",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F3/04": {
      "code": "C05F3/04",
      "level": 8,
      "parent": "C05F3/00",
      "ja": "人ぷんから造られたもの",
      "en": "from human faecal masses",
      "count": 1342,
      "ipcCode": "C05F3/04",
      "coreCode": "C05F3/04",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F3/06": {
      "code": "C05F3/06",
      "level": 8,
      "parent": "C05F3/00",
      "ja": "製造装置",
      "en": "Apparatus for the manufacture",
      "count": 2991,
      "ipcCode": "C05F3/06",
      "coreCode": "C05F3/06",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F5/00": {
      "code": "C05F5/00",
      "level": 7,
      "parent": null,
      "ja": "蒸留残留物，糖蜜，酒類の蒸留かす，製糖工場の廃物または同様な廃物または残留物から造られた肥料，　｛例．農業起源の原材料またはその誘導物質の工業プロセスから生じる廃棄物からのもの｝",
      "en": "Fertilisers from distillery wastes, molasses, vinasses, sugar plant or similar wastes or residues, {e.g. from waste originating from industrial processing of raw material of agricultural origin or derived products thereof}",
      "count": 5283,
      "ipcCode": "C05F5/00",
      "coreCode": "C05F5/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F5/002": {
      "code": "C05F5/002",
      "level": 8,
      "parent": "C05F5/00",
      "ja": "｛材料の機械的処理による固体廃棄物，例．種皮，オリーブの種，アーモンドの殻，果物の残滓，籾殻｝",
      "en": "{Solid waste from mechanical processing of material, e.g. seed coats, olive pits, almond shells, fruit residue, rice hulls}",
      "count": 19759,
      "ipcCode": "C05F5/00",
      "coreCode": "C05F5/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F5/004": {
      "code": "C05F5/004",
      "level": 8,
      "parent": "C05F5/00",
      "ja": "｛材料の機械的処理による液体廃棄物，例．洗浄水，製粉流体，ろ液｝",
      "en": "{Liquid waste from mechanical processing of material, e.g. wash-water, milling fluid, filtrate}",
      "count": 342,
      "ipcCode": "C05F5/00",
      "coreCode": "C05F5/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F5/006": {
      "code": "C05F5/006",
      "level": 8,
      "parent": "C05F5/00",
      "ja": "｛材料の化学的処理による廃棄物，例．蒸留，焙焼，調理｝",
      "en": "{Waste from chemical processing of material, e.g. diestillation, roasting, cooking}",
      "count": 5268,
      "ipcCode": "C05F5/00",
      "coreCode": "C05F5/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F5/008": {
      "code": "C05F5/008",
      "level": 9,
      "parent": "C05F5/006",
      "ja": "｛材料の生化学的な処理からの廃棄物，例．発酵，醸造｝",
      "en": "{Waste from biochemical processing of material, e.g. fermentation, breweries}",
      "count": 5024,
      "ipcCode": "C05F5/00",
      "coreCode": "C05F5/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F7/00": {
      "code": "C05F7/00",
      "level": 7,
      "parent": null,
      "ja": "廃水，下水スラッジ，海のへどろ，軟泥または類似の塊から造られた肥料（スラッジの脱水，乾燥または灰化の方法または装置Ｃ０２Ｆ１１／００）",
      "en": "Fertilisers from waste water, sewage sludge, sea slime, ooze or similar masses",
      "count": 7139,
      "ipcCode": "C05F7/00",
      "coreCode": "C05F7/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F7/005": {
      "code": "C05F7/005",
      "level": 8,
      "parent": "C05F7/00",
      "ja": "｛農業由来でも動物由来でもない材料の工業的処理からの廃水｝",
      "en": "{Waste water from industrial processing material neither of agricultural nor of animal origin}",
      "count": 218,
      "ipcCode": "C05F7/00",
      "coreCode": "C05F7/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F7/02": {
      "code": "C05F7/02",
      "level": 8,
      "parent": "C05F7/00",
      "ja": "亜硫酸パルプ廃液またはセルロース製造からのその他の廃液から造られたもの",
      "en": "from sulfite liquor or other waste lyes from the manufacture of cellulose",
      "count": 215,
      "ipcCode": "C05F7/02",
      "coreCode": "C05F7/02",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F7/04": {
      "code": "C05F7/04",
      "level": 8,
      "parent": "C05F7/00",
      "ja": "カリウム工業の廃液から造られたもの",
      "en": "from waste liquors in the potash industry",
      "count": 4,
      "ipcCode": "C05F7/04",
      "coreCode": "C05F7/04",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F9/00": {
      "code": "C05F9/00",
      "level": 7,
      "parent": null,
      "ja": "家庭または都市のじんかいから造られた肥料",
      "en": "Fertilisers from household or town refuse",
      "count": 3065,
      "ipcCode": "C05F9/00",
      "coreCode": "C05F9/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F9/02": {
      "code": "C05F9/02",
      "level": 8,
      "parent": "C05F9/00",
      "ja": "製造装置",
      "en": "Apparatus for the manufacture",
      "count": 2465,
      "ipcCode": "C05F9/02",
      "coreCode": "C05F9/02",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F9/04": {
      "code": "C05F9/04",
      "level": 8,
      "parent": "C05F9/00",
      "ja": "生物的たい肥",
      "en": "Biological compost",
      "count": 6824,
      "ipcCode": "C05F9/04",
      "coreCode": "C05F9/04",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F11/00": {
      "code": "C05F11/00",
      "level": 7,
      "parent": null,
      "ja": "他の有機質肥料",
      "en": "Other organic fertilisers",
      "count": 40387,
      "ipcCode": "C05F11/00",
      "coreCode": "C05F11/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F11/02": {
      "code": "C05F11/02",
      "level": 8,
      "parent": "C05F11/00",
      "ja": "でい炭，かっ炭または同様の植物質埋蔵物から造られたもの",
      "en": "from peat, brown coal, and similar vegetable deposits",
      "count": 18211,
      "ipcCode": "C05F11/02",
      "coreCode": "C05F11/02",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F11/04": {
      "code": "C05F11/04",
      "level": 9,
      "parent": "C05F11/02",
      "ja": "でい炭から造られた園芸用土（ガルトナーリーシェ　エルデン）",
      "en": "Horticultural earth from peat (growth substrates containing peat A01G24/28)",
      "count": 739,
      "ipcCode": "C05F11/04",
      "coreCode": "C05F11/04",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F11/06": {
      "code": "C05F11/06",
      "level": 9,
      "parent": "C05F11/02",
      "ja": "製造装置",
      "en": "Apparatus for the manufacture",
      "count": 162,
      "ipcCode": "C05F11/06",
      "coreCode": "C05F11/06",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F11/08": {
      "code": "C05F11/08",
      "level": 8,
      "parent": "C05F11/00",
      "ja": "添加された微生物の培養物，菌糸体または同じようなものを含む有機質肥料",
      "en": "Organic fertilisers containing added bacterial cultures, mycelia or the like",
      "count": 26574,
      "ipcCode": "C05F11/08",
      "coreCode": "C05F11/08",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F11/10": {
      "code": "C05F11/10",
      "level": 8,
      "parent": "C05F11/00",
      "ja": "植物ビタミンまたは植物ホルモンを含む肥料",
      "en": "Fertilisers containing plant vitamins or hormones",
      "count": 4368,
      "ipcCode": "C05F11/10",
      "coreCode": "C05F11/10",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/00": {
      "code": "C05F17/00",
      "level": 7,
      "parent": null,
      "ja": "たい肥にする方法に特徴がある肥料の製造",
      "en": "Preparation of fertilisers characterised by biological or biochemical treatment steps, e.g. composting or fermentation",
      "count": 10947,
      "ipcCode": "C05F17/00",
      "coreCode": "C05F17/00",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/05": {
      "code": "C05F17/05",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Treatments involving invertebrates, e.g. worms, flies or maggots",
      "count": 795,
      "ipcCode": "C05F17/05",
      "coreCode": "C05F17/05",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/10": {
      "code": "C05F17/10",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Addition or removal of substances other than water or air to or from the material during the treatment (for activating or stimulating the treatment C05F17/20)",
      "count": 1224,
      "ipcCode": "C05F17/10",
      "coreCode": "C05F17/10",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/15": {
      "code": "C05F17/15",
      "level": 9,
      "parent": "C05F17/10",
      "ja": "",
      "en": "{the material being gas}",
      "count": 250,
      "ipcCode": "C05F17/10",
      "coreCode": "C05F17/15",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/20": {
      "code": "C05F17/20",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "using specific microorganisms or substances, e.g. enzymes, for activating or stimulating the treatment (use of invertebrates C05F17/05)",
      "count": 7247,
      "ipcCode": "C05F17/20",
      "coreCode": "C05F17/20",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/30": {
      "code": "C05F17/30",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "using irradiation, e.g. solar or nuclear radiation; using electric or magnetic fields",
      "count": 202,
      "ipcCode": "C05F17/30",
      "coreCode": "C05F17/30",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/40": {
      "code": "C05F17/40",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Treatment of liquids or slurries",
      "count": 1182,
      "ipcCode": "C05F17/40",
      "coreCode": "C05F17/40",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/50": {
      "code": "C05F17/50",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Treatments combining two or more different biological or biochemical treatments, e.g. anaerobic and aerobic treatment or vermicomposting and aerobic treatment",
      "count": 3903,
      "ipcCode": "C05F17/50",
      "coreCode": "C05F17/50",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/60": {
      "code": "C05F17/60",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Heating or cooling during the treatment",
      "count": 1194,
      "ipcCode": "C05F17/60",
      "coreCode": "C05F17/60",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/70": {
      "code": "C05F17/70",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Controlling the treatment in response to process parameters",
      "count": 1378,
      "ipcCode": "C05F17/70",
      "coreCode": "C05F17/70",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/80": {
      "code": "C05F17/80",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Separation, elimination or disposal of harmful substances during the treatment",
      "count": 1523,
      "ipcCode": "C05F17/80",
      "coreCode": "C05F17/80",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/90": {
      "code": "C05F17/90",
      "level": 8,
      "parent": "C05F17/00",
      "ja": "",
      "en": "Apparatus therefor",
      "count": 2772,
      "ipcCode": "C05F17/90",
      "coreCode": "C05F17/90",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/907": {
      "code": "C05F17/907",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "Small-scale devices without mechanical means for feeding or discharging material, e.g. garden compost bins",
      "count": 1177,
      "ipcCode": "C05F17/907",
      "coreCode": "C05F17/907",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/914": {
      "code": "C05F17/914",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "Portable or transportable devices, e.g. transport containers or trucks",
      "count": 465,
      "ipcCode": "C05F17/914",
      "coreCode": "C05F17/914",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/921": {
      "code": "C05F17/921",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "Devices in which the material is conveyed essentially horizontally between inlet and discharge means",
      "count": 111,
      "ipcCode": "C05F17/921",
      "coreCode": "C05F17/921",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/929": {
      "code": "C05F17/929",
      "level": 10,
      "parent": "C05F17/921",
      "ja": "",
      "en": "Cylinders or drums",
      "count": 284,
      "ipcCode": "C05F17/929",
      "coreCode": "C05F17/929",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/936": {
      "code": "C05F17/936",
      "level": 10,
      "parent": "C05F17/921",
      "ja": "",
      "en": "Tunnels",
      "count": 74,
      "ipcCode": "C05F17/936",
      "coreCode": "C05F17/936",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/939": {
      "code": "C05F17/939",
      "level": 10,
      "parent": "C05F17/921",
      "ja": "",
      "en": "{Means for mixing or moving with predetermined or fixed paths, e.g. rails or cables}",
      "count": 250,
      "ipcCode": "C05F17/921",
      "coreCode": "C05F17/939",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/943": {
      "code": "C05F17/943",
      "level": 10,
      "parent": "C05F17/921",
      "ja": "",
      "en": "Means for combined mixing and conveying",
      "count": 194,
      "ipcCode": "C05F17/943",
      "coreCode": "C05F17/943",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/95": {
      "code": "C05F17/95",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "Devices in which the material is conveyed essentially vertically between inlet and discharge means",
      "count": 546,
      "ipcCode": "C05F17/95",
      "coreCode": "C05F17/95",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/955": {
      "code": "C05F17/955",
      "level": 10,
      "parent": "C05F17/95",
      "ja": "",
      "en": "{the material going from platform to platform}",
      "count": 131,
      "ipcCode": "C05F17/95",
      "coreCode": "C05F17/95",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/957": {
      "code": "C05F17/957",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "using two or more serially arranged devices",
      "count": 462,
      "ipcCode": "C05F17/957",
      "coreCode": "C05F17/957",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/964": {
      "code": "C05F17/964",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "Constructional parts, e.g. floors, covers or doors",
      "count": 3341,
      "ipcCode": "C05F17/964",
      "coreCode": "C05F17/964",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/971": {
      "code": "C05F17/971",
      "level": 10,
      "parent": "C05F17/964",
      "ja": "",
      "en": "for feeding or discharging materials to be treated; for feeding or discharging other material",
      "count": 1079,
      "ipcCode": "C05F17/971",
      "coreCode": "C05F17/971",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/979": {
      "code": "C05F17/979",
      "level": 11,
      "parent": "C05F17/971",
      "ja": "",
      "en": "the other material being gaseous",
      "count": 1758,
      "ipcCode": "C05F17/979",
      "coreCode": "C05F17/979",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/986": {
      "code": "C05F17/986",
      "level": 11,
      "parent": "C05F17/971",
      "ja": "",
      "en": "the other material being liquid",
      "count": 771,
      "ipcCode": "C05F17/986",
      "coreCode": "C05F17/986",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/989": {
      "code": "C05F17/989",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "{Flow sheets for biological or biochemical treatment}",
      "count": 241,
      "ipcCode": "C05F17/90",
      "coreCode": "C05F17/989",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    },
    "C05F17/993": {
      "code": "C05F17/993",
      "level": 9,
      "parent": "C05F17/90",
      "ja": "",
      "en": "Arrangements for measuring process parameters, e.g. temperature, pressure or humidity",
      "count": 752,
      "ipcCode": "C05F17/993",
      "coreCode": "C05F17/993",
      "note": "",
      "sourceFile": "cpc_C05F.txt",
      "kind": "cpc"
    }
  }
};

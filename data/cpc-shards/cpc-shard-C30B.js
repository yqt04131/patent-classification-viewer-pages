window.CPC_SHARD_C30B = {
  "generatedAt": "2026-03-20T02:17:57.587Z",
  "total": 230,
  "entries": {
    "C30B": {
      "code": "C30B",
      "level": 5,
      "parent": "C30",
      "ja": "単結晶成長（超高圧を用いるもの，例．ダイヤモンド生成用Ｂ０１Ｊ３／０６）；共晶物質の一方向固化または共析晶物質の一方向析出；物質のゾーンメルティングによる精製（金属または合金のゾーン精製Ｃ２２Ｂ）；特定構造を有する均質多結晶物質の製造（金属の鋳造，同じ方法と装置による他の物質の鋳造Ｂ２２Ｄ；プラスチックの加工Ｂ２９；金属または合金の物理的構造の改良Ｃ２１Ｄ，Ｃ２２Ｆ）；単結晶または特定構造を有する均質多結晶物質；単結晶または特定構造を有する均質多結晶物質の後処理（半導体装置またはその部品を製造するためのものＨ０１Ｌ，Ｈ１０）；そのための装置<br><br><u>注</u><br><br>１．このサブクラスにおいては，下記の表現は以下に示す意味で用いる：<br>－単結晶は双晶およびほぼ単結晶状の生成物も含む；<br>－均質多結晶物質はそのすべてが同じ化学組成を有する結晶粒子をもつ物質を意味する；<br>－特定構造は選択方向に配向された結晶粒または通常得られるものよりも大きい結晶粒からなる物質の構造を意味する。<br>２．このサブクラスにおいては：<br>－特殊な材料または形状の単結晶または特定構造を有する均質多結晶物質の製造は，グループＣ３０Ｂ２９／００だけでなくプロセスに関するグループにも分類する；<br>－特定のプロセスに特に適合した装置はそのプロセスに関する適当なグループに分類する。２種以上のプロセスに用いられる装置はグループＣ３０Ｂ３５／００に分類する。<br><br><u>警告</u><br><br>このサブクラスにおいては（ＩＰＣ指針第３９段落の意味における）非限定参照も分類表において依然として表示される。",
      "en": "SINGLE-CRYSTAL GROWTH (by using ultra-high pressure, e.g. for the formation of diamonds, B01J3/06); UNIDIRECTIONAL SOLIDIFICATION OF EUTECTIC MATERIAL OR UNIDIRECTIONAL DEMIXING OF EUTECTOID MATERIAL; REFINING BY ZONE-MELTING OF MATERIAL (zone-refining of metals or alloys C22B); PRODUCTION OF A HOMOGENEOUS POLYCRYSTALLINE MATERIAL WITH DEFINED STRUCTURE (casting of metals, casting of other substances by the same processes or devices B22D; working of plastics B29; modifying the physical structure of metals or alloys C21D, C22F); SINGLE CRYSTALS OR HOMOGENEOUS POLYCRYSTALLINE MATERIAL WITH DEFINED STRUCTURE; AFTER-TREATMENT OF SINGLE CRYSTALS OR A HOMOGENEOUS POLYCRYSTALLINE MATERIAL WITH DEFINED STRUCTURE (for producing semiconductor devices or parts thereof H10); APPARATUS THEREFOR<br><br><u>NOTE</u><br><br>In this subclass, the following expressions are used with the meaning indicated: <br>\"single-crystal\" includes also twin crystals and a predominantly single crystal product; <br>\"homogeneous polycrystalline material\" means a material with crystal particles, all of which have the same chemical composition; <br>\"defined structure\" means the structure of a material with grains which are oriented in a preferential way or have larger dimensions than normally obtained.<br>In this subclass: <br>the preparation of crystals or a homogeneous polycrystalline material with defined structure of particular materials or shapes is classified in the group for the process as well as in group C30B29/00; <br>an apparatus specially adapted for a specific process is classified in the appropriate group for the process. Apparatus to be used in more than one kind of process is classified in group C30B35/00.<br><br><u>WARNING</u><br><br>In this subclass non-limiting references (in the sense of paragraph 39 of the Guide to the IPC) may still be displayed in the scheme.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C30": {
      "code": "C30",
      "level": 4,
      "parent": "C",
      "ja": "結晶成長",
      "en": "CRYSTAL GROWTH",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C": {
      "code": "C",
      "level": 2,
      "parent": null,
      "ja": "化学；冶金<br>（１）セクションＣにおいて，化学元素の種類に関する定義は次の通りである:<br>アルカリ金属:Ｌｉ，Ｎａ，Ｋ，Ｒｂ，Ｃｓ，Ｆｒ<br>アルカリ土類金属:Ｃａ，Ｓｒ，Ｂａ，Ｒａ<br>ランタニド:原子番号５７から７１までの元素<br>希土類:Ｓｃ，Ｙ，ランタニド<br>アクチニド:原子番号８９から１０３までの元素<br>耐火金属:Ｔｉ，Ｖ，Ｃｒ，Ｚｒ，Ｎｂ，Ｍｏ，Ｈｆ，Ｔａ，Ｗ<br>ハロゲン:Ｆ，Ｃｌ，Ｂｒ，Ｉ，Ａｔ<br>希ガス:Ｈｅ，Ｎｅ，Ａｒ，Ｋｒ，Ｘｅ，Ｒｎ<br>白金族:Ｏｓ，Ｉｒ，Ｐｔ，Ｒｕ，Ｒｈ，Ｐｄ<br>貴金属:Ａｇ，Ａｕ，白金族<br>軽金属:アルカリ金属，アルカリ土類金属，Ｂｅ，Ａｌ，Ｍｇ<br>重金属:軽金属以外の金属<br>鉄族:Ｆｅ，Ｃｏ，Ｎｉ<br>非金属:Ｈ，Ｂ，Ｃ，Ｓｉ，Ｎ，Ｐ，Ｏ，Ｓ，Ｓｅ，Ｔｅ，希ガス，ハロゲン<br>金属:非金属以外の元素<br>遷移元素:原子番号２１から３０まで，３９から４８まで，５７から８０まで，８９以上の元素<br>（２）Ｃセクションは以下のものを包含する:<br>（ａ）無機化合物，有機化合物，高分子化合物およびそれらの調製法を包含する純粋化学；<br>（ｂ）たとえばガラス，セラミック，肥料，プラスチック組成物，ペイント，石油工業の製品のような上記化合物を含有する組成物を包含する応用化学。それはまた，爆発物，染料，接着剤，潤滑剤および洗剤のように，ある目的に適した特別の性質をもつための組成物をも包含する；<br>（ｃ）コークスの製造，固体または気体燃料の製造，油脂およびろうの製造および精製，発酵工業（例．ビールおよびぶどう酒の製造），製糖工業のようなある境界工業；<br>（ｄ）ある操作または処理であって，純粋に機械的な処理，例．レザーおよび皮の機械的処理，または部分的機械的処理，例．水処理，または腐食防止一般であるもの；<br>（ｅ）冶金，鉄または非鉄合金<br>（３）ＩＰＣの全てのセクションにおいて，相反する指示がない限り、元素記号周期律表への言及は、以下に示す８つのグループを表す。例えば、グループＣ０７Ｆ３／００「周期律表の第２族の元素を含有する化合物」は、ＩＩＡ及びＩＩＢ族の元素への言及とする。<br>「図」<br>（４）（ａ）化学的部分または面および非化学的部分または面の両方を有する操作，処理，製品または物品の場合に，一般規則として，化学的部分または面はＣセクションに包含される。<br>（ｂ）これらのうちのある場合には，化学的部分または面には非化学的部分または面を伴い，純粋に機械的部分または面を伴うことさえある，というのは非化学的面は操作または処理にとって本質的であるかまたはそれらの重要な要素を構成するからである；実際，全体のさまざまな部分または面を分割しないのがより論理的と思える。これは応用化学や上記注（１）（ｃ），（ｄ）および（ｅ）の工業，操作および処理の場合に適用される。例えば，ガラス製造用特殊炉はＣ０３に包含されＦ２７には包含されない。<br>（ｃ）しかしながら，機械的面（または非化学的面）に非化学的な面が伴うという例外もある，例えば:<br>―サブクラスＡ６１Ｋにおけるある抽出プロセス；<br>―サブクラスＡ６１Ｌにおける空気の化学的精製；<br>―サブクラスＡ６２Ｄにおける化学的消火方法；<br>―クラスＢ０１における化学的プロセスおよび装置；<br>―サブクラスＢ２７Ｋにおける木材の含浸；<br>―サブクラスＧ０１Ｎにおける化学的分析または試験法；<br>―クラスＧ０３における写真材料またはプロセス，そして一般に，Ｄセクションにおける繊維の化学的処理および繊維素，紙等の製造<br>（ｄ）さらに他の場合，純粋化学的面はＣセクションに包含され，応用化学的面はＡ，Ｂ，Ｆセクションのような他のセクションに包含される，例．以下の物質または組成物の使用:<br>―サブクラスＡ０１Ｎに包含される植物または動物の処理；<br>―クラスＡ２３に包含される食物；<br>―クラスＦ４２に包含される弾薬または爆薬<br>（ｅ）化学的面および機械的面が非常に交錯して，簡単明確な分割ができない時，または化学的処理が自然なまたは論理的な延長として機械的プロセスを要する時は，化学的面に加えて，Ｃセクションは機械的面の一部のみ，例．クラスＣ０４に包含される人造石の後処理，を包含する。この後者の場合に，たとえその区別がむしろ任意であってもその位置を明確にするよう注または参照を通常設けている。",
      "en": "CHEMISTRY; METALLURGY<br><br><u>NOTE</u><br><br>In section C, the definitions of groups of chemical elements are as follows: <br>Alkali metals: Li, Na, K, Rb, Cs, Fr <br>Alkaline earth metals: Ca, Sr, Ba, Ra<br>Lanthanides: elements with atomic numbers 57 to 71 inclusive <br>Rare earths: Sc, Y, Lanthanides<br>Actinides: elements with atomic numbers 89 to 103 inclusive<br>Refractory metals: Ti, V, Cr, Zr, Nb, Mo, Hf, Ta, W <br>Halogens: F, Cl, Br, I, At <br>Noble gases: He, Ne, Ar, Kr, Xe, Rn <br>Platinum group: Os, Ir, Pt, Ru, Rh, Pd <br>Noble metals: Ag, Au, Platinum group <br>Light metals: alkali metals, alkaline earth metals, Be, Al, Mg<br>Heavy metals: metals other than light metals<br>Iron group: Fe, Co, Ni<br>Non-metals: H, B, C, Si, N, P, O, S, Se, Te, noble gases, halogens<br>Metals: elements other than non-metals <br>Transition elements: elements with atomic numbers 21 to 30 inclusive, 39 to 48 inclusive, 57 to 80 inclusive, 89 upwards<br>Section Ccovers:<br>pure chemistry, which covers inorganic compounds, organic compounds, macromolecular compounds, and their methods of preparation;<br>applied chemistry, which covers compositions containing the above compounds, such as: glass, ceramics, fertilisers, plastics compositions, paints, products of the petroleum industry. It also covers certain compositions on account of their having particular properties rendering them suitable for certain purposes, as in the case of explosives, dyestuffs, adhesives, lubricants and detergents;<br>certain marginal industries, such as the manufacture of coke and of solid or gaseous fuels, the production and refining of oils, fats and waxes, the fermentation industry (e.g., brewing and wine-making), the sugar industry;<br>certain operations or treatments, which are either purely mechanical, e.g., the mechanical treatment of leather and skins, or partly mechanical, e.g., the treatment of water or the prevention of corrosion in general;<br>metallurgy, ferrous or non-ferrous alloys.<br>In all sections of the CPC, in the absence of an indication to the contrary, the Periodic Table of chemical elements referred to is the one with 18 groups as represented in the table below.<br><br><br>In the case of operations, treatments, products or articles having both a chemical and a non-chemical part or aspect, the general rule is that the chemical part or aspect is covered by section C.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Notes (2) c), d) and e). For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example:<br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g. the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Note (2) bullets 2, 3, 4 and 5. For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example: <br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g., the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "C30B1/00": {
      "code": "C30B1/00",
      "level": 7,
      "parent": null,
      "ja": "固相からの直接単結晶成長（共析晶物質の一方向析出Ｃ３０Ｂ３／００；保護流体下で行うものＣ３０Ｂ２７／００）",
      "en": "Single-crystal growth directly from the solid state (unidirectional demixing of eutectoid materials C30B3/00; under a protective fluid C30B27/00)",
      "count": 183,
      "ipcCode": "C30B1/00",
      "coreCode": "C30B1/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/02": {
      "code": "C30B1/02",
      "level": 8,
      "parent": "C30B1/00",
      "ja": "熱処理によるもの，例．歪焼鈍（Ｃ３０Ｂ１／１２が優先）",
      "en": "by thermal treatment, e.g. strain annealing (C30B1/12 takes precedence)",
      "count": 686,
      "ipcCode": "C30B1/02",
      "coreCode": "C30B1/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/023": {
      "code": "C30B1/023",
      "level": 9,
      "parent": "C30B1/02",
      "ja": "｛アモルファス構造を有する固体からのもの｝",
      "en": "{from solids with amorphous structure}",
      "count": 275,
      "ipcCode": "C30B1/02",
      "coreCode": "C30B1/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/026": {
      "code": "C30B1/026",
      "level": 9,
      "parent": "C30B1/02",
      "ja": "｛無秩序な中間層を通じた固相エピタキシャル成長｝",
      "en": "{Solid phase epitaxial growth through a disordered intermediate layer}",
      "count": 21,
      "ipcCode": "C30B1/02",
      "coreCode": "C30B1/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/04": {
      "code": "C30B1/04",
      "level": 9,
      "parent": "C30B1/02",
      "ja": "等温再結晶化",
      "en": "Isothermal recrystallisation",
      "count": 63,
      "ipcCode": "C30B1/04",
      "coreCode": "C30B1/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/06": {
      "code": "C30B1/06",
      "level": 9,
      "parent": "C30B1/02",
      "ja": "温度勾配下での再結晶化",
      "en": "Recrystallisation under a temperature gradient",
      "count": 28,
      "ipcCode": "C30B1/06",
      "coreCode": "C30B1/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/08": {
      "code": "C30B1/08",
      "level": 10,
      "parent": "C30B1/06",
      "ja": "ゾーン再結晶化",
      "en": "Zone recrystallisation",
      "count": 37,
      "ipcCode": "C30B1/08",
      "coreCode": "C30B1/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/10": {
      "code": "C30B1/10",
      "level": 8,
      "parent": "C30B1/00",
      "ja": "固相反応または多相拡散によるもの",
      "en": "by solid state reactions or multi-phase diffusion",
      "count": 688,
      "ipcCode": "C30B1/10",
      "coreCode": "C30B1/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B1/12": {
      "code": "C30B1/12",
      "level": 8,
      "parent": "C30B1/00",
      "ja": "成長中の圧力処理によるもの",
      "en": "by pressure treatment during the growth",
      "count": 221,
      "ipcCode": "C30B1/12",
      "coreCode": "C30B1/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B3/00": {
      "code": "C30B3/00",
      "level": 7,
      "parent": null,
      "ja": "共析晶物質の一方向析出",
      "en": "Unidirectional demixing of eutectoid materials",
      "count": 10,
      "ipcCode": "C30B3/00",
      "coreCode": "C30B3/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B5/00": {
      "code": "C30B5/00",
      "level": 7,
      "parent": null,
      "ja": "ゲルからの単結晶成長（保護流体下で行うものＣ３０Ｂ２７／００）",
      "en": "Single-crystal growth from gels (under a protective fluid C30B27/00)",
      "count": 190,
      "ipcCode": "C30B5/00",
      "coreCode": "C30B5/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B5/02": {
      "code": "C30B5/02",
      "level": 8,
      "parent": "C30B5/00",
      "ja": "ドープ物質を加えるもの",
      "en": "with addition of doping materials",
      "count": 12,
      "ipcCode": "C30B5/02",
      "coreCode": "C30B5/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/00": {
      "code": "C30B7/00",
      "level": 7,
      "parent": null,
      "ja": "常温で液体の溶媒を用いる溶液からの単結晶成長，例．水溶液（溶融溶媒からのものＣ３０Ｂ９／００；ノーマル・フリージングまたは温度勾配凝固によるものＣ３０Ｂ１１／００；保護流体下で行うものＣ３０Ｂ２７／００）",
      "en": "Single-crystal growth from solutions using solvents which are liquid at normal temperature, e.g. aqueous solutions (from molten solvents C30B9/00; by normal or gradient freezing C30B11/00; under a protective fluid C30B27/00)",
      "count": 1504,
      "ipcCode": "C30B7/00",
      "coreCode": "C30B7/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/005": {
      "code": "C30B7/005",
      "level": 8,
      "parent": "C30B7/00",
      "ja": "｛エピタキシャル層の成長｝",
      "en": "{Epitaxial layer growth}",
      "count": 277,
      "ipcCode": "C30B7/00",
      "coreCode": "C30B7/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/02": {
      "code": "C30B7/02",
      "level": 8,
      "parent": "C30B7/00",
      "ja": "溶媒の蒸発によるもの",
      "en": "by evaporation of the solvent",
      "count": 117,
      "ipcCode": "C30B7/02",
      "coreCode": "C30B7/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/04": {
      "code": "C30B7/04",
      "level": 9,
      "parent": "C30B7/02",
      "ja": "水性溶媒を用いるもの",
      "en": "using aqueous solvents",
      "count": 166,
      "ipcCode": "C30B7/04",
      "coreCode": "C30B7/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/06": {
      "code": "C30B7/06",
      "level": 9,
      "parent": "C30B7/02",
      "ja": "非水性溶媒を用いるもの",
      "en": "using non-aqueous solvents",
      "count": 245,
      "ipcCode": "C30B7/06",
      "coreCode": "C30B7/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/08": {
      "code": "C30B7/08",
      "level": 8,
      "parent": "C30B7/00",
      "ja": "溶液の冷却によるもの",
      "en": "by cooling of the solution",
      "count": 320,
      "ipcCode": "C30B7/08",
      "coreCode": "C30B7/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/10": {
      "code": "C30B7/10",
      "level": 8,
      "parent": "C30B7/00",
      "ja": "圧力を加えるもの，例．水熱法",
      "en": "by application of pressure, e.g. hydrothermal processes",
      "count": 806,
      "ipcCode": "C30B7/10",
      "coreCode": "C30B7/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/105": {
      "code": "C30B7/105",
      "level": 9,
      "parent": "C30B7/10",
      "ja": "｛アンモニアを溶媒として用いるもの，すなわちアンモニア熱法｝",
      "en": "{using ammonia as solvent, i.e. ammonothermal processes}",
      "count": 172,
      "ipcCode": "C30B7/10",
      "coreCode": "C30B7/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/12": {
      "code": "C30B7/12",
      "level": 8,
      "parent": "C30B7/00",
      "ja": "電解によるもの",
      "en": "by electrolysis",
      "count": 113,
      "ipcCode": "C30B7/12",
      "coreCode": "C30B7/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B7/14": {
      "code": "C30B7/14",
      "level": 8,
      "parent": "C30B7/00",
      "ja": "結晶化物質が溶液中の化学反応により形成されるもの",
      "en": "the crystallising materials being formed by chemical reactions in the solution",
      "count": 1527,
      "ipcCode": "C30B7/14",
      "coreCode": "C30B7/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/00": {
      "code": "C30B9/00",
      "level": 7,
      "parent": null,
      "ja": "溶融溶媒を用いる融液からの単結晶成長（ノーマル・フリージングまたは温度勾配凝固によるものはＣ３０Ｂ１１／００；ゾーンメルティングによるものＣ３０Ｂ１３／００；結晶引出しによるものＣ３０Ｂ１５／００；浸漬された種結晶上に成長するものＣ３０Ｂ１７／００；液相エピタキシャル成長によるものＣ３０Ｂ１９／００；保護流体下で行うものＣ３０Ｂ２７／００）",
      "en": "Single-crystal growth from melt solutions using molten solvents (by normal or gradient freezing C30B11/00; by zone-melting C30B13/00; by crystal pulling C30B15/00; on immersed seed crystal C30B17/00; by liquid phase epitaxial growth C30B19/00; under a protective fluid C30B27/00)",
      "count": 544,
      "ipcCode": "C30B9/00",
      "coreCode": "C30B9/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/02": {
      "code": "C30B9/02",
      "level": 8,
      "parent": "C30B9/00",
      "ja": "溶融溶媒の蒸発によるもの",
      "en": "by evaporation of the molten solvent",
      "count": 17,
      "ipcCode": "C30B9/02",
      "coreCode": "C30B9/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/04": {
      "code": "C30B9/04",
      "level": 8,
      "parent": "C30B9/00",
      "ja": "溶液の冷却によるもの",
      "en": "by cooling of the solution",
      "count": 52,
      "ipcCode": "C30B9/04",
      "coreCode": "C30B9/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/06": {
      "code": "C30B9/06",
      "level": 9,
      "parent": "C30B9/04",
      "ja": "溶媒として結晶組成の一成分を用いるもの",
      "en": "using as solvent a component of the crystal composition",
      "count": 161,
      "ipcCode": "C30B9/06",
      "coreCode": "C30B9/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/08": {
      "code": "C30B9/08",
      "level": 9,
      "parent": "C30B9/04",
      "ja": "他の溶媒を用いるもの",
      "en": "using other solvents",
      "count": 28,
      "ipcCode": "C30B9/08",
      "coreCode": "C30B9/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/10": {
      "code": "C30B9/10",
      "level": 10,
      "parent": "C30B9/08",
      "ja": "金属溶媒",
      "en": "Metal solvents",
      "count": 214,
      "ipcCode": "C30B9/10",
      "coreCode": "C30B9/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/12": {
      "code": "C30B9/12",
      "level": 10,
      "parent": "C30B9/08",
      "ja": "塩溶媒，例．フラックス成長",
      "en": "Salt solvents, e.g. flux growth",
      "count": 524,
      "ipcCode": "C30B9/12",
      "coreCode": "C30B9/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B9/14": {
      "code": "C30B9/14",
      "level": 8,
      "parent": "C30B9/00",
      "ja": "電解によるもの",
      "en": "by electrolysis",
      "count": 27,
      "ipcCode": "C30B9/14",
      "coreCode": "C30B9/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/00": {
      "code": "C30B11/00",
      "level": 7,
      "parent": null,
      "ja": "ノーマル・フリージングまたは温度勾配凝固による単結晶成長，例．ブリッジマン―ストックバーガー法（Ｃ３０Ｂ１３／００，Ｃ３０Ｂ１５／００，Ｃ３０Ｂ１７／００，Ｃ３０Ｂ１９／００が優先；保護流体下で行うものＣ３０Ｂ２７／００）",
      "en": "Single-crystal growth by normal freezing or freezing under temperature gradient, e.g. Bridgman-Stockbarger method (C30B13/00, C30B15/00, C30B17/00, C30B19/00 take precedence; under a protective fluid C30B27/00)",
      "count": 2305,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/001": {
      "code": "C30B11/001",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛連続成長｝",
      "en": "{Continuous growth}",
      "count": 156,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/002": {
      "code": "C30B11/002",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛融液を支持するためのるつぼまたは容器｝",
      "en": "{Crucibles or containers for supporting the melt}",
      "count": 986,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/003": {
      "code": "C30B11/003",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛融液または結晶化した物質の加熱または冷却｝",
      "en": "{Heating or cooling of the melt or the crystallised material}",
      "count": 994,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/005": {
      "code": "C30B11/005",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛照射または電気放電によるもの｝",
      "en": "{by irradiation or electric discharge}",
      "count": 56,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/006": {
      "code": "C30B11/006",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛制御または調整｝",
      "en": "{Controlling or regulating}",
      "count": 527,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/007": {
      "code": "C30B11/007",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛材料またはヒーターの移動機構｝",
      "en": "{Mechanisms for moving either the charge or the heater}",
      "count": 165,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/008": {
      "code": "C30B11/008",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "｛材料への遠心力を用いるもの｝",
      "en": "{using centrifugal force to the charge}",
      "count": 49,
      "ipcCode": "C30B11/00",
      "coreCode": "C30B11/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/02": {
      "code": "C30B11/02",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "溶媒を使用しないもの（Ｃ３０Ｂ１１／０６が優先）",
      "en": "without using solvents (C30B11/06 takes precedence)",
      "count": 275,
      "ipcCode": "C30B11/02",
      "coreCode": "C30B11/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/04": {
      "code": "C30B11/04",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "融液中に結晶化物質またはそれをその場所で生成する反応剤を添加するもの",
      "en": "adding crystallising materials or reactants forming it in situ to the melt",
      "count": 133,
      "ipcCode": "C30B11/04",
      "coreCode": "C30B11/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/06": {
      "code": "C30B11/06",
      "level": 9,
      "parent": "C30B11/04",
      "ja": "結晶組成の全成分ではなく少なくとも１つの成分を加えるもの",
      "en": "at least one but not all components of the crystal composition being added",
      "count": 92,
      "ipcCode": "C30B11/06",
      "coreCode": "C30B11/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/065": {
      "code": "C30B11/065",
      "level": 10,
      "parent": "C30B11/06",
      "ja": "｛結晶化する前のもの，例．合成｝",
      "en": "{before crystallising, e.g. synthesis}",
      "count": 33,
      "ipcCode": "C30B11/06",
      "coreCode": "C30B11/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/08": {
      "code": "C30B11/08",
      "level": 9,
      "parent": "C30B11/04",
      "ja": "結晶化中に結晶組成の全ての成分を加えるもの",
      "en": "every component of the crystal composition being added during the crystallisation",
      "count": 41,
      "ipcCode": "C30B11/08",
      "coreCode": "C30B11/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/10": {
      "code": "C30B11/10",
      "level": 10,
      "parent": "C30B11/08",
      "ja": "固体または液体成分，例．ベルヌーイ法",
      "en": "Solid or liquid components, e.g. Verneuil method",
      "count": 320,
      "ipcCode": "C30B11/10",
      "coreCode": "C30B11/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/12": {
      "code": "C30B11/12",
      "level": 10,
      "parent": "C30B11/08",
      "ja": "蒸気成分，例．気相―液相―固相成長（ＶＬＳ）",
      "en": "Vaporous components, e.g. vapour-liquid-solid-growth",
      "count": 241,
      "ipcCode": "C30B11/12",
      "coreCode": "C30B11/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B11/14": {
      "code": "C30B11/14",
      "level": 8,
      "parent": "C30B11/00",
      "ja": "種結晶によって特徴づけられたもの，例．その結晶方位",
      "en": "characterised by the seed, e.g. its crystallographic orientation",
      "count": 447,
      "ipcCode": "C30B11/14",
      "coreCode": "C30B11/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/00": {
      "code": "C30B13/00",
      "level": 7,
      "parent": null,
      "ja": "ゾーンメルティングによる単結晶成長；ゾーンメルティングによる精製（Ｃ３０Ｂ１７／００が優先；処理された固体の断面積を変化させるものＣ３０Ｂ１５／００；保護流体下で行なうものＣ３０Ｂ２７／００；特定物質のゾーン精製は，その物質に関連したサブクラス参照）",
      "en": "Single-crystal growth by zone-melting; Refining by zone-melting (C30B17/00 takes precedence; by changing the cross-section of the treated solid C30B15/00; under a protective fluid C30B27/00; for the growth of homogeneous polycrystalline material with defined structure C30B28/00)",
      "count": 936,
      "ipcCode": "C30B13/00",
      "coreCode": "C30B13/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/005": {
      "code": "C30B13/005",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "｛連続成長｝",
      "en": "{Continuous growth}",
      "count": 47,
      "ipcCode": "C30B13/00",
      "coreCode": "C30B13/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/02": {
      "code": "C30B13/02",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "溶媒を用いるゾーンメルティング，例．移動溶媒法",
      "en": "Zone-melting with a solvent, e.g. travelling solvent process",
      "count": 138,
      "ipcCode": "C30B13/02",
      "coreCode": "C30B13/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/04": {
      "code": "C30B13/04",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "ゾーンレベリングによる均質化",
      "en": "Homogenisation by zone-levelling",
      "count": 20,
      "ipcCode": "C30B13/04",
      "coreCode": "C30B13/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/06": {
      "code": "C30B13/06",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "溶融ゾーンが全断面にわたって広がらないもの",
      "en": "the molten zone not extending over the whole cross-section",
      "count": 95,
      "ipcCode": "C30B13/06",
      "coreCode": "C30B13/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/08": {
      "code": "C30B13/08",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "溶融ゾーンに結晶化物質またはそれをその場所で生成する反応剤を添加するもの",
      "en": "adding crystallising materials or reactants forming it in situ to the molten zone",
      "count": 68,
      "ipcCode": "C30B13/08",
      "coreCode": "C30B13/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/10": {
      "code": "C30B13/10",
      "level": 9,
      "parent": "C30B13/08",
      "ja": "ドープ物質を加えるもの",
      "en": "with addition of doping materials",
      "count": 136,
      "ipcCode": "C30B13/10",
      "coreCode": "C30B13/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/12": {
      "code": "C30B13/12",
      "level": 10,
      "parent": "C30B13/10",
      "ja": "ガスまたは蒸気状態で",
      "en": "in the gaseous or vapour state",
      "count": 73,
      "ipcCode": "C30B13/12",
      "coreCode": "C30B13/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/14": {
      "code": "C30B13/14",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "るつぼまたは容器",
      "en": "Crucibles or vessels",
      "count": 130,
      "ipcCode": "C30B13/14",
      "coreCode": "C30B13/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/16": {
      "code": "C30B13/16",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "溶融ゾーンの加熱",
      "en": "Heating of the molten zone",
      "count": 185,
      "ipcCode": "C30B13/16",
      "coreCode": "C30B13/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/18": {
      "code": "C30B13/18",
      "level": 9,
      "parent": "C30B13/16",
      "ja": "加熱素子が溶融ゾーンに接触または浸漬されるもの",
      "en": "the heating element being in contact with, or immersed in, the molten zone",
      "count": 39,
      "ipcCode": "C30B13/18",
      "coreCode": "C30B13/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/20": {
      "code": "C30B13/20",
      "level": 9,
      "parent": "C30B13/16",
      "ja": "誘導によるもの，例．ホットワイヤ技術（Ｃ３０Ｂ１３／１８が優先）",
      "en": "by induction, e.g. hot wire technique (C30B13/18 takes precedence)",
      "count": 253,
      "ipcCode": "C30B13/20",
      "coreCode": "C30B13/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/22": {
      "code": "C30B13/22",
      "level": 9,
      "parent": "C30B13/16",
      "ja": "照射または電気放電によるもの",
      "en": "by irradiation or electric discharge",
      "count": 215,
      "ipcCode": "C30B13/22",
      "coreCode": "C30B13/22",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/24": {
      "code": "C30B13/24",
      "level": 10,
      "parent": "C30B13/22",
      "ja": "電磁波を用いるもの",
      "en": "using electromagnetic waves",
      "count": 387,
      "ipcCode": "C30B13/24",
      "coreCode": "C30B13/24",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/26": {
      "code": "C30B13/26",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "溶融ゾーンの攪拌",
      "en": "Stirring of the molten zone",
      "count": 54,
      "ipcCode": "C30B13/26",
      "coreCode": "C30B13/26",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/28": {
      "code": "C30B13/28",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "制御または調整（制御または調整一般Ｇ０５）",
      "en": "Controlling or regulating",
      "count": 262,
      "ipcCode": "C30B13/28",
      "coreCode": "C30B13/28",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/285": {
      "code": "C30B13/285",
      "level": 9,
      "parent": "C30B13/28",
      "ja": "｛結晶ホルダ，例．チャック｝",
      "en": "{Crystal holders, e.g. chucks}",
      "count": 94,
      "ipcCode": "C30B13/28",
      "coreCode": "C30B13/28",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/30": {
      "code": "C30B13/30",
      "level": 9,
      "parent": "C30B13/28",
      "ja": "溶融ゾーンの安定化または形状の制御，例．コンセントレイターによるもの，電磁界によるもの；結晶断面の制御",
      "en": "Stabilisation or shape controlling of the molten zone, e.g. by concentrators, by electromagnetic fields; Controlling the section of the crystal",
      "count": 192,
      "ipcCode": "C30B13/30",
      "coreCode": "C30B13/30",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/32": {
      "code": "C30B13/32",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "材料またはヒーターの移動機構",
      "en": "Mechanisms for moving either the charge or the heater",
      "count": 150,
      "ipcCode": "C30B13/32",
      "coreCode": "C30B13/32",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B13/34": {
      "code": "C30B13/34",
      "level": 8,
      "parent": "C30B13/00",
      "ja": "種結晶によって特徴づけられたもの，例．その結晶方位",
      "en": "characterised by the seed, e.g. by its crystallographic orientation",
      "count": 90,
      "ipcCode": "C30B13/34",
      "coreCode": "C30B13/34",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/00": {
      "code": "C30B15/00",
      "level": 7,
      "parent": null,
      "ja": "融液からの引出しによる単結晶成長，例．チョクラルスキー法（保護流体下で行うものＣ３０Ｂ２７／００）",
      "en": "Single-crystal growth by pulling from a melt, e.g. Czochralski method (under a protective fluid C30B27/00)",
      "count": 3166,
      "ipcCode": "C30B15/00",
      "coreCode": "C30B15/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/002": {
      "code": "C30B15/002",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "｛連続成長｝",
      "en": "{Continuous growth}",
      "count": 278,
      "ipcCode": "C30B15/00",
      "coreCode": "C30B15/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/005": {
      "code": "C30B15/005",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "｛２以上の結晶の同時引き出し｝",
      "en": "{Simultaneous pulling of more than one crystal}",
      "count": 80,
      "ipcCode": "C30B15/00",
      "coreCode": "C30B15/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/007": {
      "code": "C30B15/007",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "｛基板上への引き出し｝",
      "en": "{Pulling on a substrate}",
      "count": 171,
      "ipcCode": "C30B15/00",
      "coreCode": "C30B15/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/02": {
      "code": "C30B15/02",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "融液に結晶化物質またはそれをその場所で生成する反応剤を添加するもの",
      "en": "adding crystallising materials or reactants forming it in situ to the melt",
      "count": 861,
      "ipcCode": "C30B15/02",
      "coreCode": "C30B15/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/04": {
      "code": "C30B15/04",
      "level": 9,
      "parent": "C30B15/02",
      "ja": "ドープ物質を加えるもの，例．ＰＮ接合用",
      "en": "adding doping materials, e.g. for n-p-junction",
      "count": 775,
      "ipcCode": "C30B15/04",
      "coreCode": "C30B15/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/06": {
      "code": "C30B15/06",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "非垂直引出し",
      "en": "Non-vertical pulling",
      "count": 107,
      "ipcCode": "C30B15/06",
      "coreCode": "C30B15/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/08": {
      "code": "C30B15/08",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "下方引出し",
      "en": "Downward pulling",
      "count": 190,
      "ipcCode": "C30B15/08",
      "coreCode": "C30B15/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/10": {
      "code": "C30B15/10",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "融液を支持するためのるつぼまたは容器",
      "en": "Crucibles or containers for supporting the melt",
      "count": 1775,
      "ipcCode": "C30B15/10",
      "coreCode": "C30B15/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/12": {
      "code": "C30B15/12",
      "level": 9,
      "parent": "C30B15/10",
      "ja": "二重るつぼ法",
      "en": "Double crucible methods",
      "count": 309,
      "ipcCode": "C30B15/12",
      "coreCode": "C30B15/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/14": {
      "code": "C30B15/14",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "融液または結晶化した物質の加熱",
      "en": "Heating of the melt or the crystallised materials",
      "count": 1568,
      "ipcCode": "C30B15/14",
      "coreCode": "C30B15/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/16": {
      "code": "C30B15/16",
      "level": 9,
      "parent": "C30B15/14",
      "ja": "照射または電気放電によるもの",
      "en": "by irradiation or electric discharge",
      "count": 104,
      "ipcCode": "C30B15/16",
      "coreCode": "C30B15/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/18": {
      "code": "C30B15/18",
      "level": 9,
      "parent": "C30B15/14",
      "ja": "直接抵抗加熱に加えて他の加熱方法を用いるもの，例．ペルチェ加熱を用いるもの",
      "en": "using direct resistance heating in addition to other methods of heating, e.g. using Peltier heat",
      "count": 78,
      "ipcCode": "C30B15/18",
      "coreCode": "C30B15/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/20": {
      "code": "C30B15/20",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "制御または調整（制御または調整一般Ｇ０５）",
      "en": "Controlling or regulating (controlling or regulating in general G05)",
      "count": 2027,
      "ipcCode": "C30B15/20",
      "coreCode": "C30B15/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/203": {
      "code": "C30B15/203",
      "level": 9,
      "parent": "C30B15/20",
      "ja": "｛引出し率（ｖ）と軸方向熱勾配（Ｇ）との関係｝",
      "en": "{the relationship of pull rate (v) to axial thermal gradient (G)}",
      "count": 403,
      "ipcCode": "C30B15/20",
      "coreCode": "C30B15/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/206": {
      "code": "C30B15/206",
      "level": 9,
      "parent": "C30B15/20",
      "ja": "｛鋳塊の成長における熱履歴｝",
      "en": "{the thermal history of growing the ingot}",
      "count": 405,
      "ipcCode": "C30B15/20",
      "coreCode": "C30B15/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/22": {
      "code": "C30B15/22",
      "level": 9,
      "parent": "C30B15/20",
      "ja": "引出された結晶近傍の溶融ゾーンの安定化または形状の制御；結晶断面の制御",
      "en": "Stabilisation or shape controlling of the molten zone near the pulled crystal; Controlling the section of the crystal",
      "count": 571,
      "ipcCode": "C30B15/22",
      "coreCode": "C30B15/22",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/24": {
      "code": "C30B15/24",
      "level": 10,
      "parent": "C30B15/22",
      "ja": "機械的手段を用いるもの，例．成形ガイド（縁部限定薄膜供給結晶成長用の成形型Ｃ３０Ｂ１５／３４）",
      "en": "using mechanical means, e.g. shaping guides (shaping dies for edge-defined film-fed crystal growth C30B15/34)",
      "count": 162,
      "ipcCode": "C30B15/24",
      "coreCode": "C30B15/24",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/26": {
      "code": "C30B15/26",
      "level": 10,
      "parent": "C30B15/22",
      "ja": "テレビジョン検出器を用いるもの；光またはＸ線検出器を用いるもの",
      "en": "using television detectors; using photo or X-ray detectors",
      "count": 367,
      "ipcCode": "C30B15/26",
      "coreCode": "C30B15/26",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/28": {
      "code": "C30B15/28",
      "level": 10,
      "parent": "C30B15/22",
      "ja": "結晶または融液の重量変化を用いるもの，例．フローテーション法",
      "en": "using weight changes of the crystal or the melt, e.g. flotation methods",
      "count": 160,
      "ipcCode": "C30B15/28",
      "coreCode": "C30B15/28",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/30": {
      "code": "C30B15/30",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "融液または結晶を回転または移動させるための機構（フローテーション法Ｃ３０Ｂ１５／２８）",
      "en": "Mechanisms for rotating or moving either the melt or the crystal (flotation methods C30B15/28)",
      "count": 833,
      "ipcCode": "C30B15/30",
      "coreCode": "C30B15/30",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/305": {
      "code": "C30B15/305",
      "level": 9,
      "parent": "C30B15/30",
      "ja": "｛融液の撹拌｝",
      "en": "{Stirring of the melt}",
      "count": 346,
      "ipcCode": "C30B15/30",
      "coreCode": "C30B15/30",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/32": {
      "code": "C30B15/32",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "種結晶保持器，例．チャック",
      "en": "Seed holders, e.g. chucks",
      "count": 327,
      "ipcCode": "C30B15/32",
      "coreCode": "C30B15/32",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/34": {
      "code": "C30B15/34",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "型またはスリットを用いる縁部限定薄膜供給結晶成長",
      "en": "Edge-defined film-fed crystal-growth using dies or slits",
      "count": 434,
      "ipcCode": "C30B15/34",
      "coreCode": "C30B15/34",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B15/36": {
      "code": "C30B15/36",
      "level": 8,
      "parent": "C30B15/00",
      "ja": "種結晶により特徴づけられたもの，例．その結晶方位",
      "en": "characterised by the seed, e.g. its crystallographic orientation",
      "count": 412,
      "ipcCode": "C30B15/36",
      "coreCode": "C30B15/36",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B17/00": {
      "code": "C30B17/00",
      "level": 7,
      "parent": null,
      "ja": "成長中融液に浸した種結晶上への単結晶成長，例．ナッケン―キロポロス法（Ｃ３０Ｂ１５／００が優先）",
      "en": "Single-crystal growth onto a seed which remains in the melt during growth, e.g. Nacken-Kyropoulos method (C30B15/00 takes precedence)",
      "count": 379,
      "ipcCode": "C30B17/00",
      "coreCode": "C30B17/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/00": {
      "code": "C30B19/00",
      "level": 7,
      "parent": null,
      "ja": "液相エピタキシャル成長",
      "en": "Liquid-phase epitaxial-layer growth",
      "count": 254,
      "ipcCode": "C30B19/00",
      "coreCode": "C30B19/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/02": {
      "code": "C30B19/02",
      "level": 8,
      "parent": "C30B19/00",
      "ja": "溶融溶媒を用いるもの，例．フラックス",
      "en": "using molten solvents, e.g. flux",
      "count": 406,
      "ipcCode": "C30B19/02",
      "coreCode": "C30B19/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/04": {
      "code": "C30B19/04",
      "level": 9,
      "parent": "C30B19/02",
      "ja": "溶媒が結晶組成の一成分であるもの",
      "en": "the solvent being a component of the crystal composition",
      "count": 330,
      "ipcCode": "C30B19/04",
      "coreCode": "C30B19/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/06": {
      "code": "C30B19/06",
      "level": 8,
      "parent": "C30B19/00",
      "ja": "反応室；融液支持用ボート；基板保持体",
      "en": "Reaction chambers; Boats for supporting the melt; Substrate holders",
      "count": 133,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/061": {
      "code": "C30B19/061",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛傾転システム，例．回転によるもの｝",
      "en": "{Tipping system, e.g. by rotation}",
      "count": 99,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/062": {
      "code": "C30B19/062",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛垂直浸漬システム｝",
      "en": "{Vertical dipping system}",
      "count": 115,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/063": {
      "code": "C30B19/063",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛スライドボートシステム｝",
      "en": "{Sliding boat system}",
      "count": 228,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/064": {
      "code": "C30B19/064",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛回転スライドボートシステム｝",
      "en": "{Rotating sliding boat system}",
      "count": 24,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/065": {
      "code": "C30B19/065",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛多層スライドシステム｝",
      "en": "{Multiple stacked slider system}",
      "count": 11,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/066": {
      "code": "C30B19/066",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛射出または遠心力システム｝",
      "en": "{Injection or centrifugal force system}",
      "count": 23,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/067": {
      "code": "C30B19/067",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛ボートまたは容器｝",
      "en": "{Boots or containers}",
      "count": 41,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/068": {
      "code": "C30B19/068",
      "level": 9,
      "parent": "C30B19/06",
      "ja": "｛基板保持体｝",
      "en": "{Substrate holders}",
      "count": 100,
      "ipcCode": "C30B19/06",
      "coreCode": "C30B19/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/08": {
      "code": "C30B19/08",
      "level": 8,
      "parent": "C30B19/00",
      "ja": "反応室または基板の加熱",
      "en": "Heating of the reaction chamber or the substrate",
      "count": 114,
      "ipcCode": "C30B19/08",
      "coreCode": "C30B19/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/10": {
      "code": "C30B19/10",
      "level": 8,
      "parent": "C30B19/00",
      "ja": "制御または調整（制御または調整一般Ｇ０５）",
      "en": "Controlling or regulating (controlling or regulating in general G05)",
      "count": 183,
      "ipcCode": "C30B19/10",
      "coreCode": "C30B19/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/103": {
      "code": "C30B19/103",
      "level": 9,
      "parent": "C30B19/10",
      "ja": "｛電流制御または誘導成長｝",
      "en": "{Current controlled or induced growth}",
      "count": 25,
      "ipcCode": "C30B19/10",
      "coreCode": "C30B19/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/106": {
      "code": "C30B19/106",
      "level": 9,
      "parent": "C30B19/10",
      "ja": "｛結晶化物質またはその場で液体を形成する反応剤を添加するもの｝",
      "en": "{adding crystallising material or reactants forming it in situ to the liquid}",
      "count": 95,
      "ipcCode": "C30B19/10",
      "coreCode": "C30B19/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B19/12": {
      "code": "C30B19/12",
      "level": 8,
      "parent": "C30B19/00",
      "ja": "基板によって特徴づけられたもの",
      "en": "characterised by the substrate",
      "count": 355,
      "ipcCode": "C30B19/12",
      "coreCode": "C30B19/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B21/00": {
      "code": "C30B21/00",
      "level": 7,
      "parent": null,
      "ja": "共晶物質の一方向固化",
      "en": "Unidirectional solidification of eutectic materials",
      "count": 20,
      "ipcCode": "C30B21/00",
      "coreCode": "C30B21/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B21/02": {
      "code": "C30B21/02",
      "level": 8,
      "parent": "C30B21/00",
      "ja": "通常の鋳造または温度勾配凝固によるもの",
      "en": "by normal casting or gradient freezing",
      "count": 244,
      "ipcCode": "C30B21/02",
      "coreCode": "C30B21/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B21/04": {
      "code": "C30B21/04",
      "level": 8,
      "parent": "C30B21/00",
      "ja": "ゾーンメルティングによるもの",
      "en": "by zone-melting",
      "count": 44,
      "ipcCode": "C30B21/04",
      "coreCode": "C30B21/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B21/06": {
      "code": "C30B21/06",
      "level": 8,
      "parent": "C30B21/00",
      "ja": "融液からの引出しによるもの",
      "en": "by pulling from a melt",
      "count": 20,
      "ipcCode": "C30B21/06",
      "coreCode": "C30B21/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/00": {
      "code": "C30B23/00",
      "level": 7,
      "parent": null,
      "ja": "蒸発または昇華した物質の凝固による単結晶成長<br><br><u>注</u><br><br>グループC30B２３/００２～C30B２３/００DがグループC３０B２３/００７～C３０B２３/０８に優先する",
      "en": "Single-crystal growth by condensing evaporated or sublimed materials<br><br><u>NOTE</u><br><br>Groups C30B23/002&#160;-&#160;C30B23/005 take precedence over groups C30B23/007&#160;-&#160;C30B23/08",
      "count": 1686,
      "ipcCode": "C30B23/00",
      "coreCode": "C30B23/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/002": {
      "code": "C30B23/002",
      "level": 8,
      "parent": "C30B23/00",
      "ja": "｛制御または調整｝",
      "en": "{Controlling or regulating}",
      "count": 921,
      "ipcCode": "C30B23/00",
      "coreCode": "C30B23/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/005": {
      "code": "C30B23/005",
      "level": 9,
      "parent": "C30B23/002",
      "ja": "｛堆積物または蒸発物のフラックスまたは流れの制御または調整｝",
      "en": "{Controlling or regulating flux or flow of depositing species or vapour}",
      "count": 208,
      "ipcCode": "C30B23/00",
      "coreCode": "C30B23/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/007": {
      "code": "C30B23/007",
      "level": 8,
      "parent": "C30B23/00",
      "ja": "｛ひげ結晶または針状結晶の成長｝",
      "en": "{Growth of whiskers or needles}",
      "count": 67,
      "ipcCode": "C30B23/00",
      "coreCode": "C30B23/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/02": {
      "code": "C30B23/02",
      "level": 8,
      "parent": "C30B23/00",
      "ja": "エピタキシャル層成長",
      "en": "Epitaxial-layer growth",
      "count": 1899,
      "ipcCode": "C30B23/02",
      "coreCode": "C30B23/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/025": {
      "code": "C30B23/025",
      "level": 9,
      "parent": "C30B23/02",
      "ja": "｛基板に特徴のあるもの｝",
      "en": "{characterised by the substrate}",
      "count": 780,
      "ipcCode": "C30B23/02",
      "coreCode": "C30B23/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/04": {
      "code": "C30B23/04",
      "level": 9,
      "parent": "C30B23/02",
      "ja": "パターン沈着，例．マスクを用いるもの",
      "en": "Pattern deposit, e.g. by using masks",
      "count": 114,
      "ipcCode": "C30B23/04",
      "coreCode": "C30B23/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/06": {
      "code": "C30B23/06",
      "level": 9,
      "parent": "C30B23/02",
      "ja": "沈着室，基板または被蒸発物質の加熱",
      "en": "Heating of the deposition chamber, the substrate or the materials to be evaporated",
      "count": 373,
      "ipcCode": "C30B23/06",
      "coreCode": "C30B23/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/063": {
      "code": "C30B23/063",
      "level": 10,
      "parent": "C30B23/06",
      "ja": "｛基板の加熱｝",
      "en": "{Heating of the substrate}",
      "count": 176,
      "ipcCode": "C30B23/06",
      "coreCode": "C30B23/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/066": {
      "code": "C30B23/066",
      "level": 10,
      "parent": "C30B23/06",
      "ja": "｛蒸発させるための材料の加熱｝",
      "en": "{Heating of the material to be evaporated}",
      "count": 423,
      "ipcCode": "C30B23/06",
      "coreCode": "C30B23/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B23/08": {
      "code": "C30B23/08",
      "level": 9,
      "parent": "C30B23/02",
      "ja": "イオン化蒸気の凝縮によるもの（反応スパッタリングによるものＣ３０Ｂ２５／０６）",
      "en": "by condensing ionised vapours (by reactive sputtering C30B25/06)",
      "count": 124,
      "ipcCode": "C30B23/08",
      "coreCode": "C30B23/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/00": {
      "code": "C30B25/00",
      "level": 7,
      "parent": null,
      "ja": "反応ガスの化学反応による単結晶成長，例．化学蒸着による成長",
      "en": "Single-crystal growth by chemical reaction of reactive gases, e.g. chemical vapour-deposition growth",
      "count": 1134,
      "ipcCode": "C30B25/00",
      "coreCode": "C30B25/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/005": {
      "code": "C30B25/005",
      "level": 8,
      "parent": "C30B25/00",
      "ja": "｛ひげ結晶または針状結晶の成長｝",
      "en": "{Growth of whiskers or needles}",
      "count": 536,
      "ipcCode": "C30B25/00",
      "coreCode": "C30B25/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/02": {
      "code": "C30B25/02",
      "level": 8,
      "parent": "C30B25/00",
      "ja": "エピタキシャル層成長",
      "en": "Epitaxial-layer growth",
      "count": 3626,
      "ipcCode": "C30B25/02",
      "coreCode": "C30B25/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/025": {
      "code": "C30B25/025",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "｛連続成長｝",
      "en": "{Continuous growth}",
      "count": 93,
      "ipcCode": "C30B25/02",
      "coreCode": "C30B25/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/04": {
      "code": "C30B25/04",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "パターン沈着，例．マスクを用いるもの",
      "en": "Pattern deposit, e.g. by using masks",
      "count": 425,
      "ipcCode": "C30B25/04",
      "coreCode": "C30B25/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/06": {
      "code": "C30B25/06",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "反応スパッタリングによるもの",
      "en": "by reactive sputtering",
      "count": 130,
      "ipcCode": "C30B25/06",
      "coreCode": "C30B25/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/08": {
      "code": "C30B25/08",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "反応室；そのための材料の選択",
      "en": "Reaction chambers; Selection of materials therefor",
      "count": 847,
      "ipcCode": "C30B25/08",
      "coreCode": "C30B25/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/10": {
      "code": "C30B25/10",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "反応室または基板の加熱",
      "en": "Heating of the reaction chamber or the substrate",
      "count": 1053,
      "ipcCode": "C30B25/10",
      "coreCode": "C30B25/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/105": {
      "code": "C30B25/105",
      "level": 10,
      "parent": "C30B25/10",
      "ja": "｛照射または電気放電によるもの｝",
      "en": "{by irradiation or electric discharge}",
      "count": 742,
      "ipcCode": "C30B25/10",
      "coreCode": "C30B25/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/12": {
      "code": "C30B25/12",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "基板保持体またはサセプタ",
      "en": "Substrate holders or susceptors",
      "count": 1420,
      "ipcCode": "C30B25/12",
      "coreCode": "C30B25/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/14": {
      "code": "C30B25/14",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "ガスの供給および排出手段；反応ガス流の調節",
      "en": "Feed and outlet means for the gases; Modifying the flow of the reactive gases",
      "count": 2133,
      "ipcCode": "C30B25/14",
      "coreCode": "C30B25/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/16": {
      "code": "C30B25/16",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "制御または調整（制御または調整一般Ｇ０５）",
      "en": "Controlling or regulating (controlling or regulating in general G05)",
      "count": 1228,
      "ipcCode": "C30B25/16",
      "coreCode": "C30B25/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/165": {
      "code": "C30B25/165",
      "level": 10,
      "parent": "C30B25/16",
      "ja": "｛反応ガスの流れの｝",
      "en": "{the flow of the reactive gases}",
      "count": 618,
      "ipcCode": "C30B25/16",
      "coreCode": "C30B25/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/18": {
      "code": "C30B25/18",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "基板によって特徴づけられたもの",
      "en": "characterised by the substrate",
      "count": 1651,
      "ipcCode": "C30B25/18",
      "coreCode": "C30B25/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/183": {
      "code": "C30B25/183",
      "level": 10,
      "parent": "C30B25/18",
      "ja": "｛バッファ層，例．格子整合層，の提供であるもの｝",
      "en": "{being provided with a buffer layer, e.g. a lattice matching layer}",
      "count": 1033,
      "ipcCode": "C30B25/18",
      "coreCode": "C30B25/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/186": {
      "code": "C30B25/186",
      "level": 10,
      "parent": "C30B25/18",
      "ja": "｛特に事前に，例．化学的または物理的手段，によって処理されたもの｝",
      "en": "{being specially pre-treated by, e.g. chemical or physical means}",
      "count": 1124,
      "ipcCode": "C30B25/18",
      "coreCode": "C30B25/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/20": {
      "code": "C30B25/20",
      "level": 10,
      "parent": "C30B25/18",
      "ja": "基板がエピタキシャル層と同一物質であるもの",
      "en": "the substrate being of the same materials as the epitaxial layer",
      "count": 812,
      "ipcCode": "C30B25/20",
      "coreCode": "C30B25/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/205": {
      "code": "C30B25/205",
      "level": 11,
      "parent": "C30B25/20",
      "ja": "｛基板が絶縁材料であるもの｝",
      "en": "{the substrate being of insulating material}",
      "count": 187,
      "ipcCode": "C30B25/20",
      "coreCode": "C30B25/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B25/22": {
      "code": "C30B25/22",
      "level": 9,
      "parent": "C30B25/02",
      "ja": "サンドイッチプロセス",
      "en": "Sandwich processes",
      "count": 104,
      "ipcCode": "C30B25/22",
      "coreCode": "C30B25/22",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B27/00": {
      "code": "C30B27/00",
      "level": 7,
      "parent": null,
      "ja": "保護流体下における単結晶成長",
      "en": "Single-crystal growth under a protective fluid",
      "count": 152,
      "ipcCode": "C30B27/00",
      "coreCode": "C30B27/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B27/02": {
      "code": "C30B27/02",
      "level": 8,
      "parent": "C30B27/00",
      "ja": "融液からの引出しによるもの",
      "en": "by pulling from a melt",
      "count": 403,
      "ipcCode": "C30B27/02",
      "coreCode": "C30B27/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/00": {
      "code": "C30B28/00",
      "level": 7,
      "parent": null,
      "ja": "特定構造を有する均質多結晶物質の製造",
      "en": "Production of homogeneous polycrystalline material with defined structure",
      "count": 56,
      "ipcCode": "C30B28/00",
      "coreCode": "C30B28/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/02": {
      "code": "C30B28/02",
      "level": 8,
      "parent": "C30B28/00",
      "ja": "固相から直接に",
      "en": "directly from the solid state",
      "count": 347,
      "ipcCode": "C30B28/02",
      "coreCode": "C30B28/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/04": {
      "code": "C30B28/04",
      "level": 8,
      "parent": "C30B28/00",
      "ja": "液体から",
      "en": "from liquids",
      "count": 285,
      "ipcCode": "C30B28/04",
      "coreCode": "C30B28/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/06": {
      "code": "C30B28/06",
      "level": 9,
      "parent": "C30B28/04",
      "ja": "ノーマル・フリージングまたは温度勾配凝固によるもの",
      "en": "by normal freezing or freezing under temperature gradient",
      "count": 930,
      "ipcCode": "C30B28/06",
      "coreCode": "C30B28/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/08": {
      "code": "C30B28/08",
      "level": 9,
      "parent": "C30B28/04",
      "ja": "ゾーンメルティングによるもの",
      "en": "by zone-melting",
      "count": 72,
      "ipcCode": "C30B28/08",
      "coreCode": "C30B28/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/10": {
      "code": "C30B28/10",
      "level": 9,
      "parent": "C30B28/04",
      "ja": "融液からの引出しによるもの",
      "en": "by pulling from a melt",
      "count": 144,
      "ipcCode": "C30B28/10",
      "coreCode": "C30B28/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/12": {
      "code": "C30B28/12",
      "level": 8,
      "parent": "C30B28/00",
      "ja": "気相から直接に",
      "en": "directly from the gas state",
      "count": 144,
      "ipcCode": "C30B28/12",
      "coreCode": "C30B28/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B28/14": {
      "code": "C30B28/14",
      "level": 9,
      "parent": "C30B28/12",
      "ja": "反応ガスの化学反応によるもの",
      "en": "by chemical reaction of reactive gases",
      "count": 437,
      "ipcCode": "C30B28/14",
      "coreCode": "C30B28/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/00": {
      "code": "C30B29/00",
      "level": 7,
      "parent": null,
      "ja": "材料または形状によって特徴づけられた単結晶または特定構造を有する均質多結晶物質<br><br><u>注</u><br><br>１．グループＣ３０Ｂ２９／０２～Ｃ３０Ｂ２９／５４においては，ラストプレイス優先ルールが適用される，すなわち各階層レベルにおいて相反する指示がない限り，物質は最後の適切な箇所に分類する。<br>２．ＩＰＣがどの版の化学元素周期表を参照しているかを示すセクションＣのタイトルに続く注（３）に注意すること。このグループで用いられるシステムは，周期表においてローマ数字で表示された８つのグループからなるシステムである。",
      "en": "Single crystals or homogeneous polycrystalline material with defined structure characterised by the material or by their shape<br><br><u>NOTE</u><br><br>In groups C30B29/02 - C30B29/54, the last place priority rule is applied, i.e. at each hierarchical level, in the absence of an indication to the contrary, a material is classified in the last appropriate place.<br>Attention is drawn to Note (3) after the title of section C, which Note indicates to which version of the Periodic Table of chemical elements the CPC refers. In this group, the system used is the 8 group system indicated by Roman numerals in the Periodic Table thereunder.",
      "count": 87,
      "ipcCode": "C30B29/00",
      "coreCode": "C30B29/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/02": {
      "code": "C30B29/02",
      "level": 8,
      "parent": "C30B29/00",
      "ja": "元素",
      "en": "Elements",
      "count": 1335,
      "ipcCode": "C30B29/02",
      "coreCode": "C30B29/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/04": {
      "code": "C30B29/04",
      "level": 9,
      "parent": "C30B29/02",
      "ja": "ダイヤモンド",
      "en": "Diamond",
      "count": 1860,
      "ipcCode": "C30B29/04",
      "coreCode": "C30B29/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/06": {
      "code": "C30B29/06",
      "level": 9,
      "parent": "C30B29/02",
      "ja": "シリコン",
      "en": "Silicon",
      "count": 9784,
      "ipcCode": "C30B29/06",
      "coreCode": "C30B29/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/08": {
      "code": "C30B29/08",
      "level": 9,
      "parent": "C30B29/02",
      "ja": "ゲルマニウム",
      "en": "Germanium",
      "count": 470,
      "ipcCode": "C30B29/08",
      "coreCode": "C30B29/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/10": {
      "code": "C30B29/10",
      "level": 8,
      "parent": "C30B29/00",
      "ja": "無機化合物または組成物",
      "en": "Inorganic compounds or compositions",
      "count": 941,
      "ipcCode": "C30B29/10",
      "coreCode": "C30B29/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/12": {
      "code": "C30B29/12",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "ハロゲン化物",
      "en": "Halides",
      "count": 1368,
      "ipcCode": "C30B29/12",
      "coreCode": "C30B29/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/14": {
      "code": "C30B29/14",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "りん酸塩",
      "en": "Phosphates",
      "count": 296,
      "ipcCode": "C30B29/14",
      "coreCode": "C30B29/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/16": {
      "code": "C30B29/16",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "酸化物",
      "en": "Oxides",
      "count": 1901,
      "ipcCode": "C30B29/16",
      "coreCode": "C30B29/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/18": {
      "code": "C30B29/18",
      "level": 10,
      "parent": "C30B29/16",
      "ja": "石英",
      "en": "Quartz",
      "count": 219,
      "ipcCode": "C30B29/18",
      "coreCode": "C30B29/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/20": {
      "code": "C30B29/20",
      "level": 10,
      "parent": "C30B29/16",
      "ja": "酸化アルミニウム",
      "en": "Aluminium oxides",
      "count": 1195,
      "ipcCode": "C30B29/20",
      "coreCode": "C30B29/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/22": {
      "code": "C30B29/22",
      "level": 10,
      "parent": "C30B29/16",
      "ja": "複合酸化物",
      "en": "Complex oxides",
      "count": 1987,
      "ipcCode": "C30B29/22",
      "coreCode": "C30B29/22",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/225": {
      "code": "C30B29/225",
      "level": 11,
      "parent": "C30B29/22",
      "ja": "｛希土類銅酸化物，例.Ｔ－超導体，に基づくもの｝",
      "en": "{based on rare earth copper oxides, e.g. high T-superconductors}",
      "count": 296,
      "ipcCode": "C30B29/22",
      "coreCode": "C30B29/22",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/24": {
      "code": "C30B29/24",
      "level": 11,
      "parent": "C30B29/22",
      "ja": "式ＡＭｅＯ３を有するもの，ここでＡは希土類金属またはＭｅはＦｅ，Ｇａ，Ｓｃ，Ｃｒ，ＣｏまたはＡｌ，例．オルソフェライト",
      "en": "with formula AMeO3, wherein A is a rare earth metal and Me is Fe, Ga, Sc, Cr, Co or Al, e.g. ortho ferrites",
      "count": 139,
      "ipcCode": "C30B29/24",
      "coreCode": "C30B29/24",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/26": {
      "code": "C30B29/26",
      "level": 11,
      "parent": "C30B29/22",
      "ja": "式ＢＭｅ２Ｏ４を有するもの，ここでＢはＭｇ，Ｎｉ，Ｃｏ，Ａｌ，ＺｎまたはＣｄまたＭｅはＦｅ，Ｇａ，Ｓｃ，Ｃｒ，ＣｏまたはＡｌ",
      "en": "with formula BMe2O4, wherein B is Mg, Ni, Co, Al, Zn, or Cd and Me is Fe, Ga, Sc, Cr, Co, or Al",
      "count": 393,
      "ipcCode": "C30B29/26",
      "coreCode": "C30B29/26",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/28": {
      "code": "C30B29/28",
      "level": 11,
      "parent": "C30B29/22",
      "ja": "式Ａ↓３Ｍｅ↓５Ｏ↓１↓２を有するもの，ここでＡは希土類金属またＭｅはＦｅ，Ｇａ，Ｓｃ，Ｃｒ，ＣｏまたはＡｌ，例．ガーネット",
      "en": "with formula A3Me5O12 wherein A is a rare earth metal and Me is Fe, Ga, Sc, Cr, Co or Al, e.g. garnets",
      "count": 601,
      "ipcCode": "C30B29/28",
      "coreCode": "C30B29/28",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/30": {
      "code": "C30B29/30",
      "level": 11,
      "parent": "C30B29/22",
      "ja": "ニオブ酸塩；バナジン酸塩；タンタル酸塩",
      "en": "Niobates; Vanadates; Tantalates",
      "count": 688,
      "ipcCode": "C30B29/30",
      "coreCode": "C30B29/30",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/32": {
      "code": "C30B29/32",
      "level": 11,
      "parent": "C30B29/22",
      "ja": "チタン酸塩；ゲルマニウム酸塩；モリブデン酸塩；タングステン酸塩",
      "en": "Titanates; Germanates; Molybdates; Tungstates",
      "count": 606,
      "ipcCode": "C30B29/32",
      "coreCode": "C30B29/32",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/34": {
      "code": "C30B29/34",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "けい酸塩",
      "en": "Silicates",
      "count": 381,
      "ipcCode": "C30B29/34",
      "coreCode": "C30B29/34",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/36": {
      "code": "C30B29/36",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "炭化物",
      "en": "Carbides",
      "count": 4363,
      "ipcCode": "C30B29/36",
      "coreCode": "C30B29/36",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/38": {
      "code": "C30B29/38",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "窒化物",
      "en": "Nitrides",
      "count": 950,
      "ipcCode": "C30B29/38",
      "coreCode": "C30B29/38",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/40": {
      "code": "C30B29/40",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "ＡⅢＢⅤ化合物｛ここでＡはＢ，Ａｌ，Ｇａ，ＩｎまたはＴｌ，およびＢはＮ，Ｐ，Ａｓ，ＳｂまたはＢ｝",
      "en": "AIIIBV compounds {wherein A is B, Al, Ga, In or Tl and B is N, P, As, Sb or Bi}",
      "count": 1749,
      "ipcCode": "C30B29/40",
      "coreCode": "C30B29/40",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/403": {
      "code": "C30B29/403",
      "level": 10,
      "parent": "C30B29/40",
      "ja": "｛ＡＩＩＩ窒化物｝",
      "en": "{AIII-nitrides}",
      "count": 2288,
      "ipcCode": "C30B29/40",
      "coreCode": "C30B29/40",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/406": {
      "code": "C30B29/406",
      "level": 11,
      "parent": "C30B29/403",
      "ja": "｛窒化ガリウム｝",
      "en": "{Gallium nitride}",
      "count": 2159,
      "ipcCode": "C30B29/40",
      "coreCode": "C30B29/40",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/42": {
      "code": "C30B29/42",
      "level": 10,
      "parent": "C30B29/40",
      "ja": "ひ化ガリウム",
      "en": "Gallium arsenide",
      "count": 710,
      "ipcCode": "C30B29/42",
      "coreCode": "C30B29/42",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/44": {
      "code": "C30B29/44",
      "level": 10,
      "parent": "C30B29/40",
      "ja": "りん化ガリウム",
      "en": "Gallium phosphide",
      "count": 104,
      "ipcCode": "C30B29/44",
      "coreCode": "C30B29/44",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/46": {
      "code": "C30B29/46",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "硫黄，セレンまたはテルルを含む化合物",
      "en": "Sulfur-, selenium- or tellurium-containing compounds",
      "count": 1620,
      "ipcCode": "C30B29/46",
      "coreCode": "C30B29/46",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/48": {
      "code": "C30B29/48",
      "level": 10,
      "parent": "C30B29/46",
      "ja": "ＡⅡＢⅥ化合物｛ここでＡはＺｎ，ＣｄまたはＨｇ，およびＢはＳ，ＳｅまたはＴｅ｝",
      "en": "AIIBVI compounds {wherein A is Zn, Cd or Hg, and B is S, Se or Te}",
      "count": 785,
      "ipcCode": "C30B29/48",
      "coreCode": "C30B29/48",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/50": {
      "code": "C30B29/50",
      "level": 11,
      "parent": "C30B29/48",
      "ja": "硫化カドミウム",
      "en": "Cadmium sulfide",
      "count": 41,
      "ipcCode": "C30B29/50",
      "coreCode": "C30B29/50",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/52": {
      "code": "C30B29/52",
      "level": 9,
      "parent": "C30B29/10",
      "ja": "合金",
      "en": "Alloys",
      "count": 1622,
      "ipcCode": "C30B29/52",
      "coreCode": "C30B29/52",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/54": {
      "code": "C30B29/54",
      "level": 8,
      "parent": "C30B29/00",
      "ja": "有機化合物",
      "en": "Organic compounds",
      "count": 1051,
      "ipcCode": "C30B29/54",
      "coreCode": "C30B29/54",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/56": {
      "code": "C30B29/56",
      "level": 9,
      "parent": "C30B29/54",
      "ja": "酒石酸塩",
      "en": "Tartrates",
      "count": 29,
      "ipcCode": "C30B29/56",
      "coreCode": "C30B29/56",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/58": {
      "code": "C30B29/58",
      "level": 9,
      "parent": "C30B29/54",
      "ja": "高分子化合物",
      "en": "Macromolecular compounds",
      "count": 542,
      "ipcCode": "C30B29/58",
      "coreCode": "C30B29/58",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/60": {
      "code": "C30B29/60",
      "level": 8,
      "parent": "C30B29/00",
      "ja": "形状により特徴づけられたもの",
      "en": "characterised by shape",
      "count": 1295,
      "ipcCode": "C30B29/60",
      "coreCode": "C30B29/60",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/602": {
      "code": "C30B29/602",
      "level": 9,
      "parent": "C30B29/60",
      "ja": "｛ナノチューブ｝",
      "en": "{Nanotubes}",
      "count": 102,
      "ipcCode": "C30B29/60",
      "coreCode": "C30B29/60",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/605": {
      "code": "C30B29/605",
      "level": 9,
      "parent": "C30B29/60",
      "ja": "｛多配向結晶子を含有する生成物，例．円柱状結晶子｝",
      "en": "{Products containing multiple oriented crystallites, e.g. columnar crystallites}",
      "count": 603,
      "ipcCode": "C30B29/60",
      "coreCode": "C30B29/60",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/62": {
      "code": "C30B29/62",
      "level": 9,
      "parent": "C30B29/60",
      "ja": "ひげ結晶または針状結晶",
      "en": "Whiskers or needles",
      "count": 988,
      "ipcCode": "C30B29/62",
      "coreCode": "C30B29/62",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/64": {
      "code": "C30B29/64",
      "level": 9,
      "parent": "C30B29/60",
      "ja": "平型結晶，例．板，帯状体，円板",
      "en": "Flat crystals, e.g. plates, strips or discs",
      "count": 604,
      "ipcCode": "C30B29/64",
      "coreCode": "C30B29/64",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/66": {
      "code": "C30B29/66",
      "level": 9,
      "parent": "C30B29/60",
      "ja": "複雑な幾何学的形状の結晶，例．管，円筒",
      "en": "Crystals of complex geometrical shape, e.g. tubes, cylinders",
      "count": 358,
      "ipcCode": "C30B29/66",
      "coreCode": "C30B29/66",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B29/68": {
      "code": "C30B29/68",
      "level": 9,
      "parent": "C30B29/60",
      "ja": "層構造からなる結晶，例．超格子",
      "en": "Crystals with laminate structure, e.g. \"superlattices\"",
      "count": 549,
      "ipcCode": "C30B29/68",
      "coreCode": "C30B29/68",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B30/00": {
      "code": "C30B30/00",
      "level": 7,
      "parent": null,
      "ja": "電場，磁場，波動エネルギーまたはその他の特殊な物理的条件の作用により特徴づけられる単結晶または特定構造を有する均質多結晶物質の製造<br><br><u>注E</u><br><br>このグループに分類する場合，結晶成長の工程に従い，グループＣ３０Ｂ１／００からＣ３０Ｂ２７／００にも分類する。",
      "en": "Production of single crystals or homogeneous polycrystalline material with defined structure characterised by the action of electric or magnetic fields, wave energy or other specific physical conditions<br><br><u>NOTE</u><br><br>When classifying in this group, classification is also made in groups C30B1/00&#160;-&#160;C30B27/00 according to the process of crystal growth.",
      "count": 179,
      "ipcCode": "C30B30/00",
      "coreCode": "C30B30/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B30/02": {
      "code": "C30B30/02",
      "level": 8,
      "parent": "C30B30/00",
      "ja": "電場を用いるもの，例．電気分解",
      "en": "using electric fields, e.g. electrolysis",
      "count": 159,
      "ipcCode": "C30B30/02",
      "coreCode": "C30B30/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B30/04": {
      "code": "C30B30/04",
      "level": 8,
      "parent": "C30B30/00",
      "ja": "磁場を用いるもの",
      "en": "using magnetic fields",
      "count": 352,
      "ipcCode": "C30B30/04",
      "coreCode": "C30B30/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B30/06": {
      "code": "C30B30/06",
      "level": 8,
      "parent": "C30B30/00",
      "ja": "機械的振動を用いるもの",
      "en": "using mechanical vibrations",
      "count": 42,
      "ipcCode": "C30B30/06",
      "coreCode": "C30B30/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B30/08": {
      "code": "C30B30/08",
      "level": 8,
      "parent": "C30B30/00",
      "ja": "無重力または低重力の条件下のもの",
      "en": "in conditions of zero-gravity or low gravity",
      "count": 76,
      "ipcCode": "C30B30/08",
      "coreCode": "C30B30/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/00": {
      "code": "C30B31/00",
      "level": 7,
      "parent": null,
      "ja": "単結晶または特定構造を有する均質多結晶物質への拡散またはドーブ工程；そのための装置",
      "en": "Diffusion or doping processes for single crystals or homogeneous polycrystalline material with defined structure; Apparatus therefor",
      "count": 144,
      "ipcCode": "C30B31/00",
      "coreCode": "C30B31/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/02": {
      "code": "C30B31/02",
      "level": 8,
      "parent": "C30B31/00",
      "ja": "固相状態の拡散物質と接触させるもの",
      "en": "by contacting with diffusion materials in the solid state",
      "count": 290,
      "ipcCode": "C30B31/02",
      "coreCode": "C30B31/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/04": {
      "code": "C30B31/04",
      "level": 8,
      "parent": "C30B31/00",
      "ja": "液相状態の拡散物質と接触させるもの",
      "en": "by contacting with diffusion materials in the liquid state",
      "count": 234,
      "ipcCode": "C30B31/04",
      "coreCode": "C30B31/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/045": {
      "code": "C30B31/045",
      "level": 9,
      "parent": "C30B31/04",
      "ja": "｛電解によるもの｝",
      "en": "{by electrolysis}",
      "count": 2,
      "ipcCode": "C30B31/04",
      "coreCode": "C30B31/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/06": {
      "code": "C30B31/06",
      "level": 8,
      "parent": "C30B31/00",
      "ja": "ガス状態の拡散物質と接触させるもの",
      "en": "by contacting with diffusion material in the gaseous state",
      "count": 314,
      "ipcCode": "C30B31/06",
      "coreCode": "C30B31/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/08": {
      "code": "C30B31/08",
      "level": 9,
      "parent": "C30B31/06",
      "ja": "拡散物質が被拡散元素の化合物であるもの",
      "en": "the diffusion materials being a compound of the elements to be diffused",
      "count": 83,
      "ipcCode": "C30B31/08",
      "coreCode": "C30B31/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/10": {
      "code": "C30B31/10",
      "level": 9,
      "parent": "C30B31/06",
      "ja": "反応室；そのための材料の選択",
      "en": "Reaction chambers; Selection of materials therefor",
      "count": 246,
      "ipcCode": "C30B31/10",
      "coreCode": "C30B31/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/103": {
      "code": "C30B31/103",
      "level": 10,
      "parent": "C30B31/10",
      "ja": "｛装入物または加熱器のどちらかを移動するための機構｝",
      "en": "{Mechanisms for moving either the charge or heater}",
      "count": 65,
      "ipcCode": "C30B31/10",
      "coreCode": "C30B31/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/106": {
      "code": "C30B31/106",
      "level": 10,
      "parent": "C30B31/10",
      "ja": "｛連続法｝",
      "en": "{Continuous processes}",
      "count": 23,
      "ipcCode": "C30B31/10",
      "coreCode": "C30B31/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/12": {
      "code": "C30B31/12",
      "level": 9,
      "parent": "C30B31/06",
      "ja": "反応室の加熱",
      "en": "Heating of the reaction chamber",
      "count": 236,
      "ipcCode": "C30B31/12",
      "coreCode": "C30B31/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/14": {
      "code": "C30B31/14",
      "level": 9,
      "parent": "C30B31/06",
      "ja": "基板保持体またはサセプタ",
      "en": "Substrate holders or susceptors",
      "count": 254,
      "ipcCode": "C30B31/14",
      "coreCode": "C30B31/14",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/16": {
      "code": "C30B31/16",
      "level": 9,
      "parent": "C30B31/06",
      "ja": "ガスの供給および排出手段；ガス流の調節",
      "en": "Feed and outlet means for the gases; Modifying the flow of the gases",
      "count": 220,
      "ipcCode": "C30B31/16",
      "coreCode": "C30B31/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/165": {
      "code": "C30B31/165",
      "level": 10,
      "parent": "C30B31/16",
      "ja": "｛拡散源｝",
      "en": "{Diffusion sources}",
      "count": 134,
      "ipcCode": "C30B31/16",
      "coreCode": "C30B31/16",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/18": {
      "code": "C30B31/18",
      "level": 9,
      "parent": "C30B31/06",
      "ja": "制御または調整",
      "en": "Controlling or regulating",
      "count": 111,
      "ipcCode": "C30B31/18",
      "coreCode": "C30B31/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/185": {
      "code": "C30B31/185",
      "level": 10,
      "parent": "C30B31/18",
      "ja": "｛パターン拡散，例．マスクを用いるもの｝",
      "en": "{Pattern diffusion, e.g. by using masks}",
      "count": 33,
      "ipcCode": "C30B31/18",
      "coreCode": "C30B31/18",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/20": {
      "code": "C30B31/20",
      "level": 8,
      "parent": "C30B31/00",
      "ja": "電磁波照射または粒子線放射によるドービング",
      "en": "Doping by irradiation with electromagnetic waves or by particle radiation",
      "count": 96,
      "ipcCode": "C30B31/20",
      "coreCode": "C30B31/20",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B31/22": {
      "code": "C30B31/22",
      "level": 9,
      "parent": "C30B31/20",
      "ja": "イオン注入によるもの",
      "en": "by ion-implantation",
      "count": 262,
      "ipcCode": "C30B31/22",
      "coreCode": "C30B31/22",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/00": {
      "code": "C30B33/00",
      "level": 7,
      "parent": null,
      "ja": "単結晶または特定構造を有する均質多結晶物質の後処理（Ｃ３０Ｂ３１／００が優先）",
      "en": "After-treatment of single crystals or homogeneous polycrystalline material with defined structure (C30B31/00 takes precedence)",
      "count": 2372,
      "ipcCode": "C30B33/00",
      "coreCode": "C30B33/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/005": {
      "code": "C30B33/005",
      "level": 8,
      "parent": "C30B33/00",
      "ja": "｛酸化｝",
      "en": "{Oxydation}",
      "count": 356,
      "ipcCode": "C30B33/00",
      "coreCode": "C30B33/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/02": {
      "code": "C30B33/02",
      "level": 8,
      "parent": "C30B33/00",
      "ja": "熱処理（Ｃ３０Ｂ３３／０４，Ｃ３０Ｂ３３／０６が優先）",
      "en": "Heat treatment (C30B33/04, C30B33/06 take precedence)",
      "count": 1933,
      "ipcCode": "C30B33/02",
      "coreCode": "C30B33/02",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/04": {
      "code": "C30B33/04",
      "level": 8,
      "parent": "C30B33/00",
      "ja": "電場，磁場または粒子線放射を用いるもの",
      "en": "using electric or magnetic fields or particle radiation",
      "count": 382,
      "ipcCode": "C30B33/04",
      "coreCode": "C30B33/04",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/06": {
      "code": "C30B33/06",
      "level": 8,
      "parent": "C30B33/00",
      "ja": "結晶の結合",
      "en": "Joining of crystals",
      "count": 693,
      "ipcCode": "C30B33/06",
      "coreCode": "C30B33/06",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/08": {
      "code": "C30B33/08",
      "level": 8,
      "parent": "C30B33/00",
      "ja": "エッチング",
      "en": "Etching",
      "count": 201,
      "ipcCode": "C30B33/08",
      "coreCode": "C30B33/08",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/10": {
      "code": "C30B33/10",
      "level": 9,
      "parent": "C30B33/08",
      "ja": "溶液または融液中で",
      "en": "in solutions or melts",
      "count": 1088,
      "ipcCode": "C30B33/10",
      "coreCode": "C30B33/10",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B33/12": {
      "code": "C30B33/12",
      "level": 9,
      "parent": "C30B33/08",
      "ja": "気体雰囲気またはプラズマ下で",
      "en": "in gas atmosphere or plasma",
      "count": 304,
      "ipcCode": "C30B33/12",
      "coreCode": "C30B33/12",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B35/00": {
      "code": "C30B35/00",
      "level": 7,
      "parent": null,
      "ja": "単結晶または特定構造を有する均質多結晶物質の成長，製造または後処理のために特に適合した他に分類されない装置",
      "en": "Apparatus&#160;not otherwise provided for, specially adapted for the growth, production or after-treatment&#160;of single&#160;crystals&#160;or of a&#160;homogeneous polycrystalline material&#160;with&#160;defined structure",
      "count": 1377,
      "ipcCode": "C30B35/00",
      "coreCode": "C30B35/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B35/002": {
      "code": "C30B35/002",
      "level": 8,
      "parent": "C30B35/00",
      "ja": "｛るつぼまたは容器｝",
      "en": "{Crucibles or containers}",
      "count": 987,
      "ipcCode": "C30B35/00",
      "coreCode": "C30B35/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B35/005": {
      "code": "C30B35/005",
      "level": 8,
      "parent": "C30B35/00",
      "ja": "｛輸送システム｝",
      "en": "{Transport systems}",
      "count": 213,
      "ipcCode": "C30B35/00",
      "coreCode": "C30B35/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    },
    "C30B35/007": {
      "code": "C30B35/007",
      "level": 8,
      "parent": "C30B35/00",
      "ja": "｛結晶の成長に用いられる原材料の準備，前処理のための装置｝",
      "en": "{Apparatus for preparing, pre-treating the source material to be used for crystal growth}",
      "count": 374,
      "ipcCode": "C30B35/00",
      "coreCode": "C30B35/00",
      "note": "",
      "sourceFile": "cpc_C30B.txt",
      "kind": "cpc"
    }
  }
};

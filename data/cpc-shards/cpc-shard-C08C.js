window.CPC_SHARD_C08C = {
  "generatedAt": "2026-03-20T02:17:57.314Z",
  "total": 49,
  "entries": {
    "C08C": {
      "code": "C08C",
      "level": 5,
      "parent": "C08",
      "ja": "　ゴムの処理または化学的変性｛特記．以下のＩＰＣグループはＥＣＬＡ分類表では使用されない。これらのグループに包含される主題事項は，以下の各ＥＣＬＡグループに分類する:Ｃ０８Ｃ１／１６(ＥＣＬＡ)　は　Ｃ０８Ｃ１／１４に包含される。｝注．　このサブクラスは以下のものを含む:　－　天然ゴムまたは共役ジエンゴムを対象としたプロセス　（その合成　Ｃ０８Ｆ）　－　ゴム一般を対象としたプロセス　（上記（ａ）に含まれない特定のゴムを対象としたもの，Ｃ０８ＦからＣ０８Ｈ）",
      "en": "TREATMENT OR CHEMICAL MODIFICATION OF RUBBERS<br><br><u>NOTE</u><br><br>This subclass covers: <br>processes directed to natural rubber or to conjugated diene rubbers <br><br>processes directed to rubbers in general. ",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C08": {
      "code": "C08",
      "level": 4,
      "parent": "C",
      "ja": "有機高分子化合物；その製造または化学的加工；それに基づく組成物<br>（１）このクラスは下記の高分子化合物それ自体は包含しない:<br>－ペプチド，例．タンパク質，はサブクラスＣ０７Ｋに包含される。<br>－ヌクレオシド基，例．核酸，の糖類基でリンクされたリン酸塩またはポリリン酸塩基を別個に持つ化合物で，２個以上のモノヌクレオチドユニットを含むものは，グループＣ０７Ｈ２１／００に包含される。<br>－遺伝子工学に関するＤＮＡまたはＲＮＡ，ベクター，例．プラスミド，またはその分離，製造または精製はグループＣ１２Ｎ１５／００に包含される。<br>（２）化合物または組成物からなる殺生物剤，有害生物忌避剤，有害生物誘引剤または植物生長調節剤は，サブクラスＡ０１Ｐにも分類される。",
      "en": "ORGANIC MACROMOLECULAR COMPOUNDS; THEIR PREPARATION OR CHEMICAL WORKING-UP; COMPOSITIONS BASED THEREON",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_C.txt",
      "kind": "cpc"
    },
    "C": {
      "code": "C",
      "level": 2,
      "parent": null,
      "ja": "化学；冶金<br>（１）セクションＣにおいて，化学元素の種類に関する定義は次の通りである:<br>アルカリ金属:Ｌｉ，Ｎａ，Ｋ，Ｒｂ，Ｃｓ，Ｆｒ<br>アルカリ土類金属:Ｃａ，Ｓｒ，Ｂａ，Ｒａ<br>ランタニド:原子番号５７から７１までの元素<br>希土類:Ｓｃ，Ｙ，ランタニド<br>アクチニド:原子番号８９から１０３までの元素<br>耐火金属:Ｔｉ，Ｖ，Ｃｒ，Ｚｒ，Ｎｂ，Ｍｏ，Ｈｆ，Ｔａ，Ｗ<br>ハロゲン:Ｆ，Ｃｌ，Ｂｒ，Ｉ，Ａｔ<br>希ガス:Ｈｅ，Ｎｅ，Ａｒ，Ｋｒ，Ｘｅ，Ｒｎ<br>白金族:Ｏｓ，Ｉｒ，Ｐｔ，Ｒｕ，Ｒｈ，Ｐｄ<br>貴金属:Ａｇ，Ａｕ，白金族<br>軽金属:アルカリ金属，アルカリ土類金属，Ｂｅ，Ａｌ，Ｍｇ<br>重金属:軽金属以外の金属<br>鉄族:Ｆｅ，Ｃｏ，Ｎｉ<br>非金属:Ｈ，Ｂ，Ｃ，Ｓｉ，Ｎ，Ｐ，Ｏ，Ｓ，Ｓｅ，Ｔｅ，希ガス，ハロゲン<br>金属:非金属以外の元素<br>遷移元素:原子番号２１から３０まで，３９から４８まで，５７から８０まで，８９以上の元素<br>（２）Ｃセクションは以下のものを包含する:<br>（ａ）無機化合物，有機化合物，高分子化合物およびそれらの調製法を包含する純粋化学；<br>（ｂ）たとえばガラス，セラミック，肥料，プラスチック組成物，ペイント，石油工業の製品のような上記化合物を含有する組成物を包含する応用化学。それはまた，爆発物，染料，接着剤，潤滑剤および洗剤のように，ある目的に適した特別の性質をもつための組成物をも包含する；<br>（ｃ）コークスの製造，固体または気体燃料の製造，油脂およびろうの製造および精製，発酵工業（例．ビールおよびぶどう酒の製造），製糖工業のようなある境界工業；<br>（ｄ）ある操作または処理であって，純粋に機械的な処理，例．レザーおよび皮の機械的処理，または部分的機械的処理，例．水処理，または腐食防止一般であるもの；<br>（ｅ）冶金，鉄または非鉄合金<br>（３）ＩＰＣの全てのセクションにおいて，相反する指示がない限り、元素記号周期律表への言及は、以下に示す８つのグループを表す。例えば、グループＣ０７Ｆ３／００「周期律表の第２族の元素を含有する化合物」は、ＩＩＡ及びＩＩＢ族の元素への言及とする。<br>「図」<br>（４）（ａ）化学的部分または面および非化学的部分または面の両方を有する操作，処理，製品または物品の場合に，一般規則として，化学的部分または面はＣセクションに包含される。<br>（ｂ）これらのうちのある場合には，化学的部分または面には非化学的部分または面を伴い，純粋に機械的部分または面を伴うことさえある，というのは非化学的面は操作または処理にとって本質的であるかまたはそれらの重要な要素を構成するからである；実際，全体のさまざまな部分または面を分割しないのがより論理的と思える。これは応用化学や上記注（１）（ｃ），（ｄ）および（ｅ）の工業，操作および処理の場合に適用される。例えば，ガラス製造用特殊炉はＣ０３に包含されＦ２７には包含されない。<br>（ｃ）しかしながら，機械的面（または非化学的面）に非化学的な面が伴うという例外もある，例えば:<br>―サブクラスＡ６１Ｋにおけるある抽出プロセス；<br>―サブクラスＡ６１Ｌにおける空気の化学的精製；<br>―サブクラスＡ６２Ｄにおける化学的消火方法；<br>―クラスＢ０１における化学的プロセスおよび装置；<br>―サブクラスＢ２７Ｋにおける木材の含浸；<br>―サブクラスＧ０１Ｎにおける化学的分析または試験法；<br>―クラスＧ０３における写真材料またはプロセス，そして一般に，Ｄセクションにおける繊維の化学的処理および繊維素，紙等の製造<br>（ｄ）さらに他の場合，純粋化学的面はＣセクションに包含され，応用化学的面はＡ，Ｂ，Ｆセクションのような他のセクションに包含される，例．以下の物質または組成物の使用:<br>―サブクラスＡ０１Ｎに包含される植物または動物の処理；<br>―クラスＡ２３に包含される食物；<br>―クラスＦ４２に包含される弾薬または爆薬<br>（ｅ）化学的面および機械的面が非常に交錯して，簡単明確な分割ができない時，または化学的処理が自然なまたは論理的な延長として機械的プロセスを要する時は，化学的面に加えて，Ｃセクションは機械的面の一部のみ，例．クラスＣ０４に包含される人造石の後処理，を包含する。この後者の場合に，たとえその区別がむしろ任意であってもその位置を明確にするよう注または参照を通常設けている。",
      "en": "CHEMISTRY; METALLURGY<br><br><u>NOTE</u><br><br>In section C, the definitions of groups of chemical elements are as follows: <br>Alkali metals: Li, Na, K, Rb, Cs, Fr <br>Alkaline earth metals: Ca, Sr, Ba, Ra<br>Lanthanides: elements with atomic numbers 57 to 71 inclusive <br>Rare earths: Sc, Y, Lanthanides<br>Actinides: elements with atomic numbers 89 to 103 inclusive<br>Refractory metals: Ti, V, Cr, Zr, Nb, Mo, Hf, Ta, W <br>Halogens: F, Cl, Br, I, At <br>Noble gases: He, Ne, Ar, Kr, Xe, Rn <br>Platinum group: Os, Ir, Pt, Ru, Rh, Pd <br>Noble metals: Ag, Au, Platinum group <br>Light metals: alkali metals, alkaline earth metals, Be, Al, Mg<br>Heavy metals: metals other than light metals<br>Iron group: Fe, Co, Ni<br>Non-metals: H, B, C, Si, N, P, O, S, Se, Te, noble gases, halogens<br>Metals: elements other than non-metals <br>Transition elements: elements with atomic numbers 21 to 30 inclusive, 39 to 48 inclusive, 57 to 80 inclusive, 89 upwards<br>Section Ccovers:<br>pure chemistry, which covers inorganic compounds, organic compounds, macromolecular compounds, and their methods of preparation;<br>applied chemistry, which covers compositions containing the above compounds, such as: glass, ceramics, fertilisers, plastics compositions, paints, products of the petroleum industry. It also covers certain compositions on account of their having particular properties rendering them suitable for certain purposes, as in the case of explosives, dyestuffs, adhesives, lubricants and detergents;<br>certain marginal industries, such as the manufacture of coke and of solid or gaseous fuels, the production and refining of oils, fats and waxes, the fermentation industry (e.g., brewing and wine-making), the sugar industry;<br>certain operations or treatments, which are either purely mechanical, e.g., the mechanical treatment of leather and skins, or partly mechanical, e.g., the treatment of water or the prevention of corrosion in general;<br>metallurgy, ferrous or non-ferrous alloys.<br>In all sections of the CPC, in the absence of an indication to the contrary, the Periodic Table of chemical elements referred to is the one with 18 groups as represented in the table below.<br><br><br>In the case of operations, treatments, products or articles having both a chemical and a non-chemical part or aspect, the general rule is that the chemical part or aspect is covered by section C.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Notes (2) c), d) and e). For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example:<br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g. the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.<br>In some of these cases, the chemical part or aspect brings with it a non-chemical one, even though purely mechanical, because this latter aspect either is essential to the operation or treatment or constitutes an important element thereof. It has seemed, in fact, more logical not to dissociate the different parts or aspects of a coherent whole. This is the case for applied chemistry and for the industries, operations and treatments mentioned in Note (2) bullets 2, 3, 4 and 5. For example, furnaces peculiar to the manufacture of glass are covered by class C03 and not by class F27.<br>There are, however, some exceptions in which the mechanical (or non-chemical) aspect carries with it the chemical aspect, for example: <br>certain extractive processes, in subclass A61K;<br>the chemical purification of air, in subclass A61L;<br>chemical methods of fire-fighting, in subclass A62D;<br>chemical processes and apparatus, in class B01;<br>impregnation of wood, in subclass B27K;<br>chemical methods of analysis or testing, in subclass G01N;<br>photographic materials and processes, in class G03, and, generally, the chemical treatment of textiles and the production of cellulose or paper, in section D.<br>In still other cases, the pure chemical aspect is covered by section C and the applied chemical aspect by another section, such as A, B or F, e.g., the use of a substance or composition for:<br>treatment of plants or animals, covered by subclass A01N;<br>foodstuffs, covered by class A23;<br>ammunition or explosives, covered by class F42.<br>When the chemical and mechanical aspects are so closely interlocked that a neat and simple division is not possible, or when certain mechanical processes follow as a natural or logical continuation of a chemical treatment, section C may cover, in addition to the chemical aspect, a part only of the mechanical aspect, e.g., after-treatment of artificial stone, covered by class C04. In this latter case, a note or a reference is usually given to make the position clear, even if sometimes the division is rather arbitrary.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "C08C1/00": {
      "code": "C08C1/00",
      "level": 7,
      "parent": null,
      "ja": "ゴムラテックスの処理",
      "en": "Treatment of rubber latex",
      "count": 109,
      "ipcCode": "C08C1/00",
      "coreCode": "C08C1/00",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/02": {
      "code": "C08C1/02",
      "level": 8,
      "parent": "C08C1/00",
      "ja": "濃縮前または中のゴムラテックスの化学的または物理的処理",
      "en": "Chemical or physical treatment of rubber latex before or during concentration",
      "count": 190,
      "ipcCode": "C08C1/02",
      "coreCode": "C08C1/02",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/04": {
      "code": "C08C1/04",
      "level": 9,
      "parent": "C08C1/02",
      "ja": "精製；脱たんぱく",
      "en": "Purifying; Deproteinising",
      "count": 335,
      "ipcCode": "C08C1/04",
      "coreCode": "C08C1/04",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/06": {
      "code": "C08C1/06",
      "level": 9,
      "parent": "C08C1/02",
      "ja": "ゴムラテックスの保存（保存成分Ｃ０８Ｋ）",
      "en": "Preservation of rubber latex",
      "count": 138,
      "ipcCode": "C08C1/06",
      "coreCode": "C08C1/06",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/065": {
      "code": "C08C1/065",
      "level": 9,
      "parent": "C08C1/02",
      "ja": "分散ゴム粒子径の増加",
      "en": "Increasing the size of dispersed rubber particles",
      "count": 105,
      "ipcCode": "C08C1/065",
      "coreCode": "C08C1/065",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/07": {
      "code": "C08C1/07",
      "level": 10,
      "parent": "C08C1/065",
      "ja": "使用凝集剤に特徴のあるもの",
      "en": "characterised by the agglomerating agents used",
      "count": 124,
      "ipcCode": "C08C1/07",
      "coreCode": "C08C1/07",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/075": {
      "code": "C08C1/075",
      "level": 9,
      "parent": "C08C1/02",
      "ja": "濃縮",
      "en": "Concentrating",
      "count": 77,
      "ipcCode": "C08C1/075",
      "coreCode": "C08C1/075",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/08": {
      "code": "C08C1/08",
      "level": 10,
      "parent": "C08C1/075",
      "ja": "クリーミング剤の使用による",
      "en": "with the aid of creaming agents",
      "count": 138,
      "ipcCode": "C08C1/08",
      "coreCode": "C08C1/08",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/10": {
      "code": "C08C1/10",
      "level": 10,
      "parent": "C08C1/075",
      "ja": "遠心分離による",
      "en": "by centrifugation",
      "count": 98,
      "ipcCode": "C08C1/10",
      "coreCode": "C08C1/10",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/12": {
      "code": "C08C1/12",
      "level": 10,
      "parent": "C08C1/075",
      "ja": "蒸発による",
      "en": "by evaporation",
      "count": 120,
      "ipcCode": "C08C1/12",
      "coreCode": "C08C1/12",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/14": {
      "code": "C08C1/14",
      "level": 8,
      "parent": "C08C1/00",
      "ja": "凝固",
      "en": "Coagulation<br><br><u>WARNING</u><br>Group C08C1/14 is impacted by reclassification into group C08C1/16.<br>Groups C08C1/14 and C08C1/16 should be considered in order to perform a complete search.",
      "count": 517,
      "ipcCode": "C08C1/14",
      "coreCode": "C08C1/14",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/145": {
      "code": "C08C1/145",
      "level": 9,
      "parent": "C08C1/14",
      "ja": "｛感熱剤｝",
      "en": "{Heat-sensitising agents}",
      "count": 84,
      "ipcCode": "C08C1/14",
      "coreCode": "C08C1/14",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/15": {
      "code": "C08C1/15",
      "level": 9,
      "parent": "C08C1/14",
      "ja": "使用する凝集剤に特徴のあるもの",
      "en": "characterised by the coagulants used",
      "count": 373,
      "ipcCode": "C08C1/15",
      "coreCode": "C08C1/15",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C1/16": {
      "code": "C08C1/16",
      "level": 9,
      "parent": "C08C1/14",
      "ja": "",
      "en": "in floc form<br><br><u>WARNING</u><br>Group C08C1/16 is incomplete pending reclassification of documents from group C08C1/14.<br>Groups C08C1/14 and C08C1/16 should be considered in order to perform a complete search.",
      "count": 118,
      "ipcCode": "C08C1/16",
      "coreCode": "C08C1/16",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C2/00": {
      "code": "C08C2/00",
      "level": 7,
      "parent": null,
      "ja": "ゴム溶液の処理",
      "en": "Treatment of rubber solutions",
      "count": 72,
      "ipcCode": "C08C2/00",
      "coreCode": "C08C2/00",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C2/02": {
      "code": "C08C2/02",
      "level": 8,
      "parent": "C08C2/00",
      "ja": "精製",
      "en": "Purification",
      "count": 75,
      "ipcCode": "C08C2/02",
      "coreCode": "C08C2/02",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C2/04": {
      "code": "C08C2/04",
      "level": 9,
      "parent": "C08C2/02",
      "ja": "触媒残渣の除去",
      "en": "Removal of catalyst residues",
      "count": 157,
      "ipcCode": "C08C2/04",
      "coreCode": "C08C2/04",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C2/06": {
      "code": "C08C2/06",
      "level": 8,
      "parent": "C08C2/00",
      "ja": "溶液からのゴムの採取",
      "en": "Winning of rubber from solutions",
      "count": 258,
      "ipcCode": "C08C2/06",
      "coreCode": "C08C2/06",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C3/00": {
      "code": "C08C3/00",
      "level": 7,
      "parent": null,
      "ja": "凝集ゴムの処理",
      "en": "Treatment of coagulated rubber",
      "count": 141,
      "ipcCode": "C08C3/00",
      "coreCode": "C08C3/00",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C3/02": {
      "code": "C08C3/02",
      "level": 8,
      "parent": "C08C3/00",
      "ja": "精製",
      "en": "Purification",
      "count": 140,
      "ipcCode": "C08C3/02",
      "coreCode": "C08C3/02",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C4/00": {
      "code": "C08C4/00",
      "level": 7,
      "parent": null,
      "ja": "グループＣ０８Ｃ１／００　から　Ｃ０８Ｃ３／０２に含まれない加硫前のゴム処理",
      "en": "Treatment of rubber before vulcanisation, not provided for in groups C08C1/00&#160;-&#160;C08C3/02",
      "count": 97,
      "ipcCode": "C08C4/00",
      "coreCode": "C08C4/00",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/00": {
      "code": "C08C19/00",
      "level": 7,
      "parent": null,
      "ja": "ゴムの化学的変性　（架橋剤，　Ｃ０８Ｃ１９／３０に含まれないもの，　Ｃ０８Ｋ）注．　このグループにおいて，相反する指示がない限り，プロセスは最後の適切な箇所に分類する。",
      "en": "Chemical modification of rubber<br><br><u>NOTE</u><br><br>In this group the last place rule is not applied. A process is classified at all appropriate places.<br><br><u>WARNING</u><br>Group C08C19/00 is impacted by reclassification into group C08C19/085.<br>Groups C08C19/00 and C08C19/085 should be considered in order to perform a complete search.",
      "count": 734,
      "ipcCode": "C08C19/00",
      "coreCode": "C08C19/00",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/02": {
      "code": "C08C19/02",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "水素添加",
      "en": "Hydrogenation",
      "count": 941,
      "ipcCode": "C08C19/02",
      "coreCode": "C08C19/02",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/04": {
      "code": "C08C19/04",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "酸化",
      "en": "Oxidation",
      "count": 212,
      "ipcCode": "C08C19/04",
      "coreCode": "C08C19/04",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/06": {
      "code": "C08C19/06",
      "level": 9,
      "parent": "C08C19/04",
      "ja": "エポキシ化",
      "en": "Epoxidation",
      "count": 351,
      "ipcCode": "C08C19/06",
      "coreCode": "C08C19/06",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/08": {
      "code": "C08C19/08",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "解重合",
      "en": "Depolymerisation<br><br><u>WARNING</u><br>Group C08C19/08 is impacted by reclassification into group C08C19/085.<br>Groups C08C19/08 and C08C19/085 should be considered in order to perform a complete search.",
      "count": 366,
      "ipcCode": "C08C19/08",
      "coreCode": "C08C19/08",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/085": {
      "code": "C08C19/085",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "",
      "en": "{Devulcanisation}<br><br><u>WARNING</u><br>Group C08C19/085 is incomplete pending reclassification of documents from groups C08C19/00 and C08C19/08.<br>All groups listed in this Warning should be considered in order to perform a complete search.",
      "count": 79,
      "ipcCode": "C08C19/00",
      "coreCode": "C08C19/08",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/10": {
      "code": "C08C19/10",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "異性化；環化",
      "en": "Isomerisation; Cyclisation",
      "count": 174,
      "ipcCode": "C08C19/10",
      "coreCode": "C08C19/10",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/12": {
      "code": "C08C19/12",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "分子内へのハロゲン原子の導入",
      "en": "Incorporating halogen atoms into the molecule",
      "count": 191,
      "ipcCode": "C08C19/12",
      "coreCode": "C08C19/12",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/14": {
      "code": "C08C19/14",
      "level": 9,
      "parent": "C08C19/12",
      "ja": "ハロゲンとの反応による",
      "en": "by reaction with halogens",
      "count": 302,
      "ipcCode": "C08C19/14",
      "coreCode": "C08C19/14",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/16": {
      "code": "C08C19/16",
      "level": 9,
      "parent": "C08C19/12",
      "ja": "ハロゲン化水素との反応による",
      "en": "by reaction with hydrogen halides",
      "count": 101,
      "ipcCode": "C08C19/16",
      "coreCode": "C08C19/16",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/18": {
      "code": "C08C19/18",
      "level": 9,
      "parent": "C08C19/12",
      "ja": "ハロゲン化炭化水素との反応による",
      "en": "by reaction with hydrocarbons substituted by halogen",
      "count": 57,
      "ipcCode": "C08C19/18",
      "coreCode": "C08C19/18",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/20": {
      "code": "C08C19/20",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "分子内へのいおう原子の導入",
      "en": "Incorporating sulfur atoms into the molecule",
      "count": 500,
      "ipcCode": "C08C19/20",
      "coreCode": "C08C19/20",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/22": {
      "code": "C08C19/22",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "分子内への窒素原子の導入",
      "en": "Incorporating nitrogen atoms into the molecule",
      "count": 1039,
      "ipcCode": "C08C19/22",
      "coreCode": "C08C19/22",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/24": {
      "code": "C08C19/24",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "分子内へのりん原子の導入",
      "en": "Incorporating phosphorus atoms into the molecule",
      "count": 102,
      "ipcCode": "C08C19/24",
      "coreCode": "C08C19/24",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/25": {
      "code": "C08C19/25",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "分子内へのけい素原子の導入",
      "en": "Incorporating silicon atoms into the molecule",
      "count": 997,
      "ipcCode": "C08C19/25",
      "coreCode": "C08C19/25",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/26": {
      "code": "C08C19/26",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "分子内への金属原子の導入",
      "en": "Incorporating metal atoms into the molecule",
      "count": 155,
      "ipcCode": "C08C19/26",
      "coreCode": "C08C19/26",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/28": {
      "code": "C08C19/28",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "炭素―炭素不飽和結合をもつ化合物との反応（グラフト重合体Ｃ０８Ｆ）",
      "en": "Reaction with compounds containing carbon-to-carbon unsaturated bonds (graft polymers C08F279/00)",
      "count": 485,
      "ipcCode": "C08C19/28",
      "coreCode": "C08C19/28",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/30": {
      "code": "C08C19/30",
      "level": 8,
      "parent": "C08C19/00",
      "ja": "高分子の異種原子または異種原子含有基と反応する試薬の付加",
      "en": "Addition of a reagent which reacts with a hetero atom or a group containing hetero atoms of the macromolecule",
      "count": 234,
      "ipcCode": "C08C19/30",
      "coreCode": "C08C19/30",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/32": {
      "code": "C08C19/32",
      "level": 9,
      "parent": "C08C19/30",
      "ja": "ハロゲンまたはハロゲン含有基との反応",
      "en": "reacting with halogens or halogen-containing groups",
      "count": 64,
      "ipcCode": "C08C19/32",
      "coreCode": "C08C19/32",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/34": {
      "code": "C08C19/34",
      "level": 9,
      "parent": "C08C19/30",
      "ja": "酸素または酸素含有基との反応",
      "en": "reacting with oxygen or oxygen-containing groups",
      "count": 82,
      "ipcCode": "C08C19/34",
      "coreCode": "C08C19/34",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/36": {
      "code": "C08C19/36",
      "level": 10,
      "parent": "C08C19/34",
      "ja": "カルボキシ基との",
      "en": "with carboxy radicals",
      "count": 254,
      "ipcCode": "C08C19/36",
      "coreCode": "C08C19/36",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/38": {
      "code": "C08C19/38",
      "level": 10,
      "parent": "C08C19/34",
      "ja": "ヒドロキシ基との",
      "en": "with hydroxy radicals",
      "count": 143,
      "ipcCode": "C08C19/38",
      "coreCode": "C08C19/38",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/40": {
      "code": "C08C19/40",
      "level": 10,
      "parent": "C08C19/34",
      "ja": "エポキシ基との",
      "en": "with epoxy radicals",
      "count": 135,
      "ipcCode": "C08C19/40",
      "coreCode": "C08C19/40",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/42": {
      "code": "C08C19/42",
      "level": 9,
      "parent": "C08C19/30",
      "ja": "金属または金属含有基との反応",
      "en": "reacting with metals or metal-containing groups",
      "count": 101,
      "ipcCode": "C08C19/42",
      "coreCode": "C08C19/42",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    },
    "C08C19/44": {
      "code": "C08C19/44",
      "level": 10,
      "parent": "C08C19/42",
      "ja": "分子鎖の一端または両端においてのみ金属原子をもつポリマーの",
      "en": "of polymers containing metal atoms exclusively at one or both ends of the skeleton",
      "count": 1423,
      "ipcCode": "C08C19/44",
      "coreCode": "C08C19/44",
      "note": "",
      "sourceFile": "cpc_C08C.txt",
      "kind": "cpc"
    }
  }
};

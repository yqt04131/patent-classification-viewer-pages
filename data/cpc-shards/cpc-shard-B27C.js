window.CPC_SHARD_B27C = {
  "generatedAt": "2026-03-20T02:17:56.525Z",
  "total": 36,
  "entries": {
    "B27C": {
      "code": "B27C",
      "level": 5,
      "parent": "B27",
      "ja": "平削り，穴あけ，フライス削り，旋削，または万能加工機械（工作機械一般Ｂ２３；砥粒を用いる木材加工，例．研削紙布加工，装置Ｂ２４；それらの目的のための工具Ｂ２７Ｇ）",
      "en": "PLANING, DRILLING, MILLING, TURNING OR UNIVERSAL MACHINES FOR WOOD OR SIMILAR MATERIAL (machine tools in general B23; working wood using abrasive, e.g. sanding, devices B24; tools for these purposes B27G)<br><br><u>WARNING</u><br><br>In this subclass non-limiting references (in the sense of paragraph 39 of the Guide to the IPC) may still be displayed in the scheme.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B27": {
      "code": "B27",
      "level": 4,
      "parent": "B",
      "ja": "木材または類似の材料の加工または保存；釘打ち機またはステープル打ち機一般",
      "en": "WORKING OR PRESERVING WOOD OR SIMILAR MATERIAL; NAILING OR STAPLING MACHINES IN GENERAL",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B": {
      "code": "B",
      "level": 2,
      "parent": null,
      "ja": "処理操作；運輸",
      "en": "PERFORMING OPERATIONS; TRANSPORTING<br><br><u>NOTE</u><br><br>The following notes are meant to assist in the use of classes B01&#160;-&#160;B09; they must not be read as modifying in any way the elaborations.<br>In this sub-section, the separation of different materials, e.g. of different matter, size, or state, is predominantly found in the following subclasses: <br>B01D<br>B03B, B03C, B03D<br>B04B, B04C<br>B07B, B07C<br>The classifying characteristics of these subclasses are: <br>the physical state of the matter to be separated <br>the principle of the process used <br>particular kinds of apparatus The first of these characteristics involves six different aspects, assembled in three groups.<br>liquid/liquid or liquid/gas and gas/gas <br>solid/liquid or solid/gas <br>solid/solid<br>Theses subclasses are to be used according to the following general rules: <br>B01D is the most general class as far as separation other  than solids from solids is concerned. <br>Apparatus for separating solids from solids are covered by B03B when the process concerned is regarded as the equivalent of \"washing\" in the sense of the mining art, even if such apparatus is a pneumatic one, especially pneumatic tables or jigs. Screens PER SE are not covered by this subclass but are classified in B07B, even if they are being used in a wet process. All other apparatus for the separation of solids from solids according to dry methods are classified in B07B. <br>If the separation takes place as a result of the detection or measurement of some feature of the material or articles to be sorted it is classified in B07C. <br>It should also be noted that the separation of isotopes of the same chemical element is covered by B01D59/00, whatever process or apparatus is employed.<br>The following scheme illustrates the classification according to these rules. <br>LIQUID/LIQUID <br>      LIQUID/GAS <br>      GAS/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force, using centrifuges <br>    or free-vortex apparatus                   B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br>SOLID/LIQUID <br>      SOLID/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force                       B01DB01D<br>       using centrifuges or free-vortex <br>       apparatus                               B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br><br>SOLID/SOLID <br>  _____________________________________________________________________ <br>  Dry Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General for material in bulk                 B07BB07B<br>  Individual sorting                           B07CB07C<br>     Screening, sifting, pneumatic sorting     B07BB07B<br>       using pneumatic tables or jigs          B03BB03B<br>     by magnetic or electrostatic effect       B03CB03C<br>     by centrifugal force                      B07BB07B<br>       using centrifuges or free-vortex <br>       apparatus                               B07BB04B, B04C<br>  _____________________________________________________________________ <br>  Wet Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B03BB03B<br>     flotation, differential sedimentation     B03DB03D<br>     screening                                 B07BB07B<br>  _____________________________________________________________________ <br>  Combinations = dry methods - wet methods: B03B<br>  _____________________________________________________________________<br>",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "B27C1/00": {
      "code": "B27C1/00",
      "level": 7,
      "parent": null,
      "ja": "平らな表面を作成するための機械，例．回転カッターによるもの；そのための装備",
      "en": "Machines for producing flat surfaces, e.g. by rotary cutters; Equipment therefor",
      "count": 456,
      "ipcCode": "B27C1/00",
      "coreCode": "B27C1/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/002": {
      "code": "B27C1/002",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "｛非回転工具をもつもの｝",
      "en": "{with non-rotating tools}",
      "count": 204,
      "ipcCode": "B27C1/00",
      "coreCode": "B27C1/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/005": {
      "code": "B27C1/005",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "｛加工表面に垂直な回転ベクトルを有する工具をもつもの｝",
      "en": "{with tools having a rotational vector which is perpendicular to the working surface}",
      "count": 280,
      "ipcCode": "B27C1/00",
      "coreCode": "B27C1/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/007": {
      "code": "B27C1/007",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "｛切削により生じた表面に平行な回転ベクトルを有する工具で工作物を切り開くためのもの｝",
      "en": "{For cutting through a work-piece with a tool having a rotational vector which is parallel to the surfaces generated by cutting}",
      "count": 60,
      "ipcCode": "B27C1/00",
      "coreCode": "B27C1/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/02": {
      "code": "B27C1/02",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "平滑加工，すなわち，一側面のみ加工",
      "en": "Smoothing, i.e. working one side only",
      "count": 305,
      "ipcCode": "B27C1/02",
      "coreCode": "B27C1/02",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/04": {
      "code": "B27C1/04",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "一定厚さに削成する機械",
      "en": "Thicknessing machines",
      "count": 294,
      "ipcCode": "B27C1/04",
      "coreCode": "B27C1/04",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/06": {
      "code": "B27C1/06",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "平滑加工とその後に一定厚さに削成するための機械",
      "en": "Machines for smoothing and subsequent thicknessing",
      "count": 58,
      "ipcCode": "B27C1/06",
      "coreCode": "B27C1/06",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/08": {
      "code": "B27C1/08",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "工作物の数個の側面を同時に加工するための機械",
      "en": "Machines for working several sides of work simultaneously",
      "count": 335,
      "ipcCode": "B27C1/08",
      "coreCode": "B27C1/08",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/10": {
      "code": "B27C1/10",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "動力駆動のカッターブロックを備えた手持ちかんな削り機",
      "en": "Hand planes equipped with power-driven cutter blocks",
      "count": 396,
      "ipcCode": "B27C1/10",
      "coreCode": "B27C1/10",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/12": {
      "code": "B27C1/12",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "工作物を供給するための設備",
      "en": "Arrangements for feeding work",
      "count": 583,
      "ipcCode": "B27C1/12",
      "coreCode": "B27C1/12",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C1/14": {
      "code": "B27C1/14",
      "level": 8,
      "parent": "B27C1/00",
      "ja": "その他の細部または付属品",
      "en": "Other details or accessories",
      "count": 778,
      "ipcCode": "B27C1/14",
      "coreCode": "B27C1/14",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C3/00": {
      "code": "B27C3/00",
      "level": 7,
      "parent": null,
      "ja": "穴あけ機械または穴あけ装置；そのための装備（Ｂ２３Ｂが優先）",
      "en": "Drilling machines or drilling devices; Equipment therefor (B23B takes precedence)",
      "count": 613,
      "ipcCode": "B27C3/00",
      "coreCode": "B27C3/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C3/02": {
      "code": "B27C3/02",
      "level": 8,
      "parent": "B27C3/00",
      "ja": "単一の加工主軸をもつた定置穴あけ機械",
      "en": "Stationary drilling machines with a single working spindle",
      "count": 1099,
      "ipcCode": "B27C3/02",
      "coreCode": "B27C3/02",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C3/04": {
      "code": "B27C3/04",
      "level": 8,
      "parent": "B27C3/00",
      "ja": "複数の加工主軸をもった定置穴あけ機械",
      "en": "Stationary drilling machines with a plurality of working spindles",
      "count": 663,
      "ipcCode": "B27C3/04",
      "coreCode": "B27C3/04",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C3/06": {
      "code": "B27C3/06",
      "level": 8,
      "parent": "B27C3/00",
      "ja": "合くぎ穴を作るための穴あけ機械または装置",
      "en": "Drilling machines or devices for making dowel holes",
      "count": 208,
      "ipcCode": "B27C3/06",
      "coreCode": "B27C3/06",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C3/08": {
      "code": "B27C3/08",
      "level": 8,
      "parent": "B27C3/00",
      "ja": "作業者が保持する穴あけ機械または装置",
      "en": "Operator-supported drilling machines or devices",
      "count": 197,
      "ipcCode": "B27C3/08",
      "coreCode": "B27C3/08",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/00": {
      "code": "B27C5/00",
      "level": 7,
      "parent": null,
      "ja": "特殊な外形の作成または特殊に成形された工作物の作成のために設計された機械，例．回転カッターによるもの；そのための装備（旋削Ｂ２７Ｃ７／００；ならい装置の特色Ｂ２３Ｑ３５／００；立て削り，ほぞ穴削り，凸縁または溝の作成Ｂ２７Ｆ）",
      "en": "Machines designed for producing special profiles or shaped work, e.g. by rotary cutters; Equipment therefor (turning B27C7/00; features of copying devices B23Q35/00; slotting, mortising, making tongues or grooves B27F)",
      "count": 1679,
      "ipcCode": "B27C5/00",
      "coreCode": "B27C5/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/003": {
      "code": "B27C5/003",
      "level": 8,
      "parent": "B27C5/00",
      "ja": "｛ならいフライス削り｝",
      "en": "{copy milling}",
      "count": 131,
      "ipcCode": "B27C5/00",
      "coreCode": "B27C5/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/006": {
      "code": "B27C5/006",
      "level": 8,
      "parent": "B27C5/00",
      "ja": "｛非回転工具をもつもの｝",
      "en": "{with non-rotating tools}",
      "count": 384,
      "ipcCode": "B27C5/00",
      "coreCode": "B27C5/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/02": {
      "code": "B27C5/02",
      "level": 8,
      "parent": "B27C5/00",
      "ja": "テーブルをもった機械",
      "en": "Machines with table",
      "count": 3451,
      "ipcCode": "B27C5/02",
      "coreCode": "B27C5/02",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/04": {
      "code": "B27C5/04",
      "level": 9,
      "parent": "B27C5/02",
      "ja": "工作物のための案内部材",
      "en": "Guide fences for work",
      "count": 237,
      "ipcCode": "B27C5/04",
      "coreCode": "B27C5/04",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/06": {
      "code": "B27C5/06",
      "level": 9,
      "parent": "B27C5/02",
      "ja": "工作物のクランプまたは送りのための設備",
      "en": "Arrangements for clamping or feeding work",
      "count": 3083,
      "ipcCode": "B27C5/06",
      "coreCode": "B27C5/06",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/08": {
      "code": "B27C5/08",
      "level": 8,
      "parent": "B27C5/00",
      "ja": "丸面とり機",
      "en": "Rounding machines",
      "count": 351,
      "ipcCode": "B27C5/08",
      "coreCode": "B27C5/08",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C5/10": {
      "code": "B27C5/10",
      "level": 8,
      "parent": "B27C5/00",
      "ja": "可搬式手動木工フライス盤；ルーター",
      "en": "Portable hand-operated wood-milling machines; Routers",
      "count": 960,
      "ipcCode": "B27C5/10",
      "coreCode": "B27C5/10",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C7/00": {
      "code": "B27C7/00",
      "level": 7,
      "parent": null,
      "ja": "木工旋削機械；そのための装備（Ｂ２３Ｂ，Ｂ２３Ｇ，Ｂ２３Ｑが優先；ならい装置の特徴Ｂ２３Ｑ３５／００）｛注．このグループにおいては，＋ＩＤＴという表現は以下の内容を意味する。　－ここに分類される文献は，木工旋削機械に関連するＩＤＴ－グループに分類されていた:　－これらは管理上の目的で上述のＩＤＴグループからこのグループに再分類された｝",
      "en": "Wood-turning machines; Equipment therefor (B23B, B23G, B23Q, take precedence; features of copying devices B23Q35/00)",
      "count": 539,
      "ipcCode": "B27C7/00",
      "coreCode": "B27C7/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C7/005": {
      "code": "B27C7/005",
      "level": 8,
      "parent": "B27C7/00",
      "ja": "｛回転工具によるもの｝",
      "en": "{by means of a rotating tool}",
      "count": 400,
      "ipcCode": "B27C7/00",
      "coreCode": "B27C7/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C7/02": {
      "code": "B27C7/02",
      "level": 8,
      "parent": "B27C7/00",
      "ja": "手廻し用の旋盤",
      "en": "Lathes for hand turning",
      "count": 44,
      "ipcCode": "B27C7/02",
      "coreCode": "B27C7/02",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C7/04": {
      "code": "B27C7/04",
      "level": 8,
      "parent": "B27C7/00",
      "ja": "工作物の心出しまたはチャック用の装置",
      "en": "Devices for centering or chucking work",
      "count": 212,
      "ipcCode": "B27C7/04",
      "coreCode": "B27C7/04",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C7/06": {
      "code": "B27C7/06",
      "level": 8,
      "parent": "B27C7/00",
      "ja": "工具の案内または支持用の設備，例．刃物台",
      "en": "Arrangements for guiding or supporting the tool, e.g. tool rests",
      "count": 233,
      "ipcCode": "B27C7/06",
      "coreCode": "B27C7/06",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C9/00": {
      "code": "B27C9/00",
      "level": 7,
      "parent": null,
      "ja": "多目的加工機械；万能加工機械；そのための装備",
      "en": "Multi-purpose machines; Universal machines; Equipment therefor",
      "count": 650,
      "ipcCode": "B27C9/00",
      "coreCode": "B27C9/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C9/005": {
      "code": "B27C9/005",
      "level": 8,
      "parent": "B27C9/00",
      "ja": "｛可搬式のもの｝",
      "en": "{portable}",
      "count": 119,
      "ipcCode": "B27C9/00",
      "coreCode": "B27C9/00",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C9/02": {
      "code": "B27C9/02",
      "level": 8,
      "parent": "B27C9/00",
      "ja": "単一の加工主軸をもつもの",
      "en": "with a single working spindle",
      "count": 628,
      "ipcCode": "B27C9/02",
      "coreCode": "B27C9/02",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    },
    "B27C9/04": {
      "code": "B27C9/04",
      "level": 8,
      "parent": "B27C9/00",
      "ja": "複数の加工主軸をもつもの",
      "en": "with a plurality of working spindles",
      "count": 1147,
      "ipcCode": "B27C9/04",
      "coreCode": "B27C9/04",
      "note": "",
      "sourceFile": "cpc_B27C.txt",
      "kind": "cpc"
    }
  }
};

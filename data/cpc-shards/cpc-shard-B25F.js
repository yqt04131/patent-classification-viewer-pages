window.CPC_SHARD_B25F = {
  "generatedAt": "2026-03-20T02:17:56.496Z",
  "total": 25,
  "entries": {
    "B25F": {
      "code": "B25F",
      "level": 5,
      "parent": "B25",
      "ja": "他に分類されない組合せ工具または多目的工具；作業に特に関連せずかつ他に分類されない携帯用動力駆動工具の細部または要素｛（単一の他のサブクラスに分類されない異なる金属加工作業を特別に組合せたものを行う機械または機械設備Ｂ２３Ｐ２３／００およびサブグループ）｝注．このサブクラスは，明らかな主機能と同時に単数または複数の二次機能をもつ工具を包含しない。それらの工具は，主機能をもつ工具としての関連サブクラスに包含され，このサブクラスのグループＢ２５Ｆ１／００またはＢ２５Ｆ３／００に分類されない。",
      "en": "COMBINATION OR MULTI-PURPOSE TOOLS NOT OTHERWISE PROVIDED FOR; DETAILS OR COMPONENTS OF PORTABLE POWER-DRIVEN TOOLS NOT PARTICULARLY RELATED TO THE OPERATIONS PERFORMED AND NOT OTHERWISE PROVIDED FOR {(machines or arrangements of machines for performing specified combinations of different metal-working operations not covered by a single other subclass B23P23/00 and subgroups)}<br><br><u>NOTE</u><br><br>This subclass does not cover tools having a clear primary function as well as one or more secondary functions. Those tools are covered by the relevant subclass for tools having such primary function and are, thus, not classified in group B25F1/00 or B25F3/00 of this subclass.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B25": {
      "code": "B25",
      "level": 4,
      "parent": "B",
      "ja": "手工具；可搬型動力工具；手工具用の柄；作業場設備；マニプレータ<br>このクラスにおいては，下記の用語は以下に示す意味で用いる:<br>―可搬型は人手で取扱い易いようにするための，例．組立てラインにそって使用されるばねで吊り下げられた可搬型の装置に関連した，吊り下げ具を含む。",
      "en": "HAND TOOLS; PORTABLE POWER-DRIVEN TOOLS; MANIPULATORS<br><br><u>NOTE</u><br><br>The work \"portable\" is to be understood as including suspension for easy manual handling, e.g. in connection with springsuspended portable apparatus for use along assembly lines",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B": {
      "code": "B",
      "level": 2,
      "parent": null,
      "ja": "処理操作；運輸",
      "en": "PERFORMING OPERATIONS; TRANSPORTING<br><br><u>NOTE</u><br><br>The following notes are meant to assist in the use of classes B01&#160;-&#160;B09; they must not be read as modifying in any way the elaborations.<br>In this sub-section, the separation of different materials, e.g. of different matter, size, or state, is predominantly found in the following subclasses: <br>B01D<br>B03B, B03C, B03D<br>B04B, B04C<br>B07B, B07C<br>The classifying characteristics of these subclasses are: <br>the physical state of the matter to be separated <br>the principle of the process used <br>particular kinds of apparatus The first of these characteristics involves six different aspects, assembled in three groups.<br>liquid/liquid or liquid/gas and gas/gas <br>solid/liquid or solid/gas <br>solid/solid<br>Theses subclasses are to be used according to the following general rules: <br>B01D is the most general class as far as separation other  than solids from solids is concerned. <br>Apparatus for separating solids from solids are covered by B03B when the process concerned is regarded as the equivalent of \"washing\" in the sense of the mining art, even if such apparatus is a pneumatic one, especially pneumatic tables or jigs. Screens PER SE are not covered by this subclass but are classified in B07B, even if they are being used in a wet process. All other apparatus for the separation of solids from solids according to dry methods are classified in B07B. <br>If the separation takes place as a result of the detection or measurement of some feature of the material or articles to be sorted it is classified in B07C. <br>It should also be noted that the separation of isotopes of the same chemical element is covered by B01D59/00, whatever process or apparatus is employed.<br>The following scheme illustrates the classification according to these rules. <br>LIQUID/LIQUID <br>      LIQUID/GAS <br>      GAS/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force, using centrifuges <br>    or free-vortex apparatus                   B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br>SOLID/LIQUID <br>      SOLID/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force                       B01DB01D<br>       using centrifuges or free-vortex <br>       apparatus                               B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br><br>SOLID/SOLID <br>  _____________________________________________________________________ <br>  Dry Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General for material in bulk                 B07BB07B<br>  Individual sorting                           B07CB07C<br>     Screening, sifting, pneumatic sorting     B07BB07B<br>       using pneumatic tables or jigs          B03BB03B<br>     by magnetic or electrostatic effect       B03CB03C<br>     by centrifugal force                      B07BB07B<br>       using centrifuges or free-vortex <br>       apparatus                               B07BB04B, B04C<br>  _____________________________________________________________________ <br>  Wet Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B03BB03B<br>     flotation, differential sedimentation     B03DB03D<br>     screening                                 B07BB07B<br>  _____________________________________________________________________ <br>  Combinations = dry methods - wet methods: B03B<br>  _____________________________________________________________________<br>",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "B25F1/00": {
      "code": "B25F1/00",
      "level": 7,
      "parent": null,
      "ja": "組合せ手工具または多目的手工具（単一の可搬形動力駆動装置といろいろな作業工具との組合せＢ２５Ｆ３／００；　｛ステープリング工具とパンチ工具の組合せＢ２５Ｃ５／０２１４｝）",
      "en": "Combination or multi-purpose hand tools (associations of tools for different working operations with one portable power-drive means B25F3/00; {combined stapling and punching tools B25C5/0214})",
      "count": 1598,
      "ipcCode": "B25F1/00",
      "coreCode": "B25F1/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F1/003": {
      "code": "B25F1/003",
      "level": 8,
      "parent": "B25F1/00",
      "ja": "｛少なくともひとつの可動ジョーをもつプライヤ，はさみまたはレンチタイプのもの（Ｂ２５Ｆ１／００６が優先）｝",
      "en": "{of pliers&apos;-, scissors&apos;- or wrench-type with at least one movable jaw (B25F1/006 takes precedence)}",
      "count": 779,
      "ipcCode": "B25F1/00",
      "coreCode": "B25F1/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F1/006": {
      "code": "B25F1/006",
      "level": 8,
      "parent": "B25F1/00",
      "ja": "｛衝撃ツールヘッドまたはツール刃をもつもの，例．ハンマー，斧｝",
      "en": "{with percussion tool-heads or -blades, e.g. hammers, axes}",
      "count": 868,
      "ipcCode": "B25F1/00",
      "coreCode": "B25F1/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F1/02": {
      "code": "B25F1/02",
      "level": 8,
      "parent": "B25F1/00",
      "ja": "交換可能または調整可能の工具要素をもつもの",
      "en": "with interchangeable or adjustable tool elements",
      "count": 1041,
      "ipcCode": "B25F1/02",
      "coreCode": "B25F1/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F1/04": {
      "code": "B25F1/04",
      "level": 9,
      "parent": "B25F1/02",
      "ja": "工具要素が回動または摺動して使用位置へ動かされるもの",
      "en": "wherein the elements are brought into working positions by a pivoting or sliding movement",
      "count": 1090,
      "ipcCode": "B25F1/04",
      "coreCode": "B25F1/04",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F3/00": {
      "code": "B25F3/00",
      "level": 7,
      "parent": null,
      "ja": "単一の可搬形動力装置といろいろな作業工具との組合せ；その組合せ用のアダプタ",
      "en": "Associations of tools for different working operations with one portable power-drive means; Adapters therefor",
      "count": 1597,
      "ipcCode": "B25F3/00",
      "coreCode": "B25F3/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/00": {
      "code": "B25F5/00",
      "level": 7,
      "parent": null,
      "ja": "作業の種類に特に関連せず，かつ他に分類されない携帯用動力駆動工具の細部または要素",
      "en": "Details or components of portable power-driven tools not particularly related to the operations performed and not otherwise provided for",
      "count": 5565,
      "ipcCode": "B25F5/00",
      "coreCode": "B25F5/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/001": {
      "code": "B25F5/001",
      "level": 8,
      "parent": "B25F5/00",
      "ja": "｛回転工具用に特に適合された歯車装置，スピードセレクタ，クラッチまたは類似のもの｝",
      "en": "{Gearings, speed selectors, clutches or the like specially adapted for rotary tools}",
      "count": 2042,
      "ipcCode": "B25F5/00",
      "coreCode": "B25F5/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/003": {
      "code": "B25F5/003",
      "level": 8,
      "parent": "B25F5/00",
      "ja": "｛回転式手工具の深さを制限するストッパー（掘削用ツール:Ｂ２３Ｂ４９／００３）｝",
      "en": "{Stops for limiting depth in rotary hand tools (for drilling tools: B23B49/003)}",
      "count": 233,
      "ipcCode": "B25F5/00",
      "coreCode": "B25F5/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/005": {
      "code": "B25F5/005",
      "level": 8,
      "parent": "B25F5/00",
      "ja": "｛液圧式駆動手段｝",
      "en": "{Hydraulic driving means}",
      "count": 368,
      "ipcCode": "B25F5/00",
      "coreCode": "B25F5/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/006": {
      "code": "B25F5/006",
      "level": 8,
      "parent": "B25F5/00",
      "ja": "｛振動減衰装置｝",
      "en": "{Vibration damping means}",
      "count": 1211,
      "ipcCode": "B25F5/00",
      "coreCode": "B25F5/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/008": {
      "code": "B25F5/008",
      "level": 8,
      "parent": "B25F5/00",
      "ja": "｛冷却手段｝",
      "en": "{Cooling means}",
      "count": 1157,
      "ipcCode": "B25F5/00",
      "coreCode": "B25F5/00",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/02": {
      "code": "B25F5/02",
      "level": 8,
      "parent": "B25F5/00",
      "ja": "ケーシング，本体またはハンドルの構造　｛（Ｂ２５Ｆ５／００６，Ｂ２５Ｆ５／００８が優先）｝",
      "en": "Construction of casings, bodies or handles {(B25F5/006, B25F5/008 take precedence)}",
      "count": 4444,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/021": {
      "code": "B25F5/021",
      "level": 9,
      "parent": "B25F5/02",
      "ja": "｛案内装置をもつもの（台または支持体の上の手工具用の案内装置Ｂ２５Ｈ１／００７８）｝",
      "en": "{with guiding devices (guiding devices for hand tools on stands or supports B25H1/0078)}",
      "count": 662,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/022": {
      "code": "B25F5/022",
      "level": 10,
      "parent": "B25F5/021",
      "ja": "｛レベルが組み込まれたもの｝",
      "en": "{with incorporated levels}",
      "count": 63,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/023": {
      "code": "B25F5/023",
      "level": 10,
      "parent": "B25F5/021",
      "ja": "｛取外し可能なレベルをもつもの｝",
      "en": "{with removably attached levels}",
      "count": 61,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/024": {
      "code": "B25F5/024",
      "level": 11,
      "parent": "B25F5/023",
      "ja": "｛補助柄の一部としてのもの｝",
      "en": "{as part of an auxiliary handle}",
      "count": 49,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/025": {
      "code": "B25F5/025",
      "level": 9,
      "parent": "B25F5/02",
      "ja": "｛回転工具用のトルク反作用バーをもつもの｝",
      "en": "{with torque reaction bars for rotary tools}",
      "count": 32,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/026": {
      "code": "B25F5/026",
      "level": 10,
      "parent": "B25F5/025",
      "ja": "｛補助柄の形態のものＦ（Ｂ２５Ｆ５／０２４が優先）｝",
      "en": "{in the form of an auxiliary handle (B25F5/024 takes precedence)}",
      "count": 652,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/027": {
      "code": "B25F5/027",
      "level": 11,
      "parent": "B25F5/026",
      "ja": "｛貯蔵区分をもつもの｝",
      "en": "{with storage compartments}",
      "count": 47,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/028": {
      "code": "B25F5/028",
      "level": 10,
      "parent": "B25F5/025",
      "ja": "｛固定物体にサポートされるためのもの｝",
      "en": "{to be supported by a fixed object}",
      "count": 22,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    },
    "B25F5/029": {
      "code": "B25F5/029",
      "level": 9,
      "parent": "B25F5/02",
      "ja": "｛貯蔵区分をもつもの（Ｂ２５Ｆ５／０２７が優先；工具要素を保存するための装備をもつ柄Ｂ２５Ｃ１／０８）｝",
      "en": "{with storage compartments (B25F5/027 takes precedence; handles with provisions for storing tool elements B25C1/08)}",
      "count": 350,
      "ipcCode": "B25F5/02",
      "coreCode": "B25F5/02",
      "note": "",
      "sourceFile": "cpc_B25F.txt",
      "kind": "cpc"
    }
  }
};

window.CPC_SHARD_B44D = {
  "generatedAt": "2026-03-20T02:17:56.716Z",
  "total": 39,
  "entries": {
    "B44D": {
      "code": "B44D",
      "level": 5,
      "parent": "B44",
      "ja": "他に分類されない絵画または美術的描画；絵画の保存；特殊な美術的表面効果または表面仕上げを得るための表面処理（液体，例．塗料，または他の流動性材料の表面への適用一般Ｂ０５）　",
      "en": "PAINTING OR ARTISTIC DRAWING, NOT OTHERWISE PROVIDED FOR; PRESERVING PAINTINGS; SURFACE TREATMENT TO OBTAIN SPECIAL ARTISTIC SURFACE EFFECTS OR FINISHES (surface treatment in general, see the relevant places, e.g. applying liquids or other fluent materials B05)<br><br><u>WARNING</u><br><br>In this subclass non-limiting references (in the sense of paragraph 39 of the Guide to the IPC) may still be displayed in the scheme.",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B44": {
      "code": "B44",
      "level": 4,
      "parent": "B",
      "ja": "装飾技術<br>いかなる機械，装置，工具，または方法も，それが目により判断されることを予定された外観またはマークを作り出す限り，そしてこのような機械，装置，工具または方法が他に分類されない限りこのクラスに分類される。",
      "en": "DECORATIVE ARTS<br><br><u>NOTE</u><br><br>Any machine, apparatus, tool or process is classified in this class insofar as it produces an effect or mark meant to be judged by the eye and insofar as such machine, apparatus, tool or process is not provided for elsewhere",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc_B.txt",
      "kind": "cpc"
    },
    "B": {
      "code": "B",
      "level": 2,
      "parent": null,
      "ja": "処理操作；運輸",
      "en": "PERFORMING OPERATIONS; TRANSPORTING<br><br><u>NOTE</u><br><br>The following notes are meant to assist in the use of classes B01&#160;-&#160;B09; they must not be read as modifying in any way the elaborations.<br>In this sub-section, the separation of different materials, e.g. of different matter, size, or state, is predominantly found in the following subclasses: <br>B01D<br>B03B, B03C, B03D<br>B04B, B04C<br>B07B, B07C<br>The classifying characteristics of these subclasses are: <br>the physical state of the matter to be separated <br>the principle of the process used <br>particular kinds of apparatus The first of these characteristics involves six different aspects, assembled in three groups.<br>liquid/liquid or liquid/gas and gas/gas <br>solid/liquid or solid/gas <br>solid/solid<br>Theses subclasses are to be used according to the following general rules: <br>B01D is the most general class as far as separation other  than solids from solids is concerned. <br>Apparatus for separating solids from solids are covered by B03B when the process concerned is regarded as the equivalent of \"washing\" in the sense of the mining art, even if such apparatus is a pneumatic one, especially pneumatic tables or jigs. Screens PER SE are not covered by this subclass but are classified in B07B, even if they are being used in a wet process. All other apparatus for the separation of solids from solids according to dry methods are classified in B07B. <br>If the separation takes place as a result of the detection or measurement of some feature of the material or articles to be sorted it is classified in B07C. <br>It should also be noted that the separation of isotopes of the same chemical element is covered by B01D59/00, whatever process or apparatus is employed.<br>The following scheme illustrates the classification according to these rules. <br>LIQUID/LIQUID <br>      LIQUID/GAS <br>      GAS/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force, using centrifuges <br>    or free-vortex apparatus                   B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br>SOLID/LIQUID <br>      SOLID/GAS <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B01DB01D<br>    by centrifugal force                       B01DB01D<br>       using centrifuges or free-vortex <br>       apparatus                               B01DB04B, B04C<br>    using magnetic or electrostatic effect     B03CB03C<br>  _____________________________________________________________________ <br><br><br>SOLID/SOLID <br>  _____________________________________________________________________ <br>  Dry Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General for material in bulk                 B07BB07B<br>  Individual sorting                           B07CB07C<br>     Screening, sifting, pneumatic sorting     B07BB07B<br>       using pneumatic tables or jigs          B03BB03B<br>     by magnetic or electrostatic effect       B03CB03C<br>     by centrifugal force                      B07BB07B<br>       using centrifuges or free-vortex <br>       apparatus                               B07BB04B, B04C<br>  _____________________________________________________________________ <br>  Wet Methods <br>  _____________________________________________________________________ <br>                                             Subclasses dealing with <br>  Operations                                   Method    Apparatus <br>  _____________________________________________________________________ <br>  General                                      B03BB03B<br>     flotation, differential sedimentation     B03DB03D<br>     screening                                 B07BB07B<br>  _____________________________________________________________________ <br>  Combinations = dry methods - wet methods: B03B<br>  _____________________________________________________________________<br>",
      "count": 0,
      "ipcCode": "",
      "coreCode": "",
      "note": "",
      "sourceFile": "cpc.txt",
      "kind": "cpc"
    },
    "B44D2/00": {
      "code": "B44D2/00",
      "level": 7,
      "parent": null,
      "ja": "美術的塗装または絵画，例．油絵，水彩画，パステル画，浮上ったように描く絵画，の特殊な技法",
      "en": "Special techniques in artistic painting or drawing, e.g. oil painting, water painting, pastel painting, relief painting",
      "count": 1027,
      "ipcCode": "B44D2/00",
      "coreCode": "B44D2/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D2/002": {
      "code": "B44D2/002",
      "level": 8,
      "parent": "B44D2/00",
      "ja": "｛絵画または塗装のための道具｝",
      "en": "{Kits for drawing or painting}",
      "count": 352,
      "ipcCode": "B44D2/00",
      "coreCode": "B44D2/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D2/005": {
      "code": "B44D2/005",
      "level": 8,
      "parent": "B44D2/00",
      "ja": "｛浮き上がったように描く絵画（はけ筋の模倣Ｂ４４Ｆ１１／０２）｝",
      "en": "{Relief painting (imitating brush strokes B44F11/02)}",
      "count": 52,
      "ipcCode": "B44D2/00",
      "coreCode": "B44D2/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D2/007": {
      "code": "B44D2/007",
      "level": 8,
      "parent": "B44D2/00",
      "ja": "｛美術的絵画または塗装でステンシルを用いるもの｝",
      "en": "{using stencils in artistic drawing or painting operations}",
      "count": 171,
      "ipcCode": "B44D2/00",
      "coreCode": "B44D2/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/00": {
      "code": "B44D3/00",
      "level": 7,
      "parent": null,
      "ja": "他類に属さない，塗装または絵画に関連して使用する付属品または器具（液体，例．ペイント，を表面に適用する手工具Ｂ０５Ｃ１７／００；塗装以外の，建築物の仕上げのための器具Ｅ０４Ｆ２１／００）；色の決定，選択，または合成のための方法または装置，例．色彩表の使用（色の測定Ｇ０１Ｊ３／００）",
      "en": "Accessories or implements for use in connection with painting or artistic drawing, not otherwise provided for (hand tools for applying liquids, e.g. paints, to surfaces B05C17/00, implements for finishing work on buildings, other than painting, E04F21/00); Methods or devices for colour determination, selection, or synthesis, e.g. use of colour tables (colorimetry G01J3/00)",
      "count": 1626,
      "ipcCode": "B44D3/00",
      "coreCode": "B44D3/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/003": {
      "code": "B44D3/003",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "｛色の決定，選択または合成のための方法または装置，例．色彩表の使用｝",
      "en": "{Methods or devices for colour determination, selection or synthesis, e.g. use of colour tables}",
      "count": 441,
      "ipcCode": "B44D3/00",
      "coreCode": "B44D3/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/006": {
      "code": "B44D3/006",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "｛使用後の塗装手持ち道具の洗浄のための装置（表面から乾いたペンキの除去Ｂ４４Ｄ３／１６；乾いたペンキを溶解するための装置Ｂ４４Ｄ３／２４；ブラシの洗浄用装置Ａ４６Ｂ１７／０６；洗浄一般Ｂ０８Ｂ）｝",
      "en": "{Devices for cleaning paint-applying hand tools after use (removing dry paint from surfaces B44D3/16; apparatus for dissolving dry paints B44D3/24; devices for cleaning brushes A46B17/06; cleaning in general B08B)}",
      "count": 885,
      "ipcCode": "B44D3/00",
      "coreCode": "B44D3/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/02": {
      "code": "B44D3/02",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "パレット",
      "en": "Palettes",
      "count": 606,
      "ipcCode": "B44D3/02",
      "coreCode": "B44D3/02",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/04": {
      "code": "B44D3/04",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "絵の具箱",
      "en": "Paint boxes",
      "count": 686,
      "ipcCode": "B44D3/04",
      "coreCode": "B44D3/04",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/06": {
      "code": "B44D3/06",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "ペイントを攪拌または混合する器具（混合一般Ｂ０１Ｆ）",
      "en": "Implements for stirring or mixing paints",
      "count": 333,
      "ipcCode": "B44D3/06",
      "coreCode": "B44D3/06",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/08": {
      "code": "B44D3/08",
      "level": 9,
      "parent": "B44D3/06",
      "ja": "液状，半液状の絵の具のためのもの",
      "en": "for liquid or semi-liquid paints",
      "count": 330,
      "ipcCode": "B44D3/08",
      "coreCode": "B44D3/08",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/10": {
      "code": "B44D3/10",
      "level": 9,
      "parent": "B44D3/06",
      "ja": "こし具，へら",
      "en": "Sieves; Spatulas",
      "count": 100,
      "ipcCode": "B44D3/10",
      "coreCode": "B44D3/10",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/105": {
      "code": "B44D3/105",
      "level": 10,
      "parent": "B44D3/10",
      "ja": "｛へら｝",
      "en": "{spatulas}",
      "count": 135,
      "ipcCode": "B44D3/10",
      "coreCode": "B44D3/10",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/12": {
      "code": "B44D3/12",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "ペイントかん，ブラシ保持具；残ったペイントを入れておくためのもの　｛（一般的な容器の細部構造Ｂ６５Ｄ）｝",
      "en": "Paint cans; Brush holders; Containers for storing residual paint {(construction details of containers of general type B65D)}",
      "count": 932,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/121": {
      "code": "B44D3/121",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛ブラシ保持具が取り外せないように固定されたペイントかん｝",
      "en": "{Paint cans equipped with permanently attached brush holding means}",
      "count": 219,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/122": {
      "code": "B44D3/122",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛異なる塗装化合物のための分離区画を有するもの｝",
      "en": "{having separate compartments for the different paint compounds}",
      "count": 149,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/123": {
      "code": "B44D3/123",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛ペイントかんから独立したブラシ保持具，例．ペイントかんから分離可能に取り付けられる保持具｝",
      "en": "{Brush holders independent from paint can, e.g. holders removably attached to paint can}",
      "count": 785,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/125": {
      "code": "B44D3/125",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛塗装作業中に缶から分離し，ペンキ用はけ及びその類似物を保管するための容器｝",
      "en": "{Containers for storing paint brushes and the like, separate from the can used in painting operation}",
      "count": 461,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/126": {
      "code": "B44D3/126",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛塗装用ローラーのトレイ｝",
      "en": "{Paint roller trays}",
      "count": 577,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/127": {
      "code": "B44D3/127",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛塗料缶のカバーまたはふた（一般的な容器のカバーまたはふたＢ６５Ｄ）｝",
      "en": "{Covers or lids for paint cans (covers or lids for containers of general type B65D)}",
      "count": 478,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/128": {
      "code": "B44D3/128",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "｛拭き取り棒；縁の保護具；しずく受け；こぼれ取り具｝",
      "en": "{Wiping bars; Rim protectors; Drip trays; Spill catchers}",
      "count": 706,
      "ipcCode": "B44D3/12",
      "coreCode": "B44D3/12",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/14": {
      "code": "B44D3/14",
      "level": 9,
      "parent": "B44D3/12",
      "ja": "ペイントかんの保持具",
      "en": "Holders for paint cans",
      "count": 583,
      "ipcCode": "B44D3/14",
      "coreCode": "B44D3/14",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/16": {
      "code": "B44D3/16",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "表面から乾燥したペイントを取除くための器具または装置，例．はぎ取りによるもの，焼くことによるもの（化学的なペイント除去剤Ｃ０９Ｄ９／００）",
      "en": "Implements or apparatus for removing dry paint from surfaces, e.g. by scraping, by burning (chemical paint-removers C09D9/00)",
      "count": 275,
      "ipcCode": "B44D3/16",
      "coreCode": "B44D3/16",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/162": {
      "code": "B44D3/162",
      "level": 9,
      "parent": "B44D3/16",
      "ja": "｛掻き取り具｝",
      "en": "{Scrapers}",
      "count": 519,
      "ipcCode": "B44D3/16",
      "coreCode": "B44D3/16",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/164": {
      "code": "B44D3/164",
      "level": 10,
      "parent": "B44D3/162",
      "ja": "｛刃からなるもの｝",
      "en": "{comprising blades}",
      "count": 250,
      "ipcCode": "B44D3/16",
      "coreCode": "B44D3/16",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/166": {
      "code": "B44D3/166",
      "level": 9,
      "parent": "B44D3/16",
      "ja": "｛加熱によるもの，例．焼成によるもの｝",
      "en": "{by heating, e.g. by burning}",
      "count": 146,
      "ipcCode": "B44D3/16",
      "coreCode": "B44D3/16",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/168": {
      "code": "B44D3/168",
      "level": 10,
      "parent": "B44D3/166",
      "ja": "｛｛電気的加熱によるもの（電気的エネルギーの供給を用いた空気加熱器，加熱媒体と直接接触する空気，Ｆ２４Ｈ３／０４０５）｝",
      "en": "{by electrically heating (air heaters using electric energy supply, the air being in direct contact with the heating medium, F24H3/0405)}",
      "count": 198,
      "ipcCode": "B44D3/16",
      "coreCode": "B44D3/16",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/18": {
      "code": "B44D3/18",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "絵を描く面をもつ板またはシート；カンバスの張り枠",
      "en": "Boards or sheets with surfaces prepared for painting or drawing pictures; Stretching frames for canvases",
      "count": 1397,
      "ipcCode": "B44D3/18",
      "coreCode": "B44D3/18",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/185": {
      "code": "B44D3/185",
      "level": 9,
      "parent": "B44D3/18",
      "ja": "｛カンバスの張り枠｝",
      "en": "{Stretching frames for canvases}",
      "count": 661,
      "ipcCode": "B44D3/18",
      "coreCode": "B44D3/18",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/22": {
      "code": "B44D3/22",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "特殊技術のための機械器具，例．線を画くためのもの，ワニス注入のためのもの；ろう染め鉛筆",
      "en": "Implements or apparatus for special techniques, e.g. for painting lines, for pouring varnish; Batik pencils",
      "count": 641,
      "ipcCode": "B44D3/22",
      "coreCode": "B44D3/22",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/225": {
      "code": "B44D3/225",
      "level": 9,
      "parent": "B44D3/22",
      "ja": "｛線を描くための機器または器具｝",
      "en": "{Instruments or apparatus for painting lines}",
      "count": 457,
      "ipcCode": "B44D3/22",
      "coreCode": "B44D3/22",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/24": {
      "code": "B44D3/24",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "ラッカーを焼きつけるランプ；塗装者の使うベルト；乾いたペイントを溶かす装置；ペイントを温める装置",
      "en": "Lamps for baking lacquers; Painters belts; Apparatus for dissolving dried paints, for heating paints",
      "count": 154,
      "ipcCode": "B44D3/24",
      "coreCode": "B44D3/24",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D3/38": {
      "code": "B44D3/38",
      "level": 8,
      "parent": "B44D3/00",
      "ja": "紐線引き",
      "en": "Cord line chalkers",
      "count": 368,
      "ipcCode": "B44D3/38",
      "coreCode": "B44D3/38",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D5/00": {
      "code": "B44D5/00",
      "level": 7,
      "parent": null,
      "ja": "特殊な美術的表面効果または表面仕上げを得るための表面処理（液体を塗布することにより被覆される表面の前処理または後処理Ｂ０５Ｄ３／００；液体または他の流動性材料を表面に塗布することにより特殊な表面効果を得るものＢ０５Ｄ５／００）",
      "en": "Surface treatment to obtain special artistic surface effects or finishes (pretreatment or after-treatment of surface coated by applying liquids B05D3/00; obtaining special surface effects by applying liquids or other fluent materials to surfaces B05D5/00; surface shaping of plastics, e.g. embossing, B29C59/00)",
      "count": 332,
      "ipcCode": "B44D5/00",
      "coreCode": "B44D5/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D5/10": {
      "code": "B44D5/10",
      "level": 8,
      "parent": "B44D5/00",
      "ja": "機械的処理",
      "en": "Mechanical treatment",
      "count": 152,
      "ipcCode": "B44D5/10",
      "coreCode": "B44D5/10",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    },
    "B44D7/00": {
      "code": "B44D7/00",
      "level": 7,
      "parent": null,
      "ja": "絵画の保存，例．ワニスがけによるもの",
      "en": "Preserving paintings, e.g. by varnishing",
      "count": 295,
      "ipcCode": "B44D7/00",
      "coreCode": "B44D7/00",
      "note": "",
      "sourceFile": "cpc_B44D.txt",
      "kind": "cpc"
    }
  }
};
